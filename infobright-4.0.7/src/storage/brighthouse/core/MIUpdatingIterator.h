 ...
 *       it.ResetCurrent();			// deletion may be delayed - do not access multiindex by other
 * iterators!
 *   ...
 *   ++it;
 * }
...
class MIUpdatingIterator : public MIIterator
{
 ...
 public:
  ...
  {
    track_changes = true;
    mind->GetFilter(one_filter_dim)->StartTrackingChanges();
  }
  ...
  bool SwapPackFilter(int pack, Filter *f);  // swap the contents of the given pack; do not swap and return true if the
                                             // pack in the current iterator is not full
