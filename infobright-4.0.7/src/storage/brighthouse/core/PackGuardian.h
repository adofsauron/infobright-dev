...
enum PG_Status
{
  PG_UNKNOWN = 0,  // status unknown (pack not used yet) - it is never returned by PackRequest() as a result
...
class VCPackGuardian
{
 ...
 protected:
  ...
  bool initialized;  // false if the object was not initialized yet.
                     // To be done at the first use, because it is too risky to init in constructor (multiindex may not
                     // be valid yet)
  ...
  // Performance note: it is assumed that dimensions involved in LOCK_ALL are rather small, thus the whole pack status
...
public:
        PackGuardian(	JustATable* _t,			// table to be guarded (switched off if the table is TempTable)
                                                int _n_a,			// attribute number (-1 means a
deactivated (fake) PackGuardian) ConnectionInfo* _conn, int divide_by = 1	// number of pack guardians in memory -
for better approximation
                                                );
        PackGuardian(	JustATable* _t,			// table to be guarded (switched off if the table is TempTable)
                                                std::vector<int>& v_a,	// attributes to be guarded (all locked at the
same time) ConnectionInfo* _conn
                                                );
        PackGuardian(	JustATable* _t,			// table to be guarded (switched off if the table is TempTable)
                                                const Descriptor& desc, 	// attributes used in this descriptor
will be guarded (all locked at the same time) ConnectionInfo* _conn
                                                );
        PackGuardian(	std::vector<std::pair<JustATable*,int> >& v_a		// tables/attributes to be guarded (all
locked at the same time)
                                                );			// NOT WORKING YET
        ~PackGuardian();

        PG_Status PackRequest(int pack);
                                                                        // try to lock the pack (if not already locked)
for all guarded columns

        bool AnythingLeft()				{ return anything_left; }		// was any pack set to
PG_LATER mode?

        void UnlockOne(int pack);					// manual unlocking, may be used for some
strategies
        void Rewind();								// reset all pack status PG_READY ->
PG_DONE, unlock packs
        void Clear();								// reset all pack status to PG_UNKNOWN,
unlock packs

        void SetStrategyUnlockAllOnRewind()	{ current_strategy = UNLOCK_ALL_ON_REWIND; }	// NOTE: usually needs
also Clear() void SetStrategyUnlockOldest()	{ current_strategy = UNLOCK_OLDEST; }
        ...
        enum { 	UNLOCK_OLDEST,			// remember when the pack was requested
                                                                        // and unlock the oldest used if required;
always one-pass scanning UNLOCK_ALL_ON_REWIND,	// deny access to a pack not in memory, if no place for loading it
                        LOCK_ONE,				// lock the desired pack, unlock the previous one (for
sequential access) LOCK_OLDEST				// lock all packs till the end of limit, then lock/unlock only
one;
                                                                        // may require manual unlocking of packs no more
needed } current_strategy;
...
private:
        ...
        void AddAttrsFromCQTerm(const CQTerm& cq, std::vector<int>& attrno);	//extract attr numbers used in a CQTerm,
add them to attrno. _int64 old_pack_requests;		// a counter of requesting packs

        bool anything_left;
        PG_Status *pstat;		// the current table of pack status: PG_UNKNOWN if a pack is not used yet (may
be locked), PG_READY for locked packs, PG_DONE for previously used _int64 *requested_at;	// the freshest request
number for this pack _int64 request_counter;	// count only newly realized requests (new locks) int pack_counter;
// the counter of locked packs int pack_limit;			// above this limit we will not lock more packs int last_locked_pack;	// the last pack locked in LOCK_ONE strategy (or -1) ConnectionInfo& conn;	// an external
pointer to the current session info (for e.g. pack statistics)
};
*/
