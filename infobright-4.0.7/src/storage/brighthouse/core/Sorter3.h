...
class Sorter3 : public TrackableObject
{
 public:
  ...
  {
    return 0;
  }  // the first stage of conserving memory: delete all redundant data (retrievable from the memory)
 ...
 private:
  ...
  class Keyblock
  {
   public:
    ...
    {
      return key_bytes > 0 && memcmp(rec, x.rec, key_bytes) > 0;
  ...
  Keyblock GetFromBlock(
      int b, bool &reloaded);  // get the next value from block, return (-1, NULL, 0) if the block is empty
