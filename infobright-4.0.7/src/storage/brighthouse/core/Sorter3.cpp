...
Sorter3 *Sorter3::CreateSorter(_int64 size, uint key_bytes, uint total_bytes, _int64 limit, int mem_modifier)
{
  ...
  if (size * total_bytes * 2 > 64 * MBYTE)
    max_mem_size = TrackableObject::MaxBufferSize(mem_modifier) *
...
SorterOnePass::SorterOnePass(uint _size, uint _key_bytes, uint _total_bytes) : Sorter3(_size, _key_bytes, _total_bytes)
{
...
void SorterOnePass::QuickSort()
{
  ...
  while (queue_read != queue_write)     // this loop is instead of Quicksort recurrency
  {
    ...
    mid_val =
        s1 + total_bytes * (_int64((s2 - s1) / total_bytes) / 8) * r;  // middle position, somewhere between 2/8 and 5/8
    i = s1;
    j = s2;
    do
    {
      while (memcmp(i, mid_val, key_bytes) < 0) i += total_bytes;
      while (memcmp(j, mid_val, key_bytes) > 0) j -= total_bytes;
      if (i <= j)
      {
        if (i != j)
        {
    ...
    if (s1 < j)
    {  // add new intervals to sort queue
...
void SorterMultiPass::InitHeap()
{
  ...
  if (rows_in_small_buffer < 3)
  {  // extremely rare case: very large data, very small memory
...
SorterMultiPass::Keyblock SorterMultiPass::GetFromBlock(int b, bool &reloaded)
{
  ...
  if (blocks[b].read_offset == -1)
  {  // new buffer, to be read
    if (blocks[b].file_offset > 0)
    {  // preserve the last row before overwriting the whole buffer
...
void SorterCounting::CountingSort()
{
  ...
  for (int i = 0; i < distrib_size; i++) distrib[i] = 0;
  unsigned char *p = buf;
  while (p < buf_input_pos)
  {  // first pass: count all cases
  ...
  p = buf;
  while (p < buf_input_pos)
  {  // second pass: move to the proper position
    int pos = Position(p) - distrib_min;
...
bool SorterLimit::PutValue(unsigned char *b)
{
  already_sorted = false;
  if (key_bytes == 0)
  {  // case of constant values: just copy the first rows
    if (no_obj < size)
    {
  ...
  {
    if (memcmp(buf, b, key_bytes) > 0)
    {  // compare with the first position
      uint n = 1;
      uint child = 2;
      uint child_greater;
      ...
      while (cont)
      {
        if (child == size || memcmp(buf + (child - 1) * total_bytes, buf + child * total_bytes, key_bytes) > 0)
    ...
    {  // check whether it is not sorted already
