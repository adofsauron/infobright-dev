...
JoinerMapped::JoinerMapped(MultiIndex *_mind, RoughMultiIndex *_rmind, TempTable *_table, JoinTips &_tips)
...
void JoinerMapped::ExecuteJoinConditions(Condition &cond)
{
  ...
  ///////////////////////////////////////////////////////////////////////////
  bool outer_join =
      !(desc->right_dims.IsEmpty());  // traversed is outer: scan facts, add nulls for all non-matching dimension values
  ...
  {  // outer join on keys verified as unique - a trivial answer
  ...
  int single_filter_dim = new_mind.OptimizedCaseDimension();  // indicates a special case: the fact table remains a
                                                              // filter and the dimension table is forgotten
  ...
  while (mit.IsValid())
  {
    ...
    if (mit.PackrowStarted())
    {
      ...
      {
        if (new_mind.CommitPack(mit.GetCurPackrow(single_filter_dim)))
        {  // processed as a pack
...
JoinerMapFunction *JoinerMapped::GenerateFunction(VirtualColumn *vc)
{
  ...
  rccontrol.lock(m_conn.GetThreadID()) << "Join mapping (offset map) created on " << traversed_rows << " rows."
                                       << unlock;
  return map_function;
}
...
OffsetMapFunction::~OffsetMapFunction() { delete[] key_status; }
...
bool OffsetMapFunction::Init(VirtualColumn *vc, MIIterator &mit)
{
  ...
  if (key_table_min == NULL_VALUE_64 || key_table_min == MINUS_INF_64 || key_table_max == PLUS_INF_64 ||
      key_table_max == NULL_VALUE_64)
    return false;
