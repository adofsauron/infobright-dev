...
bool MIUpdatingIterator::SwapPackFilter(
    int pack,
    Filter *f)  // swap the contents of the given pack; return false without swapping if the current pack is full
{
  assert(one_filter_it);
  Commit(false);
  Filter *f_old = mind->GetFilter(one_filter_dim);
