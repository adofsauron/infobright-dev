...
class MIIterator
{
 public:
  ...
  {
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(valid && n >= 0 && n < no_dims);
    return cur_pos[n];
  }
  ...
  inline MIIterator &operator++()
  {
    ...
    if (one_filter_dim > -1)
    {  // Optimized (one-dimensional filter) part:
      ++(*one_filter_it);
    ...
    {  // General part:
      GeneralIncrement();
      if (pack_size_left != NULL_VALUE_64)
      {  // else the packrow is too large to be iterated through
        --pack_size_left;
        if (pack_size_left == 0)
          InitNextPackrow();
      }
  ...
  /*! Get the number of rows omitted in unused dimensions on each step of iterator. Used e.g. as a factor in sum(...),
   * count(...).
   *
  ...
  /*! True, if there were any null object numbers (NOT null values!) possibly found in the current packrow for a given
   * dimension \warning True may mean that the null value is possible, but does not guarantee it!
   */
  ...
  {
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(valid && dim >= 0 && dim < no_dims);
    return int(cur_pos[dim] & 0x000000000000FFFFLL);
  }
 ...
 private:
  ...
  inline void InitNextPackrow()
  {  // find boundaries of the current packrow,
