...
JoinerHash::JoinerHash(MultiIndex *_mind, RoughMultiIndex *_rmind, TempTable *_table, JoinTips &_tips)
...
void JoinerHash::ExecuteJoinConditions(Condition &cond)
{
  ...
  if (min(dim1_size, dim2_size) > 100000)
  ...
  {
    if (traversed_dims[i] && !(tips.count_only && !other_cond_exist))
    {  // storing tuples may be omitted if (count_only_now && !other_cond_exist)
...
void JoinerHash::ExecuteJoin()
{
    ...
    while (tr_mit.IsValid())
    {
      ...
      if (tr_mit.IsValid())
        rccontrol.lock(m_conn.GetThreadID())
            << "Traversed " << traversed_tuples << "/" << dim1_size << " rows." << unlock;
      else
  ...
  for (int i = 0; i < cond_hashed; i++)
    if (!vc2[i]->Type().IsString())
    {  // otherwise different collations may be a counterexample
      vc2[i]->SetLocalDistVals(
          actually_traversed_rows +
          outer_tuples_matched);    // matched values: not more than the traversed ones in case of equality
...
// new_mind and outer_tuples (the counter) are used only for outer joins
{
  ...
  for (int i = 0; i < cond_hashed; i++) vc1[i]->InitPrefetching(mit);
  if (watch_traversed)
  ...
  while (mit.IsValid())
  {
    ...
    if (!omit_row)
    {  // else go to the next row - equality cannot be fulfilled
...
_int64 JoinerHash::MatchDim(MINewContents &new_mind, MIIterator &mit)
{
  ...
  for (int i = 0; i < cond_hashed; i++) vc2[i]->InitPrefetching(mit);
  ...
  while (mit.IsValid())
  {
    ...
    if (mit.PackrowStarted())
    {
      ...
      for (int i = 0; i < cond_hashed; i++)
      {
        ...
        {
          if (!vc2[i]->Type().IsLookup())
          {  // lookup treated as string, when the dictionaries aren't convertible
      ...
      if (packrow_uniform && !omit_this_packrow)
      {
        ...
        {
          while ((hash_row = jhash.GetNextRow()) != NULL_VALUE_64) outer_filter->Reset(hash_row);
        }
...
_int64 JoinerHash::SubmitOuterMatched(MIIterator &mit, MINewContents &new_mind)
{
  ...
  while (mit.IsValid())
  {
    if (outer_filter->Get(matched_row))
    {  // if the matched tuple is still unused, add it with nulls
      for (int i = 0; i < mind->NoDimensions(); i++)
      {
        if (matched_dims[i])
          new_mind.SetNewTableValue(i, mit[i]);
