...
/**
 ...
 table/TempTable attr_id, tab_id					attribute value of a given table/TempTable func,
 attr_id					attribute value of the current table/TempTable with a simple function
 applied func, attr_id, tab_id			attribute value of a given table/TempTable with a simple function
 applied tab_id							value of a TempTable (execution of a subquery) param_id
 parameter value c_term							complex expression, defined elsewhere (?) Note:
 if all above are not used, it is interpreted as a constant (even if null) val_n
