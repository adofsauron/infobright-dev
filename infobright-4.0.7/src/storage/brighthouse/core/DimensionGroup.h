...
class DimensionVector
{
 public:
  ...
  bool &operator[](int i)
  {
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(i >= 0 && i < size);
    return v[i];
  }
...
class DimensionGroup
{
 public:
  ...
  virtual Filter *GetFilter(int dim) const
  {
    return NULL;
  }  // Get the pointer to a filter attached to a dimension. NOTE: will be NULL if not applicable
  virtual Filter *GetUpdatableFilter(int dim) const
  {
    return NULL;
  }  // Get the pointer to a filter, if it may be changed. NOTE: will be NULL if not applicable
  ...
  class Iterator
  {
   public:
    ...
    virtual bool WholePack(int dim) = 0;  // True, if the current packrow contain exactly the whole pack for a dimension
                                          // (no repetitions, no null objects)
    virtual bool InsideOnePack() = 0;     // true if there is only one packrow possible for dimensions in this iterator
    virtual bool NullsExist(
        int dim) = 0;  // return true if there exist any 0 value (always false for virtual dimensions)
    virtual void NextPackrow() = 0;
    virtual _int64 GetCurPos(int dim) = 0;
    virtual int GetCurPackrow(int dim) = 0;
    virtual int GetNextPackrow(
        int dim, int ahead) = 0;  // ahead = 1 => the next packrow after this one, etc., return -1 if not known
    ...
    {
      return false;
    }                                      // true if the subclass implements in-group updating functions, like below:
  ...
  virtual DimensionGroup::Iterator *NewIterator(
      DimensionVector &) = 0;  // create a new iterator (to be deleted by user)
  virtual DimensionGroup::Iterator *NewOrderedIterator(DimensionVector &, PackOrderer *po)
  {
    return NULL;
  ...
  virtual void FillCurrentPos(DimensionGroup::Iterator *it, _int64 *cur_pos, int *cur_pack,
                              DimensionVector &dims) = 0;  // fill only relevant dimensions basing on a local iterator
...
class DimensionGroupFilter : public DimensionGroup
{
 public:
  ...
  DimensionGroupFilter(int dim, Filter *f_source,
                       int copy_mode = 0);  // copy_mode: 0 - copy filter, 1 - ShallowCopy filter, 2 - grab pointer
  ...
  class DGFilterIterator : public DimensionGroup::Iterator
  {
   public:
    ...
    virtual bool RewindToPack(int pack)
    {
  ...
  {
   public:
    DGFilterOrderedIterator(Filter *f_to_iterate, PackOrderer *po) : fi(f_to_iterate, po), f(f_to_iterate)
    {
      valid = !f->IsEmpty();
    }
  ...
  virtual DimensionGroup::Iterator *NewOrderedIterator(DimensionVector &,
                                                       PackOrderer *po);  // create a new ordered iterator, if possible
...
class DimensionGroupMaterialized : public DimensionGroup
{
 public:
  ...
  void NewDimensionContent(
      int dim, IndexTable *tnew,
      bool nulls);  // tnew will be added (as a pointer to be deleted by destructor) on a dimension dim
  ...
  class DGMaterializedIterator : public DimensionGroup::Iterator
  {
   public:
    ...
    virtual _int64 GetCurPos(int dim)
    {
      _int64 v = t[dim]->Get64(cur_pos);
      return (v == 0 ? NULL_VALUE_64 : v - 1);
    }
   ...
   private:
    ...
    bool *one_packrow;  // dimensions containing values from one packrow only (no nulls). Approximation: false => still
                        // may be one packrow
    bool *nulls_found;  // in these dimensions nulls were found in this pack
    int *cur_pack;      // the number of the current pack (must be stored separately because there may be nulls on the
                        // beginning of pack
    _int64 *next_pack;  // beginning of the next pack (or no_obj if there is no other pack)
    _int64 *ahead1, *ahead2,
        *ahead3;  // beginning of the three next packs after next_pack, or -1 if not determined properly
