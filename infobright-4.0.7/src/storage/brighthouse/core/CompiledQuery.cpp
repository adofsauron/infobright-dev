...
CompiledQuery::~CompiledQuery() { steps.clear(); }
...
void CompiledQuery::CQStep::Print(Query *query)
{
  ...
  switch (type)
  {
    ...
    case CREATE_CONDS:
      if (c2.IsNull())
      {
        sprintf(buf, "C:%d = CREATE_%sCONDS(T:%d,%s,%s,%s,%s)", N(c1.n), n1 == OR_SUBTREE ? "TREE_" : "", N(t1.n),
    ...
    //	break;
    case AND_DESC:
      sprintf(buf, "C:%d.AND(%s,%s,%s,%s)", N(c1.n), e1.ToString(b1, _countof(b1), t1.n), b_op,
              e2.ToString(b2, _countof(b2), t1.n), e3.ToString(b3, _countof(b3), t1.n));
      break;
    case OR_DESC:
      sprintf(buf, "C:%d.OR(%s,%s,%s,%s)", N(c1.n), e1.ToString(b1, _countof(b1), t1.n), b_op,
              e2.ToString(b2, _countof(b2), t1.n), e3.ToString(b3, _countof(b3), t1.n));
      break;
    case TMP_TABLE:
    {
      ...
      int i = 0;
      for (; i < tables1.size() - 1; i++) sprintf(buf + strlen(buf), "T:%d,", N(tables1[i].n));
    ...
    case CREATE_VC:
      ...
      else if (mysql_expr.size() > 1)
      {
        ...
        s1[199] = '\0';
        s2[199] = '\0';
        sprintf(buf, "VC:%d.%d = CREATE_VC(T:%d,EXPR([%s,..,%s](%ld items)))", N(t1.n), N(a1.n), N(t1.n), s1, s2,
                mysql_expr.size());
      }
      else if (virt_cols.size() > 1)
      {
        sprintf(buf, "VC:%d.%d = CREATE_VC(T:%d,VIRT_COLS([%d,..,%d](%ld items)))", N(t1.n), N(a1.n), N(t1.n),
    ...
    case LEFT_JOIN_ON:
    {
      ...
      int i = 0;
      for (; i < (int)tables1.size() - 1; i++) sprintf(buf + strlen(buf), "T:%d,", N(tables1[i].n));
      sprintf(buf + strlen(buf), "T:%d},{", N(tables1[i].n));
      i = 0;
      for (; i < (int)tables2.size() - 1; i++) sprintf(buf + strlen(buf), "T:%d,", N(tables2[i].n));
    ...
    case APPLY_CONDS:
      ...
      break;
    case ADD_COLUMN:
      sprintf(buf, "A:%d = T:%d.ADD_COLUMN(%s,%s,\"%s\",\"%s\")", N(a1.n), N(t1.n), e1.ToString(b1, _countof(b1), t1.n),
              b_cop, (alias) ? alias : "null", n1 ? "DISTINCT" : "ALL");
      break;
    case ADD_ORDER:
...
void CompiledQuery::CreateConds(CondID &c_out, const TabID &t1, CQTerm e1, Operator pr, CQTerm e2, CQTerm e3,
...
void CompiledQuery::LeftJoinOn(const TabID &temp_table, vector<TabID> &left_tables, vector<TabID> &right_tables,
...
void CompiledQuery::InnerJoinOn(const TabID &temp_table, vector<TabID> &left_tables, vector<TabID> &right_tables,
                                const CondID &cond_id)
{
  CompiledQuery::CQStep s;
...
void CompiledQuery::AddColumn(AttrID &a_out, const TabID &t1, CQTerm e1, ColOperation op, char const alias[],
                              bool distinct)
{
  assert(t1.n < 0 && NoTabs() > 0);
  CompiledQuery::CQStep s;
...
{
  uint i;
  for (i = 0; i < steps.size(); i++) steps[i].Print(query);
}
...
bool CompiledQuery::CountColumnOnly(const TabID &table)
{
  ...
  for (int i = 0; i < NoSteps(); i++)
  {
    ...
    if (step.type == CompiledQuery::ADD_COLUMN && step.t1 == table &&
        (step.cop != COUNT || (step.cop == COUNT && !step.e1.IsNull())))
    {
      count_only = false;
      break;
    }
