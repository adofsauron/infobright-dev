...
class Splice : public TrackableObject
{
 ...
 public:
  ...
  {
    for (int i(0); i < SPLICE_SIZE; ++i) new (_data + i) T();
  }
  virtual ~Splice(void)
  {
  ...
  TRACKABLEOBJECT_TYPE TrackableType(void) const { return (TO_SPLICE); }
...
struct system_heap_allocator
{
  ...
  splice_ptr_t operator()(int const &) const { return (splice_ptr_t(new splice_t())); }
  template <typename splice_vector_t, typename dummy>
  void operator()(splice_vector_t &vec_, int old_, int new_, dummy = dummy()) const
  {
    for (int i(old_); i < new_; ++i) vec_[i] = (*this)(i);
  }
};

template <typename T, typename allocator_t = system_heap_allocator<T, DEFAULT_SPLICE_SIZE>,
          int const SPLICE_SIZE_ = DEFAULT_SPLICE_SIZE>
class SplicedVector
{
 ...
 public:
  ...
  int64 capacity(void) const { return (_data.size() * SPLICE_SIZE_); }
