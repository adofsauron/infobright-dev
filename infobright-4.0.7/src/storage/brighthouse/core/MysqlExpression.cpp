...
void MysqlExpression::RemoveUnusedVarID(Item *root)
{
  ...
  switch (static_cast<int>(root->type()))
  {
    ...
    case Item::FUNC_ITEM:
    {
      ...
      {
        Item **args = ifunc->arguments();
        for (uint i = 0; i < ifunc->argument_count(); i++) RemoveUnusedVarID(args[i]);
      }
      break;
    }
    case Item::COND_ITEM:
  ...
  switch (static_cast<int>(root->type()))
  {
    ...
    case Item::FUNC_ITEM:
    {
      ...
      if (strcmp(ifunc->func_name(), "xor") != 0)
      {
        ...
        //			}
        Item **args = ifunc->arguments();
        for (uint i = 0; i < ifunc->argument_count(); i++) args[i] = TransformTree(args[i], dir);
        return root;
      }
    }
...
DataType MysqlExpression::EvalType(TypOfVars *tv)
{
  ...
  if (tv)
  {
    ...
    while (bhfield_set != bhfields_cache.end())
    {
      ...
      if (it != tv->end())
      {
        for (std::set<Item_bhfield *>::iterator bhfield = bhfield_set->second.begin();
             bhfield != bhfield_set->second.end(); bhfield++)
        {
          fieldtype = it->second;
  ...
  {
    case INT_RESULT:
      type = DataType(RC_BIGINT);
      break;
    case REAL_RESULT:
      type = DataType(RC_FLOAT);
      break;
    case DECIMAL_RESULT:
      type = DataType(RC_NUM, decimal_precision, decimal_scale);
      break;
    case STRING_RESULT:  // GA: in case of time item->max_length can contain invalid value
      if ((item->type() != Item_bhfield::get_bhitem_type() && item->field_type() == MYSQL_TYPE_TIME) ||
          (item->type() == Item_bhfield::get_bhitem_type() &&
           static_cast<Item_bhfield *>(item)->IsAggregation() == false && item->field_type() == MYSQL_TYPE_TIME))
        type = DataType(RC_TIME, 17, 0, item->collation);
               ...
               (item->type() == Item_bhfield::get_bhitem_type() &&
                static_cast<Item_bhfield *>(item)->IsAggregation() == false &&
        ...
        type = DataType(RC_DATETIME, 17, 0, item->collation);
               ...
               (item->type() == Item_bhfield::get_bhitem_type() &&
                static_cast<Item_bhfield *>(item)->IsAggregation() == false && item->field_type() == MYSQL_TYPE_DATE))
...
bool SameBHFieldSet(MysqlExpression::bhfields_cache_t::value_type const &l,
                    MysqlExpression::bhfields_cache_t::value_type const &r)
{
  return l.second.size() == r.second.size() && equal(l.second.begin(), l.second.end(), r.second.begin(), SameBHField);
}
    ...
    switch (static_cast<int>(t))
    {
      ...
      case (Item::FUNC_ITEM):
      {
        ...
        else
        {
          ...
          if (l->functype() == Item_func::COND_AND_FUNC || l->functype() == Item_func::COND_OR_FUNC ||
              l->functype() == Item_func::COND_XOR_FUNC)
          {
...
bool MysqlExpression::operator==(MysqlExpression const &other) const
{
  ...
  return ((mysql_type == other.mysql_type) && (decimal_precision == other.decimal_precision) &&
          ...
          (*item == *(other.item)) && (bhfields_cache.size() == other.bhfields_cache.size()) && vars == other.vars &&
