...
void RCAttr::EvaluatePack_IsNull(MIUpdatingIterator &mit, int dim)
{
  ...
  {  // pack is trivial - uniform or null only
...
void RCAttr::EvaluatePack_Like(MIUpdatingIterator &mit, int dim, Descriptor &d)
{
  ...
  for (uint i = 0; i < pattern.len; i++)
  {
    if (pattern[i] != '%')
      min_len++;
    if (pattern[i] == d.like_esc)
    {  // disable optimization, escape character may do a lot of mess
      min_len = 0;
      break;
    }
  }
...
void RCAttr::EvaluatePack_InString(MIUpdatingIterator &mit, int dim, Descriptor &d)
{
  ...
  do
  {
    ...
    else
    {
      BHTribool res;
      if (encoded_set)
      {  // fast path for numerics vs. encoded constant set
  ...
  do
  {
    ...
    else
    {
      RCBString v(
          p->GetVal(inpack),
          p->GetSize(
              inpack));  // change to materialized in case of problems, but the pack should be locked and unchanged here
...
void RCAttr::EvaluatePack_BetweenInt(MIUpdatingIterator &mit, int dim, Descriptor &d)
{
  ...
  else
  {
    ...
    if (((pv1 > 0 || pv2 < 0) && d.op == O_BETWEEN) || (pv1 <= 0 && pv2 >= 0 && d.op == O_NOT_BETWEEN))
    {
      mit.ResetCurrentPack();
      mit.NextPackrow();
    }
...
void RCAttr::EvaluatePack_AttrAttr(MIUpdatingIterator &mit, int dim, Descriptor &d)
{
  ...
  do
  {
    ...
    if (mit[dim] == NULL_VALUE_64 || (p1 && p1->IsNull(obj_in_pack)) ||
...
bool RCAttr::IsDistinct(Filter *f)
{
  ...
  if (ct.IsLookup() && RequiresUTFConversions(GetCollation()))
    return false;
  if (PhysicalColumn::IsDistinct() == RS_ALL)
  {  // = is_unique_updated && is_unique
    if (f == NULL)
  ...
  if (d.op == O_BETWEEN && d.val1.vc->IsConst() && d.val2.vc->IsConst() && PackType() == PackN)
  {
    ...
    if (!ATI::IsRealType(TypeName()))
    {
      ...
      for (int b = 0; b < NoPack(); b++)
      {
        ...
        span2 += 1;
        res +=
            (dpns[b].no_objs - dpns[b].no_nulls) * double(span2) / span1;  // supposing uniform distribution of values
      }
    }
    else
    {                       // double
...
bool RCAttr::TryToMerge(Descriptor &d1, Descriptor &d2)
{
  ...
  if ((d1.op != O_BETWEEN && d1.op != O_NOT_BETWEEN) || (d2.op != O_BETWEEN && d2.op != O_NOT_BETWEEN))
    return false;
  if (PackType() == PackN && d1.val1.vc && d1.val2.vc && d2.val1.vc && d2.val2.vc && d1.val1.vc->IsConst() &&
      d1.val2.vc->IsConst() && d2.val1.vc->IsConst() && d2.val2.vc->IsConst())
  {
    static MIIterator const mit(NULL);
  ...
  else if (PackType() == PackS && d1.val1.vc && d1.val2.vc && d2.val1.vc && d2.val2.vc && d1.val1.vc->IsConst() &&
           d1.val2.vc->IsConst() && d2.val1.vc->IsConst() && d2.val2.vc->IsConst() && d1.sharp == d2.sharp &&
           d1.GetCollation().collation == d2.GetCollation().collation)
  {
    ...
    if (d1.op == O_BETWEEN && d2.op == O_BETWEEN)
    {
      ...
      {
        if (d1min.IsNull() || d2min > d1min)
        {  // IsNull() means infinity here
          swap(d1.val1, d2.val1);
          swap(d1min, d2min);
        }
    ...
    if (d1.op == O_NOT_BETWEEN && d2.op == O_NOT_BETWEEN)
    {
      ...
      if (RequiresUTFConversions(my_coll))
      {
        if (CollationStrCmp(my_coll, d1min, d2max, O_LESS) && CollationStrCmp(my_coll, d2min, d1max, O_LESS))
        {
