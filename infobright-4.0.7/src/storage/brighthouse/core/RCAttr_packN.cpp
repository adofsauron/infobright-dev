...
AttrPackN::AttrPackN(const AttrPackN &apn) : AttrPack(apn)
{
...
std::auto_ptr<AttrPack> AttrPackN::Clone() const { return std::auto_ptr<AttrPack>(new AttrPackN(*this)); }
...
//: AttrPack(attr, pack_no)
{
  ...
  try
  {
    ...
    nulls = 0;
    data_full = 0;
    // read header of the pack.		WARNING: if the first 4 bytes are NOT going to indicate file size, change
    // CheckPackFileSize in RCAttr !
    uchar head[17];
    uint total_size;
...
void AttrPackN::Expand(
    uint new_no_obj, _uint64 new_max_val,
    _int64 offset)  // expand data by adding empty values (if necessary) or changing bit rate (if necessary)
{
  BHASSERT(!is_only_compressed, "The pack is compressed!");
  BHASSERT(new_no_obj >= no_obj,
           "Counter of new objects must be greater or equal to the counter total number of objects!");
...
void AttrPackN::RemoveNullsAndCompress(NumCompressor<etype> &nc, char *tmp_comp_buffer, uint &tmp_cb_len, _uint64 &maxv)
{
  ...
  if (res != CPRS_SUCCESS)
  {
    ...
    msg_buf << "Compression of numerical values failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1)
...
template <typename etype>
void AttrPackN::DecompressAndInsertNulls(NumCompressor<etype> &nc, uint *&cur_buf)
{
  CprsErr res = nc.Decompress(data_full, (char *)((cur_buf + 3)), *cur_buf, no_obj - no_nulls,
                              (etype) * (_uint64 *)((cur_buf + 1)));
  if (res != CPRS_SUCCESS)
  {
    std::stringstream msg_buf;
    msg_buf << "Decompression of numerical values failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1)
            << ", pack " << (GetCoordinate()[bh::COORD::DP] + 1) << " (error " << res << ").";
...
CompressionStatistics AttrPackN::Compress(
    DomainInjectionManager &dim)  // Create new optimal compressed buf. basing on full data.
{
  ...
  if (!compressed_buf)
    rclog << lock << "Error: out of memory (" << buffer_size << " bytes failed). (29)" << unlock;
