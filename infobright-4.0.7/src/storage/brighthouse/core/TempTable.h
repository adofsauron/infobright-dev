...
class TempTable : public JustATable
{
 public:
  ...
  class AttrBuffer : public CachedBuffer<T>
  {
   public:
    ...
    void GetString(RCBString &s, _int64 idx)
    {
      if (constant_value)
        CachedBuffer<T>::Get(s, 0);
      else
        CachedBuffer<T>::Get(s, idx);
    }
  ...
  class Attr : public PhysicalColumn
  {
   public:
    ...
    Attr(CQTerm t, ColOperation m, bool distinct = false, char *alias = NULL, int dim = -1, AttributeType type = RC_INT,
         uint scale = 0, uint precision = 10, NullMode n = NO_NULLS, DTCollation collation = DTCollation(),
    ...
    _int64 RoughMin(::Filter *f = NULL, RSValue *rf = NULL)
    {
      return MINUS_INF_64;
    }  // for numerical: best rough approximation of min for a given filter (or global min if filter is NULL)
    _int64 RoughMax(::Filter *f = NULL, RSValue *rf = NULL)
    {
      return PLUS_INF_64;
    }  // for numerical: best rough approximation of max for a given filter (or global max if filter is NULL)
 ...
 protected:
  ...
  TempTablePtr CreateMaterializedCopy(
      bool translate_order, bool in_subq);  // move all buffers to a newly created TempTable, make this VC-pointing to
                                            // it; the new table must be deleted by DeleteMaterializedCopy()
  void DeleteMaterializedCopy(
      TempTablePtr &t_new);  // delete the external table and remove VC pointers, make this fully materialized
  int mem_scale;
 ...
 public:
  ...
  bool OrderByAndMaterialize(
      std::vector<SortDescriptor> &ord, _int64 limit, _int64 offset,
      ResultSender *sender = NULL);  // Sort data contained in ParameterizedFilter by using some attributes (usually
                                     // specified by AddOrder, but in general - arbitrary)
  void FillMaterializedBuffers(_int64 local_limit, _int64 local_offset, ResultSender *sender,
                               bool pagewise = false);  // just materialize as SELECT *
  ...
  void RoughAggregateSum(VirtualColumn *vc, _int64 &min_val, _int64 &max_val, std::vector<Attr *> &group_by_attrs,
                         bool nulls_only, bool distinct_present);
  ...
  void SetNoMaterialized(_int64 n)
  {
    no_obj = n;
    no_materialized = n;
  }
  ...
  AttributeType AttrType(int a)
  {
    assert(a >= 0 && (uint)a < NoAttrs());
    return attrs[a]->TypeName();
  }
  ...
  PhysicalColumn *GetColumn(int a)
  {
    assert(a >= 0 && (uint)a < NoAttrs());
    return attrs[a];
  }
  NullMode GetAttrNullMode(int a)
  {
    assert(a >= 0 && (uint)a < NoAttrs());
    return attrs[a]->Type().GetNullsMode();
  }
  Attr *GetAttrP(uint a)
  {
    assert(a < (uint)NoAttrs());
    return attrs[a];
  }
  Attr *GetDisplayableAttrP(uint i)
  {
    assert(displayable_attr);
  ...
  VirtualColumn *GetVirtualColumn(uint col_num)
  {
 ...
 public:
  class Record
  {
   public:
    ...
    _uint64 get_row_id() { return (_atRow); }

    uint size() const { return (_it.table->NoDisplaybleAttrs()); }
