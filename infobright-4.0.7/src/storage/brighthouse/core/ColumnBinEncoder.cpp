...
bool ColumnBinEncoder::PrepareEncoder(VirtualColumn *_vc, VirtualColumn *_vc2)
{
  ...
  if (vct.IsFixed() && (!_vc2 || (vct2.IsFixed() && vct.GetScale() == vct2.GetScale())))
  {  // int/dec of the same scale
  ...
  {
    my_encoder =
        ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderDouble(vc, decodable, nulls_possible, descending));
  }
  else if (vct.IsFixed() && !vct2.IsString())
  {  // Decimals for different scale (the same scale is done by EncoderInt)
    my_encoder =
        ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderDecimal(vc, decodable, nulls_possible, descending));
  }
  ...
  else if (!monotonic_encoding && vct.IsLookup() && _vc2 == NULL && !RequiresUTFConversions(vc->GetCollation()))
  {  // Lookup encoding: only non-UTF
    my_encoder =
        ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderLookup(vc, decodable, nulls_possible, descending));
    lookup_encoder = true;
  }
  else if (!monotonic_encoding && vct.IsLookup() && _vc2 != NULL && vct2.IsLookup())
  {  // Lookup in joining - may be UTF
    my_encoder =
        ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderLookup(vc, decodable, nulls_possible, descending));
    lookup_encoder = true;
  }
  else if (vct.IsString() || vct2.IsString())
  {
    ...
    if (!noncomparable &&
        RequiresUTFConversions(coll))  // noncomparable => non-sorted cols in sorter, don't UTF-encode.
      my_encoder =
          ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderText_UTF(vc, decodable, nulls_possible, descending));
    else
    {
      my_encoder =
          ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderTextStat(vc, decodable, nulls_possible, descending));
      if (!my_encoder->Valid())
      {
        if (!monotonic_encoding && !decodable && vc->MaxStringSize() > HASH_FUNCTION_BYTE_SIZE)
          my_encoder =
              ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderTextMD5(vc, decodable, nulls_possible, descending));
        else
          my_encoder =
              ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderText(vc, decodable, nulls_possible, descending));
      }
      else
        text_stat_encoder = true;
    }
  }
  else if (vct.IsDateTime())
  {  // Date/time types except special cases (above)
    my_encoder = ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderInt(vc, decodable, nulls_possible, descending));
  }
  else
  {
  ...
  {  // multiple column encoding?
    ...
    if (!encoding_possible)
    {
      ...
      if (lookup_encoder)
      {  // try to use text (UTF) encoder instead of lookup
        my_encoder =
            ColumnValueEncoder_ptr_t(new ColumnBinEncoder::EncoderText_UTF(vc, decodable, nulls_possible, descending));
...
bool ColumnBinEncoder::ImpossibleValues(
    _int64 pack_min, _int64 pack_max)  // return true if the current contents of the encoder is out of scope
{
  if (implicit)
    return false;
...
ColumnBinEncoder::EncoderInt::EncoderInt(VirtualColumn *vc, bool decodable, bool nulls_possible, bool _descending,
                                         bool calc_int_statistics)
    : ColumnValueEncoder(vc, decodable, nulls_possible, _descending)
{
  min_val = 0;
  if (calc_int_statistics)
  {  // false if the column is actually not an integer (e.g. dec, date, textstat) and stats are calculated elsewhere
    min_val = vc->RoughMin();
...
void ColumnBinEncoder::EncoderInt::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                          bool update_stats)
{
...
bool ColumnBinEncoder::EncoderInt::IsNull(uchar *buf, uchar *buf_sec)
{
  ...
  if (descending)
  {
    if (null_status == 1)
    {
      ...
      for (int i = 0; i < size; i++) *(--val_ptr) = *(buf++);  // change endianess
...
ColumnBinEncoder::EncoderDecimal::EncoderDecimal(VirtualColumn *vc, bool decodable, bool nulls_possible,
                                                 bool _descending)
...
void ColumnBinEncoder::EncoderDecimal::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
...
void ColumnBinEncoder::EncoderDate::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                           bool update_stats)
{
  if (null_status > 0 && vc->IsNull(mit))
    SetNull(buf, buf_sec);
...
void ColumnBinEncoder::EncoderYear::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                           bool update_stats)
{
  if (null_status > 0 && vc->IsNull(mit))
    SetNull(buf, buf_sec);
...
{
  multiplier_vc1 =
      0;  // 0 if the first vc is double, otherwise it is regarded as int (decimal) which must be multiplied
  multiplier_vc2 =
      0;  // 0 if the second vc is double, otherwise it is regarded as int (decimal) which must be multiplied
  if (vc->Type().IsFixed())
...
void ColumnBinEncoder::EncoderDouble::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                             bool update_stats)
{
  if (null_status == 2)
  {
  ...
  for (int i = 0; i < 8; i++) *(buf++) = *(--val_ptr);  // change endianess - to make numbers comparable by memcmp()
}
...
void ColumnBinEncoder::EncoderText::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                           bool update_stats)
{
  if (null_status > 0 && vc->IsNull(mit))
  {
...
ColumnBinEncoder::EncoderText_UTF::EncoderText_UTF(VirtualColumn *vc, bool decodable, bool nulls_possible,
                                                   bool _descending)
    : ColumnValueEncoder(vc, decodable, nulls_possible, _descending), min_max_set(false)
{
...
bool ColumnBinEncoder::EncoderText_UTF::SecondColumn(VirtualColumn *vc2)
{
  if (vc_type.IsString() && vc2->Type().IsString() && collation.collation != vc2->GetCollation().collation)
  {
    rccontrol.lock(vc2->ConnInfo().GetThreadID()) << "Nontrivial comparison: " << collation.collation->name << " with "
                                                  << vc2->GetCollation().collation->name << unlock;
    return false;
  }
...
void ColumnBinEncoder::EncoderText_UTF::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                               bool update_stats)
{
  if (null_status > 0 && vc->IsNull(mit))
  {
...
bool ColumnBinEncoder::EncoderText_UTF::Encode(uchar *buf, uchar *buf_sec, RCBString &s, bool sec_column,
                                               bool update_stats)
{
  if (null_status > 0 && s.IsNull())
  {
...
bool ColumnBinEncoder::EncoderLookup::SecondColumn(VirtualColumn *vc)
{
  ...
  {  // binary encoding: internal dictionary search may be used
    for (int i = 0; i < no_sec_values; i++)
    {
      RCBString val2 = sec_vc->DecodeValue_S(i);
...
void ColumnBinEncoder::EncoderLookup::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                             bool update_stats /*= false*/)
{
  if (null_status > 0 && vc->IsNull(mit))
    SetNull(buf, buf_sec);
...
ColumnBinEncoder::EncoderTextStat::EncoderTextStat(VirtualColumn *vc, bool decodable, bool nulls_possible,
                                                   bool _descending)
    : EncoderInt(vc, decodable, nulls_possible, _descending, false)
{
...
void ColumnBinEncoder::EncoderTextStat::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                               bool update_stats /*= false*/)
{
  if (null_status > 0 && vc->IsNull(mit))
    SetNull(buf, buf_sec);
...
bool ColumnBinEncoder::EncoderTextStat::Encode(uchar *buf, uchar *buf_sec, RCBString &s, bool sec_column,
                                               bool update_stats)
{
  if (null_status > 0 && s.IsNull())
    SetNull(buf, buf_sec);
...
ColumnBinEncoder::EncoderTextMD5::EncoderTextMD5(VirtualColumn *vc, bool decodable, bool nulls_possible,
                                                 bool _descending)
    : ColumnValueEncoder(vc, decodable, nulls_possible, _descending), min_max_set(false)
...
ColumnBinEncoder::EncoderTextMD5::EncoderTextMD5(EncoderTextMD5 &sec)
...
bool ColumnBinEncoder::EncoderTextMD5::SecondColumn(VirtualColumn *vc) { return true; }

void ColumnBinEncoder::EncoderTextMD5::Encode(uchar *buf, uchar *buf_sec, VirtualColumn *vc, MIIterator &mit,
                                              bool update_stats)
{
  if (vc->IsNull(mit))
  {
...
bool ColumnBinEncoder::EncoderTextMD5::Encode(uchar *buf, uchar *buf_sec, RCBString &s, bool sec_column,
                                              bool update_stats)
{
  if (s.IsNull())
  {
...
void ColumnBinEncoder::EncoderTextMD5::SetNull(uchar *buf, uchar *buf_sec) { memcpy(buf, null_buf, size); }

bool ColumnBinEncoder::EncoderTextMD5::IsNull(uchar *buf, uchar *buf_sec) { return (memcmp(buf, null_buf, size) == 0); }
...
void MultiindexPositionEncoder::SetPrimaryOffset(int _offset)  // offset of the primary storage in buffer
{
  ...
  if (dim_no.size() > 0)
  {
    ...
    for (int i = 1; i < dim_offset.size(); i++) dim_offset[i] = dim_offset[i - 1] + dim_size[i - 1];
  }
}
