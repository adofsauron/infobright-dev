...
void JoinerSort::ExecuteJoinConditions(Condition &cond)
{
  ...
  {                                               // Normalize: let vc1 = a smaller table
  ...
  int sharp =
      ((cond[0].op == O_MORE || cond[0].op == O_LESS) ? 0 : 1);  // memcmp(...) < 0 for sharp, <= 0 for not sharp
  ...
  // Note: we are always checking "traversed < matched" condition. If ">" is needed, we are encoding values as for
  // decreasing order.
  ...
  for (uint i = 1; i < cond.Size(); i++)
  {
    ...
    if (dims_sum.Includes(dims_sec))
    {  // only additional conditions on the same dimensions are legal
  ...
  if (!outer_dims.IsEmpty())
  {
    ...
    else if (dims2.Includes(outer_dims))
    {  // outer dims are among matched => watch traversed for unused
  ...
  std::auto_ptr<Sorter3> s1(Sorter3::CreateSorter(mit1.NoTuples(), key_bytes, sort_encoder.TraverseBytes(), -1,
  ...
  if (packrows_omitted > 0)
    rccontrol.lock(m_conn.GetThreadID()) << "Roughly omitted "
                                         << int(packrows_omitted / double(packrows_matched) * 10000.0) / 100.0
                                         << "% packrows." << unlock;
  ...
  while (cur_traversed)
  {
    if (m_conn.killed())
      throw KilledRCException();
    if (cur_matched == NULL)
    {  // the first pass or the end of matched sorter
      if (cache_full)
      {
        rccontrol.lock(m_conn.GetThreadID()) << "Traversed " << no_of_traversed << "/" << actual_s1_size
                                             << " tuples, produced " << result_size << " tuples so far." << unlock;
        cache_full = false;
      ...
      while (cur_matched && !(memcmp(cur_traversed, cur_matched, key_bytes) < sharp)) cur_matched = s2->GetNextValue();
    }
    // find all traversed which also meet the condition
    while (!cache_full && cur_matched)
    {
      if (cur_traversed && memcmp(cur_traversed, cur_matched, key_bytes) < sharp)
  ...
  for (int j = 0; j < other_cond.size(); j++) other_cond[j].UnlockSourcePacks();
...
void JoinerSort::AddTuples(MINewContents &new_mind, JoinerSortWrapper &sort_encoder, unsigned char *matched_row,
                           _int64 &result_size, DimensionVector &dims_used_in_cond)
{
...
_int64 JoinerSort::AddOuterTuples(MINewContents &new_mind, JoinerSortWrapper &sort_encoder,
...
void JoinerSortWrapper::SetDimensions(MultiIndex *mind, DimensionVector &dim_tr, DimensionVector &dim_match,
                                      DimensionVector &dim_other, bool count_only)
{
...
unsigned char *JoinerSortWrapper::EncodeForSorter1(VirtualColumn *v, MIIterator &mit,
                                                   _int64 outer_pos)  // for Traversed
{
...
{
  // Note: we are always checking "traversed < matched" condition. If ">" is needed, we are encoding values as for
  // decreasing order. Assuming v is "matched" (the second) virtual column
...
bool JoinerSortWrapper::AddToCache(
    unsigned char *v)  // return false if there is no more space in cache (i.e. the next operation would fail)
{
...
_int64 JoinerSortWrapper::GetOuterIndex(
    int cache_position, unsigned char *matched_buffer)  // take it from traversed or matched, depending on the context
{
  _int64 ind_value = 0;
  if (watch_traversed)
