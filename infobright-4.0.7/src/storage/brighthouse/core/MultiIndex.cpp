...
MultiIndex::MultiIndex() : m_conn(&ConnectionInfoOnTLS.Get())
{
  ...
  vector<bool> can_be_distinct;  // true if the dimension contain only one copy of original rows, false if we cannot
                                 // guarantee this
...
MultiIndex::MultiIndex(const MultiIndex &s) : m_conn(s.m_conn)
{
  ...
  if (no_dimensions > 0)
  {
    ...
    for (int i = 0; i < s.dim_groups.size(); i++) dim_groups.push_back(s.dim_groups[i]->Clone(false));
...
void MultiIndex::Copy(MultiIndex &s, bool shallow)  // copy the object s
{
  ...
  if (no_dimensions > 0)
  {
    ...
    used_in_output = s.used_in_output;
    can_be_distinct = s.can_be_distinct;
    for (int i = 0; i < s.dim_groups.size(); i++) dim_groups.push_back(s.dim_groups[i]->Clone(shallow));
...
void MultiIndex::Empty(int dim_to_make_empty)
{
  ...
  {
    for (int i = 0; i < dim_groups.size(); i++) dim_groups[i]->Empty();
  }
...
void MultiIndex::AddDimension()
{
  ...
  for (int i = 0; i < dim_groups.size(); i++) dim_groups[i]->AddDimension();
...
void MultiIndex::AddDimension_cross(_uint64 size)
{
  ...
  {  // size == 0   => prepare a dummy dimension with empty filter
    dim_size[new_dim] = 1;
...
_uint64 MultiIndex::DimSize(
    int dim)  // the size of one dimension: material_no_tuples for materialized, NoOnes for virtual
{
...
int MultiIndex::MaxNoPacks(
    int dim)  // maximal (upper approx.) number of different nonempty data packs for the given dimension
{
  int max_packs = 0;
...
vector<int> MultiIndex::ListInvolvedDimGroups(
