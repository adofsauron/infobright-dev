...
void AggregatorAvg64::PutAggregatedValue(unsigned char *buf, _int64 v, _int64 factor)
{
  ...
  *((double *)buf) += double(v) * factor;
  ...
  {
    IBPushWarning(&ConnectionInfoOnTLS.Get().Thd(), MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                  "Values rounded in average() in Brighthouse");
    warning_issued = true;
  }
  *((_int64 *)(buf + 8)) += factor;
#else
...
{
  if (*((int *)buf) == NULL_VALUE_32 || *((int *)buf) > v)
  {
    stats_updated = false;
    *((int *)buf) = (int)v;
  }
...
void AggregatorMin32::Merge(unsigned char *buf, unsigned char *src_buf)
{
  if (*((int *)src_buf) == NULL_VALUE_32)
    return;
  if (*((int *)buf) == NULL_VALUE_32 || *((int *)buf) > *((int *)src_buf))
  {
    stats_updated = false;
    *((int *)buf) = *((int *)src_buf);
  }
...
{
  if (*((_int64 *)buf) == NULL_VALUE_64 || *((_int64 *)buf) > v)
  {
    stats_updated = false;
    *((_int64 *)buf) = v;
  }
...
void AggregatorMin64::Merge(unsigned char *buf, unsigned char *src_buf)
{
  if (*((_int64 *)src_buf) == NULL_VALUE_64)
    return;
  if (*((_int64 *)buf) == NULL_VALUE_64 || *((_int64 *)buf) > *((_int64 *)src_buf))
  {
    stats_updated = false;
...
{
  if (*((_int64 *)buf) == NULL_VALUE_64 || *((double *)buf) > *((double *)(&v)))
  {
    stats_updated = false;
    *((double *)buf) = *((double *)(&v));
  }
...
void AggregatorMinD::Merge(unsigned char *buf, unsigned char *src_buf)
{
  if (*((_int64 *)src_buf) == NULL_VALUE_64)
    return;
  if (*((_int64 *)buf) == NULL_VALUE_64 || *((double *)buf) > *((double *)src_buf))
  {
    stats_updated = false;
    *((double *)buf) = *((double *)src_buf);
  }
