...
class JoinerMapped : public TwoDimensionalJoiner
{
   ...
   * Assumptions: only for the easiest cases, i.e. integers without encoding, one equality condition, "traversed" has
   * unique values.
   *
   * Algorithm:
 ...
 private:
  ...
  DimensionVector
      traversed_dims;            // the mask of dimension numbers of traversed dimensions to be put into the join result
...
class JoinerMapFunction  // a superclass for all implemented types of join mapping
{
 public:
  ...
  virtual bool ImpossibleValues(_int64 local_min,
                                _int64 local_max) = 0;  // true if the given interval is out of scope of the function
  virtual bool CertainValues(_int64 local_min, _int64 local_max) = 0;  // true if the given interval is fully covered by
                                                                       // the function values (all rows will be joined)
 ...
 public:
  OffsetMapFunction()
      : key_status(NULL),
        key_table_min(0),
        key_min(PLUS_INF_64),
        key_max(MINUS_INF_64),
        key_continuous_max(MINUS_INF_64),
        dist_vals_found(0)
  {
  ...
  bool CertainValues(_int64 local_min, _int64 local_max)
  {
    return local_min >= key_min && local_max <= key_continuous_max;
  }
  _int64 DistinctVals() { return dist_vals_found; }

  unsigned char *key_status;  // every key value has its own status here: 255 - not exists, other - row offset is stored
                              // in offset_table
