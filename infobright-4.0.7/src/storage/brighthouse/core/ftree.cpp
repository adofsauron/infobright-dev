...
int FTree::GetEncodedValue(const RCBString &str)
{
  ...
  if (local_last_code > -1 && str.len == len[local_last_code] &&
      memcmp(str.val, (mem + value_offset[local_last_code]), str.len) == 0)
...
int FTree::Add(const RCBString &str)
{
  ...
  changed = 1;
  if (total_dic_size < dic_size + 1)
  {  // Enlarge tables, if required
  ...
  if (total_buf_size < new_value_offset + int(str.len))
  {  // Enlarge tables, if required
...
int FTree::CountOfUniqueValues() { return dic_size; }
...
void FTree::Init(unsigned char *&buf)
{
  ...
  if (!mem)
  {
    rclog << lock << "Error: FTree::Init, out of memory (" << total_buf_size * sizeof(char) << " bytes failed). (13)"
          << unlock;
    throw OutOfMemoryRCException();
  }
  memset(mem, 0, total_buf_size);
...
void FTree::InitHash()
{
  ...
  for (int i = 0; i < dic_size; i++) HashFind(mem + value_offset[i], len[i], i);
}
...
int FTree::HashFind(char *v, int v_len, int position_if_not_found)
{
  ...
  {  // infinite loop
    ...
    if (hash_pos == -1)
    {
      ...
      {
        if (dic_size > hash_size * 0.9)
        {  // too many values, hash table should be rebuilt
          InitHash();
