...
class Query
{
 ...
 public:
  ...
  int Compile(CompiledQuery *compiled_query, SELECT_LEX *selects_list, SELECT_LEX *last_distinct, TabID *res_tab = NULL,
              bool ignore_limit = false, Item *left_expr_for_subselect = NULL, Operator *oper_for_subselect = NULL,
              bool ignore_minmax = false, bool for_subq_in_where = false);
  ...
  int BuildConditions(COND *conds, CondID &cond_id, CompiledQuery *cq, const TabID &tmp_table, CondType filter_type,
 ...
 private:
  ...
  typedef std::multimap<TabID, virtual_column_meta_t>
      tab_id2expression_t;  // all expression based virtual columns for a given table
  ...
  std::vector<JustATablePtr>
      ta;  // table aliases - sometimes point to TempTables (maybe to the same one), sometimes to RCTables
  RCTable **t;
  int no_tab;
  int capacity;
  int *last_join;
  ...
  int Item2CQTerm(Item *an_arg, CQTerm &term, const TabID &tmp_table, CondType filter_type, bool negative = false,
                  Item *left_expr_for_subselect = NULL, Operator *oper_for_subselect = NULL);
   ...
   */
  CondID ConditionNumberFromNaked(COND *conds, const TabID &tmp_table, CondType filter_type, CondID *and_me_filter,
  ...
  CondID ConditionNumber(COND *conds, const TabID &tmp_table, CondType filter_type, CondID *and_me_filter = 0,
                         bool is_or_subtree = false);
 ...
 private:
  /*! \brief Creates AddColumn step in compilation by creating, if does not exist, Virtual Column based on
   * PhysicalColumn \param item - what Item \param tmp_table - for which TempTable \param oper - defines type of added
   * column \param distinct - flag for AddColumn operation \param group_by - indicates if it is column for group by
   * query \return column number
   */
  int AddColumnForPhysColumn(Item *item, const TabID &tmp_table, const ColOperation oper, const bool distinct,
   ...
   */
  WrapStatus::wrap_status_t WrapMysqlExpression(Item *item, const TabID &tmp_table, MysqlExpression *&expr,
  ...
  static bool ClearSubselectTransformation(Operator &oper_for_subselect, Item *&field_for_subselect, COND *&conds,
                                           Item *&having, Item *&cond_removed, List<Item> *&list_to_reinsert,
  ...
  VirtualColumn *CreateColumnFromExpression(Query::expressions_t const &exprs, TempTable *temp_table,
  ...
  VirtualColumn *CreateIBExpFunction(Item_func *item, MultiIndex *mind, std::vector<VirtualColumn *> child_cols,
                                     TempTable *t = NULL);
  ...
  ColumnType GetArgumentType(Item_func *ifunc, const ColumnType &result_type,
                             uint arg_pos = std::numeric_limits<uint>::max());
