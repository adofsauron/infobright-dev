...
using namespace std;
/*
const _int64 DATA_PACK_STRUCT_SIZE = 8500;	// typical size of all non-data RCAttrPack structures (virt. functions,
NULL mask etc.)
...
//	n_a = new int [1];
        ...
        if(t)
        {
                _int64 pack_size = (t->GetInternalSize(n_a[0]) + 2) * _int64(65536)
                                                        + DATA_PACK_STRUCT_SIZE;		// size +2 because of
index table pack_limit = (MemoryLimit() / pack_size) / divide_by; if(pack_limit<4) pack_limit = 4;
        }
}
                        ...
                        if( pack_counter>=pack_limit )
                        {
                                ...
                                for( int i=0; i<no_pack; i++ )
                                        if( pstat[i] == PG_READY && requested_at[i] < oldest_request ) {
                                                ...
                                                oldest_request = requested_at[i];	// note: may be -1 if switched
from other mode
                                        }
...
void PackGuardian::UnlockOne(int pack)					// manual unlocking, may be used for some
strategies
{
        if(t==NULL) return;
...
VCPackGuardian::VCPackGuardian(VirtualColumn *vc) : current_strategy(LOCK_ONE), my_vc(*vc), threads(1)
{
  initialized = false;
  last_pack = NULL;
  pack_locked = NULL;
...
{
  for (int i = 0; i < no_dims; ++i) delete[] last_pack[i];
  delete[] last_pack;
  delete[] dim_size;
  if (pack_locked)
  {
    for (int i = 0; i < no_dims; i++) delete[] pack_locked[i];
    delete[] pack_locked;
  }
}
...
void VCPackGuardian::Initialize(const MIIterator &mit, int no_th)  // mit used only for dim sizes
{
  ...
  no_dims = -1;
  for (vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end();
       iter++)
    if (iter->dim > no_dims)
  ...
  {  // else constant
    if (current_strategy == LOCK_ONE)
    {
      ...
      for (int i = 0; i < no_dims; ++i) last_pack[i] = new int[threads];

      for (int i = 0; i < no_dims; i++)
        for (int j = 0; j < threads; ++j) last_pack[i][j] = NULL_VALUE_32;
    }
    else if (current_strategy == LOCK_ALL)
    {
      dim_size = new int[no_dims];
      for (int i = 0; i < no_dims; i++) dim_size[i] = bh::common::NoObj2NoPacks(mit.OrigSize(i));

      pack_locked = new char *[no_dims];
      for (int i = 0; i < no_dims; i++) pack_locked[i] = NULL;
      for (vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin();
           iter != my_vc.GetVarMap().end(); iter++)
      {
        if (pack_locked[iter->dim] == NULL)
        {  // dimensions which are not used are left NULL
...
PG_Status VCPackGuardian::LockPackrow(const MIIterator &mit)
{
  ...
  if (current_strategy == LOCK_ONE)
  {
    ...
    for (vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin();
         iter != my_vc.GetVarMap().end(); iter++)
      last_pack[iter->dim][0] =
          mit.GetCurPackrow(iter->dim);  // must be in a separate loop, otherwise for "a + b" will not lock b
  }
  else if (current_strategy == LOCK_ALL)
  {
    for (vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin();
...
void VCPackGuardian::UnlockAll()
{
  ...
  else if (current_strategy == LOCK_ALL)
  {
    ...
    for (vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin();
         iter != my_vc.GetVarMap().end(); iter++)
      memset(pack_locked[iter->dim], 0,
             dim_size[iter->dim]);  // must be in a separate loop, otherwise for "a + b" will not unlock b
  }
}
