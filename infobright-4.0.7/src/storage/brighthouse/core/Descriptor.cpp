...
int SortDescriptor::operator==(const SortDescriptor &sec) { return (dir == sec.dir) && (vc == sec.vc); }
...
Descriptor::Descriptor(
    TempTable *t,
    int no_dims)  // no_dims is a destination number of dimensions (multiindex may be smaller at this point)
    : op(O_OR_TREE),
      lop(O_AND),
      sharp(false),
      encoded(false),
...
Descriptor::Descriptor(CQTerm e1, Operator pr, CQTerm e2, CQTerm e3, TempTable *t, int no_dims, char like_escape)
    : lop(O_AND),
      sharp(false),
      encoded(false),
      done(false),
...
int Descriptor::operator==(const Descriptor &sec) const
{
  return (attr == sec.attr && op == sec.op && lop == sec.lop && val1 == sec.val1 && val2 == sec.val2 &&
          sharp == sec.sharp && encoded == sec.encoded && delayed == sec.delayed && like_esc == sec.like_esc &&
          table == sec.table && left_dims == sec.left_dims &&  // check: dims order in vectors can be different
    ...
    switch (op)
    {
      case O_EQ:
        if (sec.op == O_BETWEEN && val1.vc->GetValue(dummy_mit) >= sec.val1.vc->GetValue(dummy_mit) &&
          ...
          return true;
        if (sec.op == O_IN &&
            static_cast<MultiValColumn *>(sec.val1.vc)->Contains(dummy_mit, *val1.vc->GetValue(dummy_mit).Get()) ==
                true)
          return true;
...
{
  BHASSERT_WITH_NO_PERFORMANCE_IMPACT(op == O_EQ || op == O_NOT_EQ || op == O_LESS || op == O_MORE || op == O_LESS_EQ ||
                                      op == O_MORE_EQ);
  SwitchOperator(op);
  CQTerm p = attr;
  attr = val1;
...
{                                                    // or "phys_column IS NULL/NOT NULL"
  if (attr.vc == NULL || !attr.vc->IsSingleColumn())
    return false;
  return ((val1.vc && val1.vc->IsConst()) || (op == O_IS_NULL || op == O_NOT_NULL)) &&
         (!val2.vc || (val2.vc && val2.vc->IsConst()));
}

bool Descriptor::IsType_AttrMultiVal() const
{
  return op != O_BETWEEN && op != O_NOT_BETWEEN && attr.vc && attr.vc->IsSingleColumn() && val1.vc &&
...
*/
void Descriptor::AppendString(char *buffer, size_t buffer_size, const char *string, size_t string_length,
                              size_t offset) const
{
...
void Descriptor::LockSourcePacks(const MIIterator &mit, int th_no) { LockSourcePacks(mit); }
#endif
  ...
  else
  {
    if (IsType_Subquery() && op != O_OR_TREE)
    {
      ...
      else if (res == BHTRIBOOL_UNKNOWN)
      {
        ...
        // cout << "# of skipped subqueries: " << true_c << "/" << false_c << "/" << unkn_c << " -> " << (true_c +
        // false_c) << " / " << (true_c + false_c + unkn_c) << endl;
      }
    }
    else
    {
...
void Descriptor::UpdateVCStatistics()  // Apply all the information from constants etc. to involved VC
{
  ...
  {  // if not encoded, we may have an incompatible comparison here (e.g. double between int and int)
...
bool Descriptor::CheckCondition_UTF(const MIIterator &mit)
{
  ...
  if (op == O_EQ)
  {  // fast track for the most common operator
  ...
  else if (op == O_BETWEEN || op == O_NOT_BETWEEN)
  {
    ...
    if (encoded)
    {  // Rare case: for encoded conditions treat NULL as +/- inf.
    ...
    if (encoded)
    {
      ...
      {  // Rare case: for encoded conditions treat NULL as +/- inf.
        ...
        val1_res = val1.vc->IsNull(mit) ? true
                                        : (sharp ? BHTribool(rcvo1 > val1.vc->GetValue(mit, false))
                                                 : BHTribool(rcvo1 >= val1.vc->GetValue(mit, false)));
        val2_res = val2.vc->IsNull(mit) ? true
                                        : (sharp ? BHTribool(rcvo1 < val2.vc->GetValue(mit, false))
                                                 : BHTribool(rcvo1 <= val2.vc->GetValue(mit, false)));
      }
    }
    else
    {
...
BHTribool Descriptor::RoughCheckSubselectCondition(MIIterator &mit, SubSelectOptimizationType sot)
{
  ...
  BHASSERT_WITH_NO_PERFORMANCE_IMPACT(dynamic_cast<SubSelectColumn *>(attr.vc) ||
                                      dynamic_cast<SubSelectColumn *>(val1.vc));
  SubSelectColumn *sub;
  VirtualColumn *val;
    ...
    case O_LESS_EQ_ALL:
      ...
      if (mvc->NoValues(mit) == 0)
        result = true;  // op ALL (empty_set) is TRUE
               ...
               !CollationStrCmp(collation, s1, aggr.ToRCString(), op))
        result = false;
      break;
    case O_MORE_ANY:
    case O_MORE_EQ_ANY:
...
bool Descriptor::CheckSetCondition(const MIIterator &mit, Operator op)
{
  ...
  switch (op)
  {
    ...
    case O_MORE_EQ_ALL:
      ...
      break;
    default:
      BHASSERT_WITH_NO_PERFORMANCE_IMPACT(0 && "unexpected operator");
      break;
  }
  return result;
}
...
BHTribool Descriptor::RoughCheckSetSubSelectCondition(const MIIterator &mit, Operator op, SubSelectOptimizationType sot)
{
  ...
  switch (op)
  {
    ...
    case O_MORE_EQ_ALL:
      ...
      break;
    default:
      BHASSERT_WITH_NO_PERFORMANCE_IMPACT("unexpected operator");
      break;
  }
  return BHTRIBOOL_UNKNOWN;
}
...
void Descriptor::CoerceColumnType(VirtualColumn *&for_typecast)
{
  ...
  {
    if (rccontrol.isOn())
      rccontrol.lock(ConnectionInfoOnTLS.Get().GetThreadID()) << "Type conversion for VC:"
...
bool Descriptor::NullMayBeTrue()  // true, if the descriptor may give nontrivial answer if any of involved dimension is
                                  // null
{
...
bool Descriptor::IsBHItemsEmpty()
{
  if (IsType_OrTree())
    ...
    return (!attr.vc || attr.vc->GetBHItems().empty()) && (!val1.vc || val1.vc->GetBHItems().empty()) &&
...
bool IsSetOperator(Operator op) { return IsSetAnyOperator(op) || IsSetAllOperator(op) || op == O_IN || op == O_NOT_IN; }

bool IsSetAllOperator(Operator op)
{
  return op == O_EQ_ALL || op == O_NOT_EQ_ALL || op == O_LESS_ALL || op == O_MORE_ALL || op == O_LESS_EQ_ALL ||
         op == O_MORE_EQ_ALL;
}

bool IsSimpleEqualityOperator(Operator op)
{
  return op == O_EQ || op == O_NOT_EQ || op == O_LESS || op == O_MORE || op == O_LESS_EQ || op == O_MORE_EQ;
}

bool IsSetAnyOperator(Operator op)
{
  return op == O_EQ_ANY || op == O_NOT_EQ_ANY || op == O_LESS_ANY || op == O_MORE_ANY || op == O_LESS_EQ_ANY ||
         op == O_MORE_EQ_ANY;
}

bool ISTypeOfEqualOperator(Operator op) { return op == O_EQ || op == O_EQ_ALL || op == O_EQ_ANY; }

bool ISTypeOfNotEqualOperator(Operator op) { return op == O_NOT_EQ || op == O_NOT_EQ_ALL || op == O_NOT_EQ_ANY; }

bool ISTypeOfLessOperator(Operator op) { return op == O_LESS || op == O_LESS_ALL || op == O_LESS_ANY; }

bool ISTypeOfLessEqualOperator(Operator op) { return op == O_LESS_EQ || op == O_LESS_EQ_ALL || op == O_LESS_EQ_ANY; }

bool ISTypeOfMoreOperator(Operator op) { return op == O_MORE || op == O_MORE_ALL || op == O_MORE_ANY; }

bool ISTypeOfMoreEqualOperator(Operator op) { return op == O_MORE_EQ || op == O_MORE_EQ_ALL || op == O_MORE_EQ_ANY; }
...
void DescTree::AddDescriptor(LogicalOperator lop, CQTerm e1, Operator op, CQTerm e2, CQTerm e3, TempTable *t,
...
void DescTreeNode::EvaluatePack(MIUpdatingIterator &mit)
{
  ...
  {                 // i.e., not a leaf
    ...
    if (desc.lop == O_AND)
    {
      ...
      if (right->desc.rv != RS_ALL && mit.RewindToPack(pack_start) &&
    ...
    else
    {
      ...
      else
      {
        ...
        left->EvaluatePack(mit);
        if (mit.SwapPackFilter(pack_start, f.get()))
        {  // return true if the pack in the current iterator is not full
...
void DescTreeNode::IncreaseDescriptorCount(vector<pair<int, Descriptor>> &desc_counts)
{
  ...
  p = find_if(desc_counts.begin(), desc_counts.end(),
              boost::bind(&DescriptorEqual, make_pair(0, boost::ref(desc)), _1));
