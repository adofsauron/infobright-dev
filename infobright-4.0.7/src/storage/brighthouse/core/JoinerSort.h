...
class JoinerSort : public TwoDimensionalJoiner
{
 ...
 private:
  void AddTuples(MINewContents &new_mind, JoinerSortWrapper &sort_wrapper, unsigned char *matched_row,
                 _int64 &result_size, DimensionVector &dims_used_in_other_cond);
...
class JoinerSortWrapper : public TrackableObject
{
 public:
  ...
  bool AddToCache(
      unsigned char *);  // return false if there is no more space in cache (i.e. the next operation would fail)
  void ClearCache() { cur_cache_used = 0; }
  _int64 CacheUsed() { return cur_cache_used; }

  bool PackPossible(VirtualColumn *v,
                    MIIterator &mit);  // return false if a Matched data pack defined by (v, mit) is outside Traversed
                                       // statistics, given a comparison direction stored in less

  // Dimension encoding section
  bool DimEncoded(int d) { return (dim_size[d] > 0); }
  _int64 DimValue(int dim, int cache_position,
                  unsigned char *matched_buffer);  // a stored dimension value (NULL_VALUE_64 for null object)

  // Outer joins
  void WatchMatched() { watch_matched = true; }
  void WatchTraversed() { watch_traversed = true; }
  _int64 GetOuterIndex(int cache_position,
                       unsigned char *matched_buffer);  // take it from traversed or matched, depending on the context
