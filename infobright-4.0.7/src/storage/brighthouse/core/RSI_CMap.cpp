...
RSIndex_CMap::RSIndex_CMap(int no_pos) : no_positions(no_pos), total_size(0), cmap_buffers()  // create an empty index
{
  no_obj = 0;
  no_pack = 0;
  no_pack_declared = 0;
...
RSIndex_CMap::~RSIndex_CMap() { Deallocate(); }

void RSIndex_CMap::Deallocate()
{
  for (cmap_buffers_t::iterator it(cmap_buffers.begin()), end(cmap_buffers.end()); it != end; ++it) dealloc(*it);
  cmap_buffers.clear();
}
...
void RSIndex_CMap::Alloc(_int64 new_no_pack_declared)
{
  ...
  if (size > total_size)
  {
    ...
    if (bc > old_bc)
    {
      ...
      {
        cmap_buffers[old_bc - 1] =
            static_cast<uint *>(rc_realloc(cmap_buffers[old_bc - 1], MAX_CMAP_BLOCK_SIZE, BLOCK_TEMPORARY));
        memset(cmap_buffers[old_bc - 1] + (total_size % MAX_CMAP_BLOCK_SIZE) / sizeof(int), 255,
    ...
    {
      cmap_buffers[bc - 1] =
          static_cast<uint *>(rc_realloc(cmap_buffers[bc - 1], size % MAX_CMAP_BLOCK_SIZE, BLOCK_TEMPORARY));
...
void RSIndex_CMap::AppendKNs(int new_no_pack)
{
  ...
  int new_no_pack_declared =
      (new_no_pack / 32 + 1) * 32;  // now the minimal size of hist is below 4 KB (including header)
...
bool RSIndex_CMap::UpToDate(_int64 cur_no_obj, int pack)  // true, if this pack is up to date (i.e. either
...
void RSIndex_CMap::Load(IBFile *frs_index, int current_read_loc)
{
  ...
  // <table_t>									- (no_pack - 1) * no_positions * 256 / 8
  // bytes
  ...
  if (no_pack * no_positions > 0)
  {
    ...
    for (int i(0); i < (no_pack_declared - no_pack); ++i) memset(PackBuf(no_pack + i), 255, no_positions * 32);
  }
}
