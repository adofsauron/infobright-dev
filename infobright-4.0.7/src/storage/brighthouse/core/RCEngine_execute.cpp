...
int optimize_select(THD *thd, Item ***rref_pointer_array, TABLE_LIST *tables, uint wild_num, List<Item> &fields,
                    ...
                    int &optimize_after_bh, int &free_join);
...
*/
int RCEngine::HandleSelect(THD *thd, LEX *lex, select_result *&result, ulong setup_tables_done_option, int &res,
                           int &optimize_after_bh, int &bh_free_join, int with_insert)
{
#ifdef PURE_LIBRARY
  ...
  if (thd->fill_derived_tables() && lex->derived_tables)
  {
    ...
    for (SELECT_LEX *sl = lex->all_selects_list; sl; sl = sl->next_select_in_list())        // for all selects
      for (TABLE_LIST *cursor = sl->get_table_list(); cursor; cursor = cursor->next_local)  // for all tables
        ...
        {  // data source (view or FROM subselect)
          // optimize derived table
          SELECT_LEX *first_select = cursor->derived->first_select();
          if (first_select->next_select() && first_select->next_select()->linkage == UNION_TYPE)
          {  //?? only if union
            if (cursor->derived->describe || cursor->derived->item)
            {  //??called for explain
               // OR there is subselect(?)
              route = RETURN_QUERY_TO_MYSQL_ROUTE;
              goto ret_derived;
            }
            if (!cursor->derived->executed || cursor->derived->uncacheable)
            {  //??not already executed (not materialized?)
          ...
          {  //??not union
            ...
            res = optimize_select(
                thd, &first_select->ref_pointer_array,  //===mysql_select
                (TABLE_LIST *)first_select->table_list.first, first_select->with_wild, first_select->item_list,
                ...
                (ORDER *)NULL, ulong(first_select->options | thd->options | SELECT_NO_UNLOCK), cursor->derived_result,
                cursor->derived, first_select, optimize_derived_after_bh, free_join);
        ...
        if (!res)
        {
          ...
          try
          {
            ...
            if (route == RETURN_QUERY_TO_MYSQL_ROUTE)
              ...
              {
                rclog << lock
                      << "Error: Query syntax not implemented in Brighthouse, can export only to MySQL format (set "
                         "BH_DATAFORMAT to 'MYSQL')."
                      << unlock;
                my_message(ER_SYNTAX_ERROR,
                           "Query syntax not implemented in Brighthouse, can export only to MySQL format (set "
                           "BH_DATAFORMAT to 'MYSQL').",
                           MYF(0));
  ...
  else
  {
    ...
    int err;
    err = optimize_select(
        thd, &select_lex->ref_pointer_array, (TABLE_LIST *)select_lex->table_list.first, select_lex->with_wild,
        select_lex->item_list, select_lex->where, select_lex->order_list.elements + select_lex->group_list.elements,
        (ORDER *)select_lex->order_list.first, (ORDER *)select_lex->group_list.first, select_lex->having,
        (ORDER *)lex->proc_list.first, ulong(select_lex->options | thd->options | setup_tables_done_option), result,
        unit, select_lex, optimize_after_bh, bh_free_join);
      ...
      try
      {
        ...
        if (route == RETURN_QUERY_TO_MYSQL_ROUTE && !in_case_of_failure_can_go_to_mysql)
        {
          rclog << lock
                ...
                << unlock;
          my_message(ER_SYNTAX_ERROR,
                     "Query syntax not implemented in Brighthouse, can export only to MySQL format (set BH_DATAFORMAT "
                     "to 'MYSQL').",
                     MYF(0));
          throw ReturnMeToMySQLWithError();
        }
    ...
    if (bh_free_join)
    {  // there was a join created in an upper function
...
ret_derived:
  ...
  if (!res && route == RETURN_QUERY_TO_MYSQL_ROUTE)
    if (thd->fill_derived_tables() && mysql_handle_derived(thd->lex, &mysql_derived_filling, &derived_optimized))
      res = TRUE;
  if (res)
  {
    // if an error appears during query preparation/optimization
...
int RCEngine::Execute(THD *thd, LEX *lex, select_result *result_output, SELECT_LEX_UNIT *unit_for_union)
{
  ...
  if (is_dumpfile)
  {
    push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                 "Dumpfile not implemented in Brighthouse, executed by MySQL engine.");
    return RETURN_QUERY_TO_MYSQL_ROUTE;
  }
  ...
  try
  {
    ...
    {
      push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                   "Query syntax not implemented in Brighthouse, executed by MySQL engine.");
      return RETURN_QUERY_TO_MYSQL_ROUTE;
    }
  }
  ...
  FunctionExecutor lock_and_unlock_pack_info(boost::bind(&Query::LockPackInfoForUse, query),
                                             boost::bind(&Query::UnlockPackInfoFromUse, query));
  ...
  try
  {
        ...
        RCEngine::IsBHTable(((Query_tables_list *)lex)->query_tables->table))
    {
      ...
      rct = cur_connection->GetTransaction()->GetTableShared(table_path,
                                                             ((Query_tables_list *)lex)->query_tables->table->s);
    }
    ...
    if (unit_for_union != NULL)
    {
      ...
      if (res)
      {
        rccontrol.lock(cur_connection->GetThreadID()) << "Error: "
                                                      << "Unsupported UNION" << unlock;
    ...
    for (std::map<int, std::vector<int>>::const_iterator it = cqu.GetTableIds().begin(); it != cqu.GetTableIds().end();
         ++it)
    {
      cur_connection->GetTransaction()->TableLimitedToColumns(it->first, cqu.GetColumnsPerTable(it->first),
                                                              thd->spcont != NULL && thd->spcont->is_function());
    }
#endif
    ...
    sender.reset();
    rccontrol.lock(cur_connection->GetThreadID())
        << "Total data packs actually loaded (approx.): " << cur_connection->GetPackCount() << unlock;
    rccontrol.lock(cur_connection->GetThreadID())
        << "----------------------------------------------------------------------------" << unlock;
    result = 0;
  }
...
int handle_exceptions(THD *thd, ConnectionInfo *cur_connection, bool with_error)
{
  ...
  catch (NotImplementedRCException const &x)
  {
    ...
    push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                 (string("Brighthouse: ") + x.what() + " Query executed by MySQL engine.").c_str());
    if (with_error)
    {
  ...
  catch (DBObjectRCException const &x)
  {  // the subselect had more than one row in a comparison without ANY or ALL
    rccontrol.lock(cur_connection->GetThreadID()) << "Error: " << x.what() << unlock;
