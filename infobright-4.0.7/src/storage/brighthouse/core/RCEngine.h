...
class RCEngine
{
 ...
 public:
  RCEngine()
      : m_tab_ext(NULL),
        ...
        m_engine_slot(0),
        m_loader_main_heapsize(0),
        m_loader_compressed_heapsize(0)
  {
  }
  ~RCEngine();
  ...
  int HandleSelect(THD *thd, LEX *lex, select_result *&result_output, ulong setup_tables_done_option, int &res,
                   int &optimize_after_bh, int &bh_free_join, int with_insert = false);
  ...
  BHError PrivateLoad(THD *thd, const std::string &table_path, struct st_table_share *table_share,
 ...
 private:
  ...
  BHError PrivateLoadInternal(THD *thd, RCTable *tab, const std::string &table_path, struct st_table_share *table_share,
                              bool refresh = true);

 private:
  // static void SendResults(JustATable & results, THD *thd, select_result *res, List<Item> & fields, ConnectionInfo
  // *conn);
 ...
 private:
  static bool IsBHRoute(THD *thd, TABLE_LIST *table_list, SELECT_LEX *selects_list,
 ...
 public:
  ...
  static BHError GetIOParameters(IOParameters &io_params, THD &thd, sql_exchange &ex, TABLE *table = 0,
                                 bool for_exporter = false);
...
class ResultSender
{
 public:
  ...
  void SetLimits(_int64 *_offset, _int64 *_limit)
  {
    offset = _offset;
    limit = _limit;
  }
...
static std::string bh_var_name_strings[] = {
    "BH_TIMEOUT",       "BH_DATAFORMAT",    "BH_PIPEMODE",         "BH_NULL",           "BH_THROTTLE",
    "BH_IBEXPRESSIONS", "BH_PARALLEL_AGGR", "BH_REJECT_FILE_PATH", "BH_ABORT_ON_COUNT", "BH_ABORT_ON_THRESHOLD"};
