...
Filter::Filter(const Filter &filter)
    : no_blocks(0),
      ...
      blocks(0),
      no_of_bits_in_last_block(0),
      track_changes(filter.track_changes),
      shallow(false),
      block_last_one(NULL),
      bit_block_pool(0),
      delayed_stats(-1),
...
Filter *Filter::ShallowCopy(Filter &f)
{
  ...
  for (int i = 0; i < sc->no_blocks; i++) sc->blocks[i] = f.blocks[i];
...
std::auto_ptr<Filter> Filter::Clone() const { return std::auto_ptr<Filter>(new Filter(*this)); }
...
void Filter::SetDelayed(int b, int pos)
{
  ...
  if (block_status[b] == FB_EMPTY)
  {
    ...
    else if (pos > delayed_stats_set + 1)
    {  // then we can't delay
...
void Filter::SetBetween(int b1, int n1, int b2, int n2)
{
  ...
  else
  {
    ...
    for (int i = b1 + 1; i < b2; i++) SetBlock(i);
...
void Filter::Reset(int b, int n)
{
        ...
        if(block_status[b] == FB_FULL) {
                ...
                if(b == no_blocks - 1) {
                        blocks[b] = block_allocator->Alloc();
                        new(blocks[b]) Block(this, no_of_bits_in_last_block, true); // set as full, then reset a part of
it } else { blocks[b] = block_allocator->Alloc(); new(blocks[b]) Block(this, 65536, true);
                }
                if(blocks[b] == NULL)
                        throw OutOfMemoryRCException();
        }
...
void Filter::ResetBetween(int b1, int n1, int b2, int n2)
{
  ...
  else
  {
    ...
    for (int i = b1 + 1; i < b2; i++) ResetBlock(i);
...
void Filter::Reset(Filter &f2)
{
  ...
  for (int b = 0; b < mb; b++)
  {
    ...
    else if (f2.block_status[b] != FB_EMPTY)
    {  // else no change
      if (block_status[b] == FB_FULL)
      {
        blocks[b] = block_allocator->Alloc();
...
bool Filter::IsFullBetween(_int64 n1, _int64 n2)  // true if there are only 1 between n1 and n2, inclusively
{
  ...
  else
  {
    ...
    if (b1 != full_pack_start)
    {
      if (block_status[b1] == FB_EMPTY)
        return false;
      if (!IsFull(b1) && !blocks[b1]->IsFullBetween(nn1, 65535))  // note that b1 is never the last block
        return false;
    }
    if (b2 != full_pack_stop)
    {
...
void Filter::CopyBlock(Filter &f, int block)
{
  ...
  if (f.GetBlock(block))
  {
    if (bit_block_pool == f.bit_block_pool)
    {  // f is a shallow copy of this
...
bool Filter::IsEqual(Filter &sec)
{
  ...
  for (int b = 0; b < no_blocks; b++)
  {
    if (block_status[b] != sec.block_status[b])
    {
      ...
      if (block_status[b] == FB_FULL && sec.block_status[b] == FB_MIXED)
      {  // Note: may still be equal!
      ...
      if (sec.block_status[b] == FB_FULL && block_status[b] == FB_MIXED)
      {  // Note: may still be equal!
...
void Filter::AndNot(Filter &f2)  // reset all positions which are set in f2
{
  ...
  for (int b = 0; b < mb; b++)
  {
    if (f2.block_status[b] == FB_FULL)
      ResetBetween(b, 0, b, f2.block_last_one[b]);
    else if (f2.block_status[b] != FB_EMPTY && block_status[b] != FB_EMPTY)
    {  // else no change
      if (block_status[b] == FB_FULL)
      {
        int old_block_size = block_last_one[b];
...
void Filter::SwapPack(Filter &f2, int pack)
{
  ...
  {
    // save block
    assert(shallow || blocks[pack] && blocks[pack]->Owner() ==
                                          this);  // shallow copy can have the original filter as the block owner
...
int Filter::NoBlocks() const { return no_blocks; }
...
void Filter::Grow(_int64 grow_size, bool value)
{
  ...
  int bits_added_to_block =
      grow_size > 0x10000 - no_of_bits_in_last_block ? 0x10000 - no_of_bits_in_last_block : (int)grow_size;
  ...
  if (no_blocks > 0 && bits_added_to_block > 0)
  {
    ...
    if (value)
    {
      ...
      {  // FB_FULL, but not really full: 111..11000...00 with ones added to the end
        block_status[no_blocks - 1] = FB_MIXED;
        blocks[no_blocks - 1] = block_allocator->Alloc();
...
Filter::BlockAllocator::~BlockAllocator() { delete[] pool; }
