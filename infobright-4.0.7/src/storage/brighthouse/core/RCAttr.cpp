...
RCAttr::RCAttr(char *a_name, int a_num, int t_num, AttributeType a_type, int a_field_size, int a_dec_places,
               std::string const &a_path, uint param, uint s_id, DTCollation collation)
    : packs_omitted(0), dpns(0, cached_dpn_splice_allocator(this)), path(add_dir_sep(a_path)), last_pack_locked(false)
{
  ...
  if (a_field_size <= 0)
  {
    ...
    if (a_type == RC_REAL)
      ...
      fsize = 18;
    else if (a_type == RC_DATE)
      fsize = 10;
    else if (a_type == RC_TIME)
      fsize = 10;
    else if (a_type == RC_YEAR)
      fsize = 4;
    else if (a_type == RC_DATETIME || a_type == RC_TIMESTAMP)
      fsize = 19;
    a_field_size = fsize;
  }
...
RCAttr::RCAttr(int a_num, AttributeType a_type, int a_field_size, int a_dec_places, uint param, DTCollation collation,
               bool compress_lookups, string const &path_)
...
bool RCAttr::HasUnCommitedSession() { return (DoesFileExist(AttrFileName(FILE_SAVE_SESSION))); }
...
RCAttr::RCAttr(int a_num, int t_num, string const &a_path, int conn_mode, uint s_id,
               DTCollation collation) throw(DatabaseRCException)
    : packs_omitted(0), dpns(0, cached_dpn_splice_allocator(this)), path(add_dir_sep(a_path)), last_pack_locked(false)
{
  ...
  if (current_state == SESSION_WRITE)
  {
    if (HasUnCommitedSession())
    {
      ...
      {
        rclog << lock << "Error - previous session (id=" << GetSaveSessionId() << ") was not properly committed!"
              << unlock;
        new_save_session = 1;
      }
    }
...
void RCAttr::UpgradeFormat()
{
  ...
  {
    rclog << lock << "Internal upgrade error: unable to remove " << fn
          << " backup file (change *.bck to *.ctb before next startup)." << unlock;
    throw;
  }
}
...
void RCAttr::LoadPackInfo_physical(Transaction *trans)
{
  ...
  if (FileFormat() == 9 || GetDictOffset() != 0)
  {
    ...
    try
    {
      ...
      int const SIZE_OF_DPN = sizeof(static_cast<DPN *>(NULL)->pack_file) +
                              sizeof(static_cast<DPN *>(NULL)->pack_addr) +
                              sizeof(static_cast<DPN *>(NULL)->local_min) +
                              sizeof(static_cast<DPN *>(NULL)->local_max) + sizeof(static_cast<DPN *>(NULL)->sum_size) +
      ...
      if (GetDictOffset() != 0 && (fin_size - GetDictOffset()))
      {
        ...
        //[mk]
        {
          if (process_type == ProcessType::BHLOADER || process_type == ProcessType::DIM)
            dic = GlobalDataCache::GetGlobalDataCache().GetObject<FTree>(FTreeCoordinate(table_number, attr_number),
                                                                         bind(&RCAttr::LoadLookupDictFromFile, this));
          else
            dic = trans->GetObject<FTree>(FTreeCoordinate(table_number, attr_number),
                                          bind(&RCAttr::LoadLookupDictFromFile, this));
        }
        // [emk]
      }
...
void RCAttr::LoadDictionaries(const char *buf_ptr)
{
  ...
  else if (ATI::IsNumericType(a_type) || ATI::IsDateTimeType(a_type) || ATI::IsDateTimeNType(a_type))
  {
    SetMinInt64(*((_uint64 *)(buf)));
    SetMaxInt64(*((_uint64 *)(buf + 8)));
    buf += 16;
...
shared_ptr<FTree> RCAttr::LoadLookupDictFromFile()  // bool include_restore_dpn)
{
  ...
  if (GetDictOffset() != 0)
  {
    ...
    {
      delete[] buf_ptr;
      // rclog << lock << "Internal error: unable to load column dictionary from " << fn << ". " <<
      // GetErrorMessage(errno) << unlock;
      throw;
    }
  }
  return dic;
...
void RCAttr::RestoreDPN(char *buf, DPN &dpn)
{
  ...
  if (dpn.pack_file == PF_NULLS_ONLY || (PackType() == PackN && dpn.local_min == dpn.local_max && dpn.no_nulls == 0))
  {
...
void RCAttr::RestoreDPN(char *buf, uint pack_no) { RestoreDPN(buf, dpns[pack_no]); }
...
void RCAttr::GetValueBin(_int64 obj, int &size,
                         char *val_buf)  // original 0-level value (text, string, date, time etc.)
{
  if (obj == NULL_VALUE_64)
    return;
  AttributeType a_type = TypeName();
...
int RCAttr::EncodeValue_T(const RCBString &rcbs, int new_val,
                          BHReturnCode *bhrc)  // 1-level code value for a given 0-level (text) value
...
RCBString RCAttr::GetPrefix(int pack)
{
  ...
  int dif_pos = 0;
  for (; (dif_pos < sizeof(_uint64)) && min_s[dif_pos] && max_s[dif_pos] && (min_s[dif_pos] == max_s[dif_pos]);
       ++dif_pos)
    ;
...
void RCAttr::LockPackForUse(unsigned pack_no, Transaction &trans, boost::function0<void> notify_pack_load)
{
  ...
  AttrPackPtr p =
      trans.GetAttrPack(PackCoordinate(table_number, attr_number, pack_no + packs_omitted, 0, 0), *m_allocator);
...
void RCAttr::LockPackForUse(unsigned pack_no) { LockPackForUse(pack_no, ConnectionInfoOnTLS.Get()); }
...
bool RCAttr::OldFormat() { return (FileFormat() < 10); }
...
uint RCAttr::LoadHeader(const char *a_buf)
{
  ...
  if (((type == RC_VARCHAR || type == RC_STRING) && is_lookup) || ATI::IsNumericType(type) || ATI::IsDateTimeType(type))
    pack_type = PackN;
  else
    pack_type = PackS;
...
uint RCAttr::LoadSessionInfo(const char *cur_buf, uint actual_size, IBFile *fcmd_file)
{
  ...
  while (special_offset > 0)
  {
    ...
    if (b_type == 0)
    {                                             // type 0: the first special block, rollback and session info
      ...
      // if (open_session_type == 0 || open_session_type == 3)	// only in case of read (normal/backup) sessions,
      // otherwise read_session_id remains 0 	last_save_session_id = *((uint*)(cur_buf + 23));
      natural_size_saved = *((_uint64 *)(cur_buf + 23 + sizeof(uint)));
      compressed_size = *((_uint64 *)(cur_buf + 23 + sizeof(uint) + sizeof(_uint64)));
    }
    else
      rclog << lock << "Warning: special block (unknown type " << int(b_type) << ") ignored in attribute " << Name()
            << unlock;
...
bool RCAttr::Save(string const &cmd_file, FTree *dic, _int64 comprs_size)
{
  ...
  *((_int64 *)(buf + 17)) = no_nulls;
  ...
  // buf[25] stores attribute's flags: null_mode + 4*declared_unique + 8*is_primary  + 16*is_unique +
  // 32*is_unique_updated Now declared_unique, is_primary are obsolete. Can be removed in future file format
  buf[25] = char(Type().GetNullsMode()) + (IsUnique() ? 16 : 0) + (IsUniqueUpdated() ? 32 : 0) +
            (inserting_mode == 1 ? 0x40 : 0);
  ...
  *((_uint64 *)(buf + 23 + sizeof(uint) + sizeof(_uint64))) = compressed_size;
  ...
  compressed_size =
      comprs_size + fattr.Tell() * 2;  // *2, because column header (and lookup dict.) is stored in TA00... and TB00...
...
IBFile *RCAttr::GetPackFile(int pack_no)
{
...
bool PackAllocator::isLoadable(int pack)
{
  attr->LoadPackInfo();
  return attr->dpns[pack].pack_mode == PACK_MODE_UNLOADED && attr->dpns[pack].is_stored;
}

AttrPackPtr PackAllocator::lockedAlloc(const PackCoordinate &pack_coord)
{
  AttrPackPtr p;
  if (pt == PackS)
  {
...
IBFile *PackAllocator::getFile(int pack) { return attr->GetPackFile(pack); }
