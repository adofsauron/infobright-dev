...
void JoinerGeneral::ExecuteJoinConditions(Condition &cond)
{
  ...
  for (uint i = 0; i < cond.Size(); ++i) cond[i].InitPrefetching(mit);
#endif
  ...
  else
  {
    while (mit.IsValid() && !stop_execution)
    {
      ...
      for (int i = 0; (i < no_desc && loc_result); i++)
      {
        ...
        {  // delayed locking - maybe will not be locked at all?
  ...
  if (rows_passed > 0 && rows_omitted > 0)
    rccontrol.lock(m_conn.GetThreadID()) << "Roughly omitted " << int(rows_omitted / double(rows_passed) * 1000) / 10.0
                                         << "% rows." << unlock;
#ifndef __BH_COMMUNITY__
  for (uint i = 0; i < cond.Size(); ++i) cond[i].StopPrefetching();
#endif
