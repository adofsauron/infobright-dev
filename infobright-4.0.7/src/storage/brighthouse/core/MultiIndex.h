...
class MultiIndex
{
 public:
  ...
  void AddDimension_cross(_uint64 size);  // calculate a cross product of the previous value of index and the full table
                                          // (trivial filter) of 'size' objects
  void AddDimension_cross(Filter &new_f);  // calculate a cross product of the previous value of index and the new set
                                           // of objects (defined as Filter, to be copied inside)

  //////////// retrieve information

  int NoDimensions() const { return no_dimensions; }  // number of dimensions
  _int64 NoTuples() const
  ...
  Filter *GetFilter(int dim) const  // Get the pointer to a filter attached to a dimension. NOTE: will be NULL in case
                                    // of materialized MultiIndex!
  {
    return no_dimensions > 0 ? group_for_dim[dim]->GetFilter(dim) : NULL;
  }
  Filter *GetUpdatableFilter(int dim) const  // Get the pointer to a filter, if it may be changed. NOTE: will be NULL in
                                             // case of materialized MultiIndex!
  {
  ...
  _uint64 DimSize(
      int dim);  // the size of one dimension: NoOnes for virtual, number of materialized tuples for materialized
  ...
  void MIFilterAnd(
      MIIterator &mit,
      Filter &fd);  // limit the MultiIndex by excluding all tuples which are not present in fd, in order given by mit
  ...
  // recalculate the number of tuples, assuming mat_tuples is the new material_no_tuples and the dimensions from the
  // list are deleted
 ...
 private:
  ...
  std::vector<int> ListInvolvedDimGroups(
      DimensionVector &v);  // List all internal numbers of groups touched by the set of dimensions

  int no_dimensions;
  _int64 *dim_size;  // the size of a dimension
  _uint64
      no_tuples;  // actual number of tuples (also in case of virtual index); should be updated in any change of index
  bool no_tuples_too_big;             // this flag is set if a virtual number of tuples exceeds 2^64
  std::vector<bool> can_be_distinct;  // true if the dimension contain only one copy of original rows, false if we
...
class MINewContents  // a class to store new (future) contents of multiindex
{
 public:
  ...
  void CommitNewTableValues();  // move all values set by SetNew...(); roughsort if needed; if the index is larger than
                                // the current size, enlarge table automatically
  bool CommitPack(
      int pack);  // in case of single filter as a result: set a pack as not changed (return false if cannot do it)

  bool NoMoreTuplesPossible();  // for optimized cases: the join may be ended
  void Commit(
      _int64 joined_tuples);  // commit changes to multiindex - must be called at the end, or changes will be lost
