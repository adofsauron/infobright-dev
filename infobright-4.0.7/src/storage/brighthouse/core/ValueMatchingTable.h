...
// A structure to store large data rows consisting of <matching_data> and <store_data>, divided into "columns" defined
// as byte offsets. Functionality:
...
class ValueMatchingTable  // abstract class: interface for value matching (Group By etc.)
{
 public:
  ...
  virtual bool
  IsOnePass() = 0;  // true if the aggregator is capable of storing all groups up to max_no_groups declared in Init()
  virtual bool NoMoreSpace() = 0;  // hard stop: any new value will be rejected / we already found all groups
  virtual int
  MemoryBlocksLeft() = 0;  // soft stop: if 0, finish the current pack and start rejecting new values, default: 999
  virtual _int64 ByteSize() = 0;  // total size of the structure (for reporting / parallel threads number evaluation)
  virtual _int64 RowNumberScope() = 0;  // max_nr + 1, where max_nr is the largest row number returned by
                                        // FindCurrentRow, GetCurrentRow etc.

  // Iterator
  virtual void Rewind(
      bool release = false) = 0;       // rewind a row iterator, release=true - free memory occupied by traversed rows
  ...
  static ValueMatchingTable *CreateNew_ValueMatchingTable(_int64 mem_available, _int64 max_no_groups,
                                                          _int64 max_group_code, int _total_width, int _input_buf_width,
                                                          int _match_width);
...
class ValueMatching_LookupTable : public TrackableObject, public ValueMatchingTable
{
 public:
  ...
  virtual unsigned char *GetGroupingRow(_int64 row)
  {
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(row < max_no_rows);
    return t + row * total_width;
  }
  virtual unsigned char *GetAggregationRow(_int64 row)
  {
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(row < max_no_rows);
