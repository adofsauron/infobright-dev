...
int RCEngine::Init(const char *tab_ext, uint engine_slot)
{
  ...
  m_control_output = ((process_type != ProcessType::EMBEDDED) || !log_error_file_ptr || !log_error_file_ptr[0])
                         ? static_cast<ChannelOut *>(new StdOut())
                         : static_cast<ChannelOut *>(new FileOut(log_error_file_ptr));
  ...
  if (hugefiledir.empty())
    ...
    MemoryManagerInitializer::Instance(compressed_size, main_size, compressed_size / 10, main_size / 10, hugefiledir,
                                       hugefilesize);
  ...
  ParallScanThreadParams::no_DPs_per_thread_for_filter =
      ConfMan.GetValueInt("brighthouse/parallelscan/noDPsAtOnce", conferror);
  ParallScanThreadParams::min_no_DPs_per_thread =
      ConfMan.GetValueInt("brighthouse/parallelscan/minDPsPerThread", conferror);
...
BHErrorCode RCEngine::CreateTable(const char *table, TABLE *from)
{
  ...
  {
    rclog << lock << "Brighthouse: Can not create more tables in this version please ask the support team for an update"
          << unlock;
    return BHERROR_UNKNOWN;
  };
  string p = infobright_data_dir + "brighthouse.seq";
  ...
  shared_ptr<RCTable> tab =
      boost::shared_ptr<RCTable>(new RCTable(tab_path, charsets, tab_name, m_tab_sequencer++, true));
  delete[] tab_path;
  ...
  try
  {
    while (field != 0)
    {
      ...
      tab->AddAttribute(const_cast<char *>(field->field_name), ati, ati.Precision(), ati.Scale(), ati.Params(),
                        ati.GetCollation());
...
without {..} while (p<comm.length() && !isspace(comm.at(p))) p++; count = p-beg;
        }
        if (count==0)
                throw UnsupportedDataTypeRCException("Wrong definition of a column decomposition.");
...
ATI RCEngine::GetAttrTypeInfo(const Field &field) throw(UnsupportedDataTypeRCException)
{
  ...
  switch (field.type())
  {
    ...
    case MYSQL_TYPE_NEWDATE:
      return ATI(RCEngine::GetCorrespondingType(field), field.null_bit == 0, (ushort)field.field_length, 0, is_lookup,
                 for_insert);
    ...
    case MYSQL_TYPE_VARCHAR:
    {
      ...
      if (const Field_str *fstr = dynamic_cast<const Field_string *>(&field))
      {
        if (fstr->charset() != &my_charset_bin)
          return ATI(RC_STRING, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,
                     DTCollation(fstr->charset(), fstr->derivation()));
        return ATI(RC_BYTE, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,
                   DTCollation(fstr->charset(), fstr->derivation()));
      }
      else if (const Field_str *fvstr = dynamic_cast<const Field_varstring *>(&field))
      {
        if (fvstr->charset() != &my_charset_bin)
          return ATI(RC_VARCHAR, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,
                     DTCollation(fvstr->charset(), fvstr->derivation()));
        return ATI(RC_VARBYTE, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,
                   DTCollation(fvstr->charset(), fvstr->derivation()));
      }
      throw UnsupportedDataTypeRCException();
    }
...
void RCEngine::DropTable(const char *table)
{
  ...
  for (uint a = 0; a < no_attrs; a++) GlobalDataCache::GetGlobalDataCache().DropObject(FTreeCoordinate(id, a));
...
void RCEngine::GetTableIterator(const char *table_path, RCTableImpl::Iterator &iter_begin,
                                RCTableImpl::Iterator &iter_end, JustATable *&table, const std::vector<bool> &attrs,
                                THD *thd, ConnectionInfo *conn)
{
...
RCTablePtr RCEngine::GetTableShared(const string &table_path, struct st_table_share *table_share)
{
  ...
  if (!tab)
  {
    ...
    {
      tab->Rollback();
      rclog << lock << "WARNING: Rollback after uncommitted transaction on table " << db_name << '.' << tab_name
            << unlock;
    }
  }
  return tab;
...
int RCEngine::SetUpCacheFolder(const string &cachefolder_path)
{
  ...
  if (!exists(cachefolder_path))
  {
    ...
    {
      sql_print_error(
          "Brighthouse: Can not create folder %s. Make sure the parent folders exist and there is proper permission to "
          "create this folder on target device. Or create this folder with proper permission before starting "
          "Infobright server.",
          cachefolder_path.c_str());
...
void RCEngine::DatadirMigrator::Downgrade()
{
  ...
  std::string str = "You are attempting to downgrade to " + string(InfobrightServerVersion()) + " from " +
                    dv.GetServerVersion() + ". Due to data incompatibilities, the server is unable to start.";
  str += " Please contact your Infobright Support representative, or review the " + dv.GetServerVersion() +
         " documentation for specific downgrade instructions.";
