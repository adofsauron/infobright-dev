...
void ValueMatching_HashTable::Init(_int64 mem_available, _int64 max_no_groups, int _total_width, int _input_buf_width,
                                   int _match_width)
{
  ...
  {  // not enough memory - split between ht and t
  ...
  if (min_block_len > GBYTE &&
      mem_available > 256 * MBYTE)  // very large space needed (>1 GB) and substantial memory available
...
bool ValueMatching_HashTable::FindCurrentRow(unsigned char *input_buffer, _int64 &row, bool add_if_new)
{
  ...
  while (row_no < no_rows)
  {
    ...
    if (*next_pos == 0)
    {  // not found and no more conflicted values
      if (add_if_new)
        *next_pos = no_rows;
      row_no = no_rows;
    }
