...
RoughMultiIndex::RoughMultiIndex(vector<int> no_of_packs)
{
  ...
  for (int d = 0; d < no_dims; d++)
  {
    ...
    if (no_packs[d] > 0)
      ...
      rf[d] = NULL;
    for (int p = 0; p < no_packs[d]; p++) rf[d][p] = RS_UNKNOWN;
  }
}
...
RoughMultiIndex::~RoughMultiIndex()
{
  for (int d = 0; d < no_dims; d++)
  {
    if (rf[d])
      delete[] rf[d];
    for (int i = 0; i < local_desc[d].size(); i++) delete local_desc[d][i];
  }
  delete[] local_desc;
  delete[] rf;
  delete[] no_packs;
...
void RoughMultiIndex::MakeDimensionEmpty(int dim /*= -1*/)
{
  for (int d = 0; d < no_dims; d++)
    if ((dim == d || dim == -1) && rf[d])
    {
      for (int p = 0; p < no_packs[d]; p++) rf[d][p] = RS_NONE;
    }
}
...
bool RoughMultiIndex::UpdateGlobalRoughFilter(int dim, int desc_num)
{
  ...
  for (int p = 0; p < no_packs[dim]; p++)
  {
    if (rf[dim][p] == RS_UNKNOWN)
    {  // not known yet - get the first information available
      rf[dim][p] = loc_rs[p];
      if (rf[dim][p] != RS_NONE)
        any_nonempty = true;
    }
...
void RoughMultiIndex::UpdateLocalRoughFilters(
    int dim)  // make projection from global filters to all local ones for given dimensions
{
  for (int p = 0; p < no_packs[dim]; p++)
  {
    if (rf[dim][p] == RS_NONE)
    {
      for (int i = 0; i < local_desc[dim].size(); i++) (local_desc[dim])[i]->desc_rf[p] = RS_NONE;
    }
  }
}
