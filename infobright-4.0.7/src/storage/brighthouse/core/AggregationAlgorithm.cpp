...
void AggregationAlgorithm::Aggregate(bool just_distinct, _int64 &limit, _int64 &offset, ResultSender *sender)
{
  ...
  map<int, vector<PackOrderer::OrderingInfo>>
      oi;  // for each dimension, what column should be taken into account for pack ordering
  for (uint i = 0; i < t->NoAttrs(); i++)
  {  // first pass: find all grouping attributes
  ...
  for (uint i = 0; i < t->NoAttrs(); i++)
  {  // second pass: find all aggregated attributes
    TempTable::Attr &cur_a = *(t->GetAttrP(i));
    if (cur_a.mode == DELAYED)
    ...
    if ((!just_distinct && cur_a.mode != GROUP_BY) ||  // aggregation
        (just_distinct && cur_a.alias == NULL))
    {  // special case: hidden column for DISTINCT
      bool already_added = false;
      for (uint j = 0; j < i; j++)
      {
        if (*(t->GetAttrP(j)) == cur_a)
        ...
        else if (cur_a.distinct)
        {
          ...
          if (rccontrol.isOn())
            rccontrol.lock(m_conn->GetThreadID()) << "Adding dist. column, min = " << min_v << ",  max = " << max_v
                                                  << ",  dist = " << max_no_of_distinct << unlock;
        }
      }
      if (max_no_of_distinct == 0)
        max_no_of_distinct = 1;  // special case: aggregations on empty result (should not be 0, because it triggers
                                 // max. buffer settings)
  ...
  if (just_distinct && t->NoAttrs() == 1)
  {
    ...
    if (cur_a.mode == LISTING && cur_a.term.vc && cur_a.term.vc->GetDim() != -1)
      oi[cur_a.term.vc->GetDim()].push_back(PackOrderer::OrderingInfo(
          cur_a.term.vc, COUNT, true));  // simulate count(distinct) to obtain proper pack ordering
  }
  ...
  // Optimization for cases when limit is much less than a number of groups (but this optimization disables
  // multithreading)
  if (limit != -1 && upper_approx_of_groups / 10 > offset + limit && !(t->HasHavingConditions()))
  {  // HAVING should disable this optimization
  ...
  {
    for (uint i = 0; i < t->NoAttrs(); i++)
    {  // left as uninitialized (NULL or 0)
...
void AggregationAlgorithm::MultiDimensionalGroupByScan(GroupByWrapper &gbw, _int64 &limit, _int64 &offset,
                                                       map<int, vector<PackOrderer::OrderingInfo>> &oi,
                                                       ResultSender *sender, bool limit_less_than_no_groups)
{
  ...
  factor = mit.Factor();
  if (mit.NoTuples() == NULL_VALUE_64 || mit.NoTuples() > MAX_ROW_NUMBER)
  {  // 2^47, a limit for filter below
  ...
  try
  {
    do
    {
      if (rccontrol.isOn())
      {
        if (gbw.UpperApproxOfGroups() == 1 || first_pass)
          ...
          rccontrol.lock(m_conn->GetThreadID()) << "Aggregating: " << gbw.TuplesNoOnes() << " tuples left, "
                                                << displayed_no_groups << " gr. found so far" << unlock;
      }
      cur_tuple = 0;
      ...
      while (mit.IsValid())
      {  ///////////////////////// First stage - some distincts may be delayed
        if (m_conn->killed())
      ...
      if (first_pass)
      {
        ...
        _int64 upper_groups =
            gbw.NoGroups() +
            gbw.TuplesNoOnes();  // upper approximation: the current size + all other possible rows (if any)
        t->CalculatePageSize(upper_groups);
        if (upper_groups > gbw.UpperApproxOfGroups())
          upper_groups =
              gbw.UpperApproxOfGroups();  // another upper limitation: not more than theoretical number of combinations
  ...
  if (rccontrol.isOn())
    rccontrol.lock(m_conn->GetThreadID())
        << "Aggregated (" << displayed_no_groups << " gr). Omitted packrows: " << gbw.packrows_omitted << " + "
...
int AggregationAlgorithm::AggregatePackrow(GroupByWrapper &gbw, MIIterator *mit, _int64 cur_tuple)
{
  ...
  if (t->NoObj() + gbw.NoGroups() == gbw.UpperApproxOfGroups())
  {  // no more groups!
    gbw.SetNoMoreGroups();
    ...
    if (gbw.NoGroupingAttr() == gbw.NoAttr()  // just DISTINCT without grouping
        || stop_all)
    {                        // or aggregation already done on rough level
    ...
    if (gbw.TuplesGet(cur_tuple))
    {
      ...
      {                              // Any place left? If not, just omit the tuple.
        ...
        if (!existed)
        {
          ...
          {  // no more groups!
            gbw.SetNoMoreGroups();
            if (gbw.NoGroupingAttr() == gbw.NoAttr())
            {                        // just DISTINCT without grouping
              gbw.TuplesResetAll();  // no more rows needed, just produce output
              return 1;              // aggregation finished
            }
          }
...
void AggregationAlgorithm::AggregateFillOutput(GroupByWrapper &gbw, _int64 gt_pos, _int64 &omit_by_offset)
{
  ...
  if (!(t->HasHavingConditions()) && omit_by_offset > 0)
  {  // note that the rows not meeting conditions should not count in offset
    omit_by_offset--;
    return;
  }
  ...
  if (t->HasHavingConditions())
  {
    if (!(t->CheckHavingConditions(it)))
    {                                          // condition not met - forget about this row (will be overwritten soon)
...
bool AggregationAlgorithm::AggregateRough(GroupByWrapper &gbw, MIIterator &mit, bool &packrow_done, bool &part_omitted,
                                          bool &aggregations_not_changeable, bool &stop_all, _int64 &uniform_pos,
                                          _int64 rows_in_pack, _int64 local_factor, int just_one_aggr)
{
  ...
  {  // first test: whether the grouping values are not all uniform
  ...
  {                                                  // the whole packrow is defined by uniform values
    bool existed = gbw.FindCurrentRow(uniform_pos);  // the row is fully prepared
    if (uniform_pos != NULL_VALUE_64)
    {  // Successfully found? The whole packrow goes to one group.
      if (!existed)
      {
        gbw.AddGroup();  // successfully added
  ...
  if (gbw.IsFull())
  {
    ...
    for (int i = 0; i < gbw.NoGroupingAttr(); i++)
      if (gbw.ColumnNotOmitted(i))
      {
        ...
        if (!gbw.AttrMayBeUpdatedByPack(i, mit))
        {  // grouping values out of scope?
          all_done = true;
          break;
        }
      }
  ...
  aggregations_not_changeable =
      true;  // note that it is for current groups only, the flag will be ignored if any new group is found
  for (int i = gbw.NoGroupingAttr(); i < gbw.NoAttr(); i++)
    if (just_one_aggr == -1 || just_one_aggr == i)
    {
      if (gbw.ColumnNotOmitted(i))
  ...
  if (all_done && gbw.IsFull() && gbw.NoMoreGroups())
  {  // no aggregation may be changed by this packrow?
