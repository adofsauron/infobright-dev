...
void SetLimit(SELECT_LEX *sl, SELECT_LEX *gsl, _int64 &offset_value, _int64 &limit_value)
{
  ...
  if (sl->select_limit && (!gsl || sl->select_limit != gsl->select_limit))
  {
    ...
    if (limit_value == UINT_MAX)
    {                    /* this value seems to be sometimes automatically set by MYSQL to UINT_MAX*/
    ...
    case Item::SUM_FUNC_ITEM:  // min(k), max(k), count(), avg(k), sum
    {
      ...
      if (tmp_item->type() == Item::FIELD_ITEM)
        ifield = (Item_field *)tmp_item;
               ...
               static_cast<int>(
                   Item_bhfield::BHFIELD_ITEM)) /* *CAUTION* comparision of enumerators from different enums */
        ifield = dynamic_cast<Item_bhfield *>(tmp_item)->OriginalItem();
      else
        return RETURN_QUERY_TO_MYSQL_ROUTE;
      break;
...
bool Query::FieldUnmysterify(Item *item, TabID &tab, AttrID &col)
{
  ...
  if (!ifield->table_name)
  {
    ...
    // if(field_alias2num.find(TabIDColAlias((*table_alias2index)[ifield->table_name], ifield->name)) ==
    // field_alias2num.end()) 	return false; col.n =
    // field_alias2num[TabIDColAlias((*table_alias2index)[ifield->table_name], ifield->name)]; tab.n = NULL_VALUE_32;
    // return true;
  }
                    ...
                    bool for_subq_in_where /*false*/)
{
  ...
  while (join_ptr = li++) reversed.push_back(join_ptr);
                     ...
                     bool ignore_minmax, bool &aggregation_used)
{
  ...
  while (item)
  {
    ...
    //			}
             ...
             (UnRef(((Item_sum *)item)->args[0])->type() == Item_bhfield::get_bhitem_type() ||
              (UnRef(((Item_sum *)item)->args[0])->type() == Item_bhfield::FIELD_ITEM)) &&
    ...
    else if (IsAggregationItem(item))
    {
      ...
      if (item_sum->arg_count > 1 || HasAggregation(item_sum->args[0]))
        return RETURN_QUERY_TO_MYSQL_ROUTE;
      if (IsCountStar(item_sum))
      {  // count(*) doesn't need any virtual column
        AttrID at;
      ...
      else
      {
        ...
        if (ws == WrapStatus::FAILURE)
          return RETURN_QUERY_TO_MYSQL_ROUTE;
        AddColumnForMysqlExpression(expr, tmp_table, ignore_minmax ? item_sum->args[0]->name : item_sum->name, oper,
                                    distinct);
      }
    }
    else if (item->type() == Item::SUBSELECT_ITEM)
    {
      CQTerm term;
      AttrID at;
      if (Item2CQTerm(item, term, tmp_table, /*group_by_clause ? HAVING_FILTER :*/ WHERE_COND) ==
...
int Query::AddGroupByFields(ORDER *group_by, const TabID &tmp_table)
{
  ...
  for (; group_by; group_by = group_by->next)
  {
    ...
    if ((IsFieldItem(item) || (IsAggregationItem(item) && IsFieldItem(((Item_sum *)item)->args[0]))) &&
...
Query::WrapStatus::wrap_status_t Query::WrapMysqlExpression(Item *item, const TabID &tmp_table, MysqlExpression *&expr,
                                                            bool in_where, bool aggr_used)
{
#ifdef PURE_LIBRARY
  BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
    ...
    for (set<Item *>::iterator it = ifields.begin(); it != ifields.end(); it++)
    {
      if (IsAggregationItem(*it))
      {
        ...
        else
        {
          ...
          AttrID col;
          TabID tab;
          if (IsFieldItem(aggregation->args[0]) && FieldUnmysterify(aggregation, tab, col) &&
...
int Query::AddColumnForPhysColumn(Item *item, const TabID &tmp_table, const ColOperation oper, const bool distinct,
                                  bool group_by, const char *alias)
{
  ...
  if (item->type() == Item_bhfield::get_bhitem_type() &&
      IsAggregationItem(dynamic_cast<Item_bhfield *>(item)->OriginalItem()))
  ...
  else
  {
    ...
    else if (group_by && oper == GROUP_BY &&
             (attr = GetAddColumnId(AttrID(vc.second), tmp_table, LISTING, distinct)) != NULL_VALUE_32)
    {
      // modify existing column
      CQChangeAddColumnLIST2GROUP_BY(tmp_table, attr);
      return attr;
    }
    else if (group_by && oper == LISTING &&
             (attr = GetAddColumnId(AttrID(vc.second), tmp_table, GROUP_BY, distinct)) != NULL_VALUE_32)
    {
  ...
  if (!item->name && item->type() == Item::SUM_FUNC_ITEM)
    cq->AddColumn(at, tmp_table, CQTerm(vc.second), oper,
                  group_by ? NULL : ((Item_field *)(((Item_sum *)item)->args[0]))->name, distinct);
  else
...
int Query::AddColumnForMysqlExpression(MysqlExpression *mysql_expression, const TabID &tmp_table, const char *alias,
                                       const ColOperation oper, const bool distinct, bool group_by /*= false*/)
{
  AttrID at, vc;
...
int Query::Compile(CompiledQuery *compiled_query, SELECT_LEX *selects_list, SELECT_LEX *last_distinct, TabID *res_tab,
                   bool ignore_limit, Item *left_expr_for_subselect, Operator *oper_for_subselect, bool ignore_minmax,
                   bool for_subq_in_where)
{
#ifdef PURE_LIBRARY
  BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
  return 0;
  ...
  if (selects_list != selects_list->join->unit->global_parameters)
  {  // only in case of unions this is set
  ...
  for (SELECT_LEX *sl = selects_list; sl; sl = sl->next_select())
  {
    ...
    if (sl == selects_list)
    {
      ...
      if (global_order && !selects_list->next_select())
      {  // trivial union with one select and ext. order by
        tmp_table = TabID();
...
void Query::FindLastJoinsForTables(CompiledQuery &qu)
{
  ...
  for (int i = 0; i < no_tab; i++) last_join[i] = -1;
