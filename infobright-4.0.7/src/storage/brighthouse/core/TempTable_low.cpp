...
bool TempTable::OrderByAndMaterialize(
    vector<SortDescriptor> &ord, _int64 limit, _int64 offset,
...
{  // "limit=10; offset=20" means that the first 10 positions of sorted table will contain objects 21...30.
    ...
    if (attrs[i]->alias != NULL)
    {
      if (sender)
        attrs[i]->CreateBuffer(no_obj > RESULT_SENDER_CACHE_SIZE ? RESULT_SENDER_CACHE_SIZE : no_obj, &m_conn,
                               no_obj > RESULT_SENDER_CACHE_SIZE);
      else
  ...
  bool valid = true;
  do
  {  // outer loop - through streaming buffers (if sender != NULL)
    do
    {
      ...
      global_row++;
             ...
             !(sender && local_row >= RESULT_SENDER_CACHE_SIZE));  // a limit for streaming buffer
...
void TempTable::FillMaterializedBuffers(_int64 local_limit, _int64 local_offset, ResultSender *sender, bool pagewise)
{
  ...
  if (!filter.mind->ZeroTuples())
  {
    if (no_materialized == 0)
    {
      ...
      for (uint i = 0; i < NoAttrs(); i++) attrs[i]->CreateBuffer(no_obj, &m_conn, sender || pagewise);
    }
    bool has_intresting_columns = false;
    for (attrs_t::const_iterator attr = attrs.begin(), end = attrs.end(); attr != end; ++attr)
    { /* materialize dependent tables */
      if (((*attr)->mode == LISTING) && (*attr)->term.vc && (*attr)->alias)
      {  // constant value, the buffer is already filled in
    ...
    while (it.IsValid() && row < no_obj + local_offset)
    { /* go thru all rows */
      ...
      for (attrs_t::iterator attr = attrs.begin(), end = attrs.end(); attr != end; ++attr)
      { /* fill buffers in attrs */
        VirtualColumn *vc = (*attr)->term.vc;
        if ((((*attr)->mode == LISTING) && vc && ((*attr)->alias)) || !vc->IsConst())
        {  // constant value, the buffer is already filled in
          ...
          do
          { /* go thru all rows (fitting into page size) in given column in given packrow */
...
std::vector<ATI> TempTable::GetATIs(bool orig)
{
  ...
  for (uint i = 0; i < NoAttrs(); i++)
  {
    if (!IsDisplayAttr(i))
      continue;
    deas += ATI(attrs[i]->TypeName(), !attrs[i]->Type().GetNullsMode(),
                orig ? attrs[i]->orig_precision : attrs[i]->Type().GetPrecision(), attrs[i]->Type().GetScale(), false,
