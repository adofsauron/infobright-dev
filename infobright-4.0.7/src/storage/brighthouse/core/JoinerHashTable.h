...
class JoinerHashTable : public TrackableObject
{
 public:
  ...
  bool AddKeyColumn(VirtualColumn *vc,
                    VirtualColumn *vc_matching);  // Lookups should not be numerical in case of join, as the code values
                                                  // may be incompatible. Return false if not compatible.
  ...
  {
    bool encoded = encoder[col].PutValue64(input_buffer, v, true,
