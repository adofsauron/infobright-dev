...
Item_bhfield::Item_bhfield(Item_field *ifield, VarID varID)
    : Item_field(&ConnectionInfoOnTLS.Get().Thd(), ifield), ifield(ifield), buf(NULL), ivalue(NULL)
{
...
void Item_bhfield::SetType(DataType t)
{
  ...
  switch (bhtype.valtype)
  {
    ...
    case VT_DATETIME:
      ...
      {
        case RC_DATETIME:
          ivalue = new Item_bhdatetime();
          break;
        case RC_TIMESTAMP:
          ivalue = new Item_bhtimestamp();
          break;
        case RC_DATE:
          ivalue = new Item_bhdate();
          break;
        case RC_TIME:
          ivalue = new Item_bhtime();
          break;
        case RC_YEAR:
          ivalue = new Item_bhyear(bhtype.precision);
          break;
        default:
...
const ValueOrNull Item_bhfield::GetCurrentValue() { return *buf; }
...
{
  if (null_value = buf->null || ((bhtype.attrtype == RC_DATETIME || bhtype.attrtype == RC_DATE) &&
...
mysql_bool Item_bhfield::operator==(Item_bhfield const &o) const { return (varID == o.varID); }

Item_bhdecimal::Item_bhdecimal(DataType t) : Item_decimal(0, false)
{
  scale = t.fixscale;
...
mysql_bool Item_bhdatetime_base::get_time(TIME *ltime) { return get_date(ltime, 0); }

longlong Item_bhdatetime::val_int()
{
  return dt.Year() * 10000000000LL + dt.Month() * 100000000 + dt.Day() * 1000000 + dt.Hour() * 10000 +
...
longlong Item_bhdate::val_int() { return dt.Year() * 10000 + dt.Month() * 100 + dt.Day(); }
...
longlong Item_bhtime::val_int() { return dt.Hour() * 10000 + dt.Minute() * 100 + dt.Second(); }
...
longlong Item_bhyear::val_int() { return length == 4 ? dt.Year() : dt.Year() % 100; }
