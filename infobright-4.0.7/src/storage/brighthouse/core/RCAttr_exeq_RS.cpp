...
RSValue RCAttr::RoughCheck(int pack, Descriptor &d,
                           bool additional_nulls_possible)  // NOTE: similar code is in VirtualColumnBase::DoRoughCheck
{
#ifdef PURE_LIBRARY
  ...
  if (PackType() == PackS && RequiresUTFConversions(d.GetCollation()) &&
      d.GetCollation().collation != Type().GetCollation().collation)
  {
    if (d.encoded)
    {
      ...
      if (dpn.pack_file == PF_NULLS_ONLY || dpn.no_nulls - 1 == dpn.no_objs)
  ...
  if (d.IsType_AttrValOrAttrValVal())
  {
    ...
    else if ((d.op == O_LIKE || d.op == O_NOT_LIKE) && PackType() == PackS)
    {
      ...
      if (RequiresUTFConversions(d.GetCollation()))
      {
            ...
            && pattern_fixed_prefix + 1 == pat.len && pat[pattern_fixed_prefix] == '%')
        {
          if (d.GetCollation().collation->coll->strnncoll(d.GetCollation().collation, (const uchar *)pat.val,
                                                          pattern_fixed_prefix, (const uchar *)(&dpn.local_min),
      ...
      else
      {
        ...
        if ((pattern_fixed_prefix > 0) && RCBString(pat.val, pattern_fixed_prefix).LessEqThanMax(dpn.local_max) ==
      ...
      if (res == RS_SOME && min(pattern_prefix, pack_prefix) < pat.len && !RequiresUTFConversions(d.GetCollation()))
      {
        ...
        if (!(pattern_for_cmap.len == 1 && pattern_for_cmap[0] == '%'))
        {  // i.e. "%" => all is matching
    ...
    else if ((d.op == O_IN || d.op == O_NOT_IN) && PackType() == PackS)
    {
      ...
      if ((mvc->IsConst()) && (mvc->NoValues(mit) > 0 && mvc->NoValues(mit) < 64) &&
          !RequiresUTFConversions(d.GetCollation()))
      {
        RSIndex_CMap *rsi_cmap = LoadRSI_CMap();
        if (rsi_cmap && rsi_cmap->UpToDate(NoObj(), pack))
        {
          ...
          for (MultiValColumn::Iterator it = mvc->begin(mit), end = mvc->end(mit); (it != end) && (res == RS_NONE);
               ++it)
          {
            ...
            if (pack_prefix <= v1.len)
            {
              if (pack_prefix == 0 || memcmp(v1.val, (char *)(&dpn.local_min), pack_prefix) == 0)
              {
                ...
                if (v1.len == pack_prefix ||
                    rsi_cmap->IsValue(v, v, pack) !=
                        RS_NONE)  // suspected, if any value is possible (due to the prefix or CMAP)
                  res = RS_SOME;
              }
            }
          }
    ...
    else if ((d.op == O_IN || d.op == O_NOT_IN) && (PackType() == PackN))
    {
      if (vc1->IsConst())
      {
        ...
        {
          RCValueObject rcvo(ATI::IsDateTimeType(TypeName()) ? RCValueObject(RCDateTime(dpn.local_min, TypeName()))
                                                             : RCValueObject(RCNum(dpn.local_min, Type().GetScale())));
        ...
        if (res == RS_SOME && (mvc->NoValues(mit) > 0 && mvc->NoValues(mit) < 64))
        {
          ...
          {
            if (!Type().IsLookup())
            {  // otherwise it will be decoded to text
            ...
            if (!v_rounded)
            {
              if ((rsi_hist == NULL && v <= dpn.local_max && v >= dpn.local_min) ||
                  (rsi_hist && rsi_hist->IsValue(v, v, pack, dpn.local_min, dpn.local_max) != RS_NONE))
    ...
    {  // Note: text operations as PackN calculated as IN or below
      ...
      if (val_prefix > GetActualSize(pack))
      {  // value to be found is longer than texts in the pack
        res = RS_NONE;
      }
      else if ((vmax.val && vmax.GreaterEqThanMinUTF(dpn.local_min, Type().GetCollation()) == false) ||
               (vmin.val &&
                vmin.LessEqThanMaxUTF(dpn.local_max, Type().GetCollation()) == false))  // val_t==NULL means +/-infty
        res = RS_NONE;
      else if ((vmin.val == NULL || vmin.GreaterEqThanMinUTF(dpn.local_min, Type().GetCollation()) == false) &&
               (vmax.val == NULL ||
                vmax.LessEqThanMaxUTF(dpn.local_max, Type().GetCollation()) == false))  // val_t==NULL means +/-infty
        res = RS_ALL;
      else if (pack_prefix == GetActualSize(pack) && vmin == vmax)
      {  // exact case for short texts
        if (vmin.GreaterEqThanMinUTF(dpn.local_min, Type().GetCollation()) &&
            vmin.LessEqThanMaxUTF(dpn.local_min, Type().GetCollation()))
          res = RS_ALL;
        else
          res = RS_NONE;
      }
      /////////////////////////////////////////////
      if (res == RS_SOME && vmin.len >= pack_prefix && vmax.len >= pack_prefix &&
          !RequiresUTFConversions(d.GetCollation()))
      {
    ...
    else if (PackType() == PackN)
    {
      ...
      _int64 v1 = d.val1.vc->GetValueInt64(
          mit);  // 1-level values; note that these values were already transformed in EncodeCondition
    ...
    if (d.IsType_AttrAttr() && d.op != O_BETWEEN && d.op != O_NOT_BETWEEN && sec)
    {
      ...
      if (d.op != O_IN && d.op != O_NOT_IN)
      {
        if (PackType() == PackN && !ATI::IsRealType(TypeName()))
        {
          ...
          if (v1min >= v2max)
          {  // NOTE: only O_MORE, O_LESS and O_EQ are analyzed here, the rest of operators will be taken into account
          ...
          if (res == RS_SOME && (d.op == O_EQ || d.op == O_NOT_EQ))
          {  // the second case will be negated soon
            RSIndex_Hist *rsi_hist1 = LoadRSI_Hist();
            RSIndex_Hist *rsi_hist2 = sec->LoadRSI_Hist();

            // check intersection possibility on histograms
            if (rsi_hist1 && rsi_hist2 && rsi_hist1->UpToDate(NoObj(), pack) &&
...
RSValue RCAttr::RoughCheck(int pack1, int pack2, Descriptor &d)
{
  ...
  if(d.op==O_EQ || d.op==O_NOT_EQ)	// if rsi_pp exists, then this is the most accurate information about equality
  joins
  {
          bool res;
  ...
  {
          if(Type().GetScale()!=sec->Type().GetScale() || ATI::IsRealType(TypeName()) ||
  ATI::IsRealType(sec->TypeName()))
          ...
          if( d.op==O_EQ)
          {
                  ...
                  {
                          if( rsi_hist1->Intersection(			pack1,	     dpn.local_min,
  dpn.local_max, rsi_hist2,	pack2,	secDpn.local_min,	secDpn.local_max) == false)
                          {
                                  ReleaseRSI(rsi_hist1);
                                  ReleaseRSI(rsi_hist2);
                                  return RS_NONE;
...
RSValue RCAttr::RoughCheckBetween(
    int pack, _int64 v1,
    _int64 v2)  // check whether any value from the pack may meet the condition "... BETWEEN min AND max"
{
  ...
  else if (is_float && (*(double *)&v1 > *(double *)&dpn.local_max || *(double *)&v2 < *(double *)&dpn.local_min))
  {
    res = RS_NONE;
  }
  ...
  else if (is_float && (*(double *)&v1 <= *(double *)&dpn.local_min && *(double *)&v2 >= *(double *)&dpn.local_max))
  {
    res = RS_ALL;
  }
...
_uint64 RCAttr::ApproxDistinctVals(bool incl_nulls, Filter *f, RSValue *rf, bool outer_nulls_possible)
{
  ...
  if (TypeName() == RC_DATE)
  {
    ...
    {  // in case of any problems with conversion of dates - just numerical approximation
  ...
  else if (PackType() == PackN && TypeName() != RC_REAL && TypeName() != RC_FLOAT)
  {
    ...
    if (span < 100000)
    {  // use Histograms to calculate exact number of distinct values
      RSIndex_Hist *rsi_hist = LoadRSI_Hist();
      ...
      for (int p = 0; p < NoPack(); p++)
      {
        ...
        if ((f == NULL || !f->IsEmpty(p)) && (rf == NULL || rf[p] != RS_NONE) && dpn.local_min <= dpn.local_max)
        {
          ...
          {  // no Histogram or not exact: mark the whole interval
...
_uint64 RCAttr::ExactDistinctVals(
      ...
      for (int p = 0; p < NoPack(); p++)
        if (f->IsFull(p))
        {
          ...
          if (dpn.local_min < dpn.local_max)
          {
            ...
            {  // no Histogram or not exact: cannot calculate exact number
              ReleaseRSI(rsi_hist);
              return NULL_VALUE_64;
            }
          }
...
void RCAttr::RoughStats(double &hist_density, int &trivial_packs,
                        double &span)  // calculate the number of 1's in histograms and other KN stats
{
  ...
  for (int pack = 0; pack < npack; pack++)
  {
    DPN const &dpn(dpns[pack]);
    if (dpn.pack_file == PF_NULLS_ONLY || dpn.no_objs == dpn.no_nulls - 1 ||
        (PackType() == PackN && dpn.no_nulls == 0 && dpn.local_min == dpn.local_max))
      trivial_packs++;
    else
    {
      if (PackType() == PackN && !ATI::IsRealType(TypeName()))
...
void RCAttr::DisplayAttrStats(Filter *f)  // filter is for # of objects
{
  int npack = NoPack();
  LoadPackInfo();
  rccontrol.lock() << "Column " << attr_number << ", table " << table_number << (IsUnique() ? ", unique" : " ")
                   << unlock;
  if (PackType() == PackN)
  ...
  for (int pack = 0; pack < npack; pack++)
  {
    ...
    sprintf(line_buf, "%-7d %5lld %5d", pack, cur_obj, dpn.no_nulls);
    ...
    if (dpn.no_objs + 1 != dpn.no_nulls)
    {
      if (PackType() == PackN && !ATI::IsRealType(TypeName()))
      {
        ...
        sprintf(line_buf + strlen(line_buf), " %16lld %16lld", GetMinInt64(pack), GetMaxInt64(pack));
      ...
      else if (PackType() == PackN && ATI::IsRealType(TypeName()))
      {
        ...
        sprintf(line_buf + strlen(line_buf), " %16g %16g %15g  -", *((double *)(&dpn.local_min)),
                *((double *)(&dpn.local_max)), *((double *)(&sum_d)));
      }
      else
      {  // PackType = PackS
        char smin[10];
        ...
        if (rsi_cmap && !RequiresUTFConversions(Type().GetCollation()))
        {
          ...
          int i;
          for (i = 0; i < len && i < 8; i++)
          {  // display at most 8 positions (limited by line width)
            if ((i == len - 1) || (i == 7))
