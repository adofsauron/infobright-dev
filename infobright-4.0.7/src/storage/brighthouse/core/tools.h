...
template <int const id, typename T, int const no_dims, template <typename> class cmp_algo = object_id_helper::range_eq,
...
class ObjectId : public U
{
 ...
 public:
  ...
  ObjectId(void) : _coord() { std::fill(_coord, _coord + no_dims, 0); }
  ...
  size_t hash(void) const { return (object_id_helper::hasher<T, no_dims>::calc(_coord)); }
...
inline int pc_table(PackCoordinate const &pc) { return (pc[bh::COORD::TABLE]); }

inline int pc_column(PackCoordinate const &pc) { return (pc[bh::COORD::COLUMN]); }

inline int pc_dp(PackCoordinate const &pc) { return (pc[bh::COORD::DP]); }
...
struct copy_object
{
  T operator()(T const &obj) { return (obj); }
};
...
class MemberFunctionExecutor
{
 public:
  ...
  ~MemberFunctionExecutor() { (obj->*on_end)(argument); }
...
{
 public:
  TemporalValueReplacement(T &to_replace, T temp_value) : orig_value(to_replace), orig_value_ptr(&to_replace)
  {
    *orig_value_ptr = temp_value;
  }

  ~TemporalValueReplacement() { *orig_value_ptr = orig_value; }
