...
RSI_Manager::RSI_Manager(const char *_path, int _index_l) : path(_path)
{
...
RSI_Manager::~RSI_Manager() { IBGuard guard(rsi_mutex); }
...
RSIndex *RSI_Manager::GetIndex(RSIndexID id, int read_loc)
{
  ...
  assert(rsi ==
         NULL);  // index doesn't exist or ID is invalid (old and waiting for destruction) or ID is not created yet
  if (!valid)
  {
...
RSIndex *RSI_Manager::GetIndexForUpdate(RSIndexID id, int read_loc)
{
  ...
  if (pos < 0)
    return NULL;
  if (pool->IsLocked(pos))
    return NULL;  // the index is already locked for update by this process
...
void RSI_Manager::DeleteIndex(int tab, int col)
{
  ...
  for (int i = 0; i < numberKNtypes; i++) DeleteIndexFile(RSIndexID(t[i], tab, col));
  if (changed)
    pool->Cleaning();
}
...
RSIndex *RSI_Manager::LoadIndex(RSIndexID id, bool write, int read_loc)
{
  ...
  {
    case RSI_TEST:
      rsi = new RSIndex_Test();
      break;
    case RSI_HIST:
      rsi = new RSIndex_Hist();
      break;
    case RSI_CMAP:
      rsi = new RSIndex_CMap();
      break;
    default:
      assert(0);
      break;
...
{
  versions_map_type::iterator iter =
      find_if(versions_map.begin(), versions_map.end(), bind(&versions_map_type::value_type::first, _1) == ib_version);
...
void RSIndexPool::DeleteFromDef(int pos)
{
  ...
  if (index[pos].info->write)
    index[pos].info->write->Invalidate();
...
inline bool RSIndexPool::IsLocked(int pos) { return index[pos].info->write != NULL; }

inline void RSIndexPool::LoadFailed(int pos) { index[pos].info->nfail++; }
...
inline void RSIndexPool::PutForWrite(int pos, RSIndex *rsi, int lock)
{
  ...
  {  // in the case that ID is during destruction, already deleted from RSI def
...
inline void RSIndexPool::Cleaning(int pos)
{
  if (pos < 0)
  {
    for (int i = 0; i < (int)index.size(); i++) Cleaning(i);
    return;
  }
  ...
  if (index[pos].info->write)
  {
    assert(index[pos].info->write->IsLocked());
  }
  vector<RSIndex *> &read = index[pos].info->read;
...
{
  int typeIn = 0;
  if (1 != fscanf(f, "%18d ", &typeIn))
    return 1;
  type = static_cast<RSIndexType>(typeIn); /* *CAUTION* */
  if (!IsType1() && !IsType2())
    return 1;
  if (2 != fscanf(f, "%18d %18d ", &tab, &col))
    return 1;
  if (IsType2() && (2 != fscanf(f, "%18d %18d ", &tab2, &col2)))
    return 1;
  if (!IsCorrect())
    return 1;
  return 0;
...
{
  if (0 > fprintf(f, "%d", type))
    return 1;
  if (0 > fprintf(f, "\t%d\t%d", tab, col))
    return 1;
  if (IsType2() && (0 > fprintf(f, "\t%d\t%d", tab2, col2)))
    return 1;
  if (0 > fprintf(f, "\n"))
    return 1;
  return 0;
...
{
  return (col == id.col) && (tab == id.tab) && (type == id.type) &&
         (IsType1() || ((tab2 == id.tab2) && (col2 == id.col2)));
}
...
std::ostream &operator<<(std::ostream &outstr, const RSIndexID &rsi)
{
  switch (rsi.type)
  {
    case RSI_CMAP:
    case RSI_HIST:
      outstr << rsi.type << "\t" << rsi.tab << "\t" << rsi.col;
      break;
    case RSI_JPP:
      outstr << rsi.type << "\t" << rsi.tab << "\t" << rsi.col << "\t" << rsi.tab2 << "\t" << rsi.col2;
      break;
    default:
      break;
  }
...
{
  delete write;
  for (size_t i = 0; i < read.size(); i++) delete read[i];
}
...
vector<RSIndexID> RSI_Manager::GetAll(int tab)  // get a list of all existing RSIndices concerning the given table
{
  ...
  for (directory_iterator itr_db(path); itr_db != end_itr; ++itr_db)
  {
    ...
    vector<string> vs;
    for (tokenizer::iterator it = tok.begin(); it != it_end; ++it) vs.push_back(*it);
  ...
  if (unpars > 2)
  {
    rclog << lock << "WARNING: There were found " << unpars << " filenames in KNFolder that could not be parsed."
          << unlock;
  }
  return list;
}
    ...
    if (index_level > 0 || force)
    {
      ...
      try
      {
        if (!DoesFileExist(rsi_path))
        {
          ...
          try
          {
            RSI_Manager::CreateMetadataFile(MakeDirectoryPathOsSpecific(rsi_path) + RSI_METADATAFILE,
                                            RSI_Manager::GetRSIVersionCode((RSI_MANAGER_VERSION)));
          }
          catch (std::exception &)
          {
...
int RunRSIUpdater(const string &datadir_path, const string &kn_folder)
{
  ...
  {
    rclog << lock
          << string("Brighthouse: Fatal error: Update process failed.") +
                 string(" For details on the error please see the updater.log file in the " + datadir_path +
                        " directory.")
          << unlock;
  }
