...
MIIterator::MIIterator(MultiIndex *_mind, DimensionVector &_dimensions, std::vector<PackOrderer> &_po)
    : mind(_mind), po(_po)  // Ordered iterator
{
  no_dims = mind->NoDimensions();
  mind_created_locally = false;
  cur_pos = NULL;
      ...
      one_filter_dim(sec.one_filter_dim)
{
  ...
  for (int i = 0; i < sec.it.size(); i++) it.push_back(sec.dg[i]->CopyIterator(sec.it[i]));
...
void MIIterator::Init(bool lock)
{
  ...
  for (int i = 0; i < mind->dim_groups.size(); i++) dim_group_used[i] = false;
  for (int i = 0; i < no_dims; i++)
    if (dimensions[i])
    {
  ...
  if (no_dims > 0)
  {
    ...
    for (int i = 0; i < mind->dim_groups.size(); i++)
      if (dim_group_used[i] && (mind->dim_groups[i]->Type() == DimensionGroup::DG_FILTER ||
        ...
        ordering_filters.push_back(pair<int, int>(65537 - mind->dim_groups[i]->GetFilter(-1)->DensityWeight(),
                                                  i));  // -1: the default filter for this group
    sort(ordering_filters.begin(),
         ordering_filters.end());  // order filters starting from the densest one (for pack decompression efficiency)

    for (uint i = 0; i < ordering_filters.size(); i++)
    {  // create iterators for DimensionGroup numbers from sorter
...
void MIIterator::GeneralIncrement()
{
  ...
  if (!done)
  {  // pack switched on every dimension - make progress in the whole pack numbering
    for (int i = 0; i < it.size(); i++)
    {
      it[i]->NextPackrow();
      if (it[i]->IsValid())
...
void MIDummyIterator::Combine(
    const MIIterator &sec)  // copy position from the second iterator (only the positions used)
{
  for (int i = 0; i < no_dims; i++)
    if (sec.DimUsed(i))
    {
