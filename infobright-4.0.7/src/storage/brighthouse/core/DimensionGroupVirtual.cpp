...
DimensionGroupVirtual::DimensionGroupVirtual(
    DimensionVector &dims, int bdim, Filter *f_source,
    int copy_mode)  // copy_mode: 0 - copy filter, 1 - ShallowCopy filter, 2 - grab pointer
{
  dims_used = dims;
  base_dim = bdim;
  no_dims = dims.Size();
...
void DimensionGroupVirtual::NewDimensionContent(
...
void DimensionGroupVirtual::FillCurrentPos(DimensionGroup::Iterator *it, _int64 *cur_pos, int *cur_pack,
...
{
  // orderable only if all IndexTables are one-block, otherwise shuffling will occur (and IT block may end inside a
  // packrow)
  for (int d = 0; d < no_dims; d++)
...
void DimensionGroupVirtual::DGVirtualIterator::operator++()
{
  ...
  valid = fi.IsValid();
  dim_pos++;
  // WARNING: do not mix iterating with ++ and NextInsidePack(), as the value cur_pack_start is not updated here (for
  // performance reasons)
}
...
bool DimensionGroupVirtual::DGVirtualIterator::BarrierAfterPackrow()
{
  ...
  if (dim_pos + fi.GetPackSizeLeft() >= no_obj)  // important e.g. for case when we're restarting the same dimension
                                                 // group (iterating on a product of many groups)
    return true;
  for (int i = 0; i < no_dims; i++)
...
DimensionGroupVirtual::DGVirtualOrderedIterator::DGVirtualOrderedIterator(Filter *f_to_iterate, int b_dim,
                                                                          DimensionVector &dims, _int64 *ppos,
