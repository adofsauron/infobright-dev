...
class constant
{
 ...
 public:
  ...
  result_type operator()(void) { return (_constant); }
  result_type const operator()(void) const { return (_constant); }
};

/*cached_dpn_splice_allocator::splice_ptr_t cached_dpn_splice_allocator::operator()( int const& spliceNo, Transaction*
trans ) const
{
        splice_ptr_t sp;
        ...
        if ( ( process_type == ProcessType::MYSQLD ) || ( process_type == ProcessType::EMBEDDED ) ) {
                ...
                if(dynamic_cast<RCAttrLoadBase*>(_attr)) {
                        sp = trans->GetSpliceForUpdate( SpliceCoordinate( _attr->table_number, _attr->attr_number,
spliceNo_ ) ); if(!sp) trans->PutObject(SpliceCoordinate( _attr->table_number, _attr->attr_number, spliceNo_ ), sp =
fetch(spliceNo_)); } else sp = trans->GetSplice( SpliceCoordinate( _attr->table_number, _attr->attr_number, spliceNo_ ),
*this ); } else { sp = GlobalDataCache::GetGlobalDataCache().GetObject<splice_t>(SpliceCoordinate( _attr->table_number,
_attr->attr_number, spliceNo_ ), bind( &cached_dpn_splice_allocator::fetch, this, spliceNo_ ) );
        }
        return ( sp );
}*/

cached_dpn_splice_allocator::splice_ptr_t cached_dpn_splice_allocator::get_from_cache(int const &spliceNo,
                                                                                      Transaction *trans) const
{
  ...
  {
    sp = GlobalDataCache::GetGlobalDataCache().GetObject<splice_t>(
        SpliceCoordinate(_attr->table_number, _attr->attr_number, spliceNo_));
  }
  ...
  if ((process_type == ProcessType::MYSQLD) || (process_type == ProcessType::EMBEDDED))
  {
    ...
    else
    {
      if (trans->IsTableModified(_attr->table_number) && for_local_cache(_initialNumberOfPacks, spliceNo_))
        ...
        GlobalDataCache::GetGlobalDataCache().PutObject(
            SpliceCoordinate(_attr->table_number, _attr->attr_number, spliceNo_), TrackableObjectPtr(sp_));
    }
  }
  else
  {
    GlobalDataCache::GetGlobalDataCache().GetObject<splice_t>(
        SpliceCoordinate(_attr->table_number, _attr->attr_number, spliceNo_), constant<splice_ptr_t>(sp_));
  }
}
...
bool cached_dpn_splice_allocator::in_cache(int const &spliceNo, Transaction *trans) const
{
  ...
  if ((process_type == ProcessType::MYSQLD) || (process_type == ProcessType::EMBEDDED))
  {
    ...
    {
      if (trans->IsTableModified(_attr->table_number) && for_local_cache(_initialNumberOfPacks, spliceNo_))
        inCache = trans->HasLocalObject(SpliceCoordinate(
            _attr->table_number, _attr->attr_number,
            spliceNo_));  //<TODO>[MC] if this is for local cache why we were checking if it is in global?
      else
        inCache = GlobalDataCache::GetGlobalDataCache().HasObject(
            SpliceCoordinate(_attr->table_number, _attr->attr_number, spliceNo_));
    }
  }
  else
...
/*cached_dpn_splice_allocator::splice_ptr_t cached_dpn_splice_allocator::fetch( int const& spliceNo_) const
{
        ...
        int no_splices = ( no_packs + (splice_t::SPLICE_SIZE - 1)) / splice_t::SPLICE_SIZE; // total number of splices
for whole attribute if ( ( ( no_packs - _attr->packs_omitted ) > 0 ) && ( spliceNo_ < no_splices ) ) {
                ...
                try	{
                        ...
                        if( spliceNo_ == ( no_splices - 1 ) ) {
                                ...
                                /* In case we load last splice, the number of actual dpns to load sould be decreased by
1 due
                        ...
                        fdpn.Seek( ( ( spliceNo_ * splice_t::SPLICE_SIZE ) ) * DPN::STORED_SIZE + 2 * DPN::STORED_SIZE,
SEEK_SET);
...
int cached_dpn_splice_allocator::RestoreCachedSplices(int old_, int newCount, int offset, splice_vector_t &vec_,
                                                      Transaction *trans) const
{
  ...
  while (first_not_cached < newCount && in_cache(first_not_cached - offset, trans)) first_not_cached++;
...
void cached_dpn_splice_allocator::operator()(splice_vector_t &vec_, int old_, int new_, Transaction *trans) const
{
  ...
  if ((no_packs - static_cast<int>(_attr->packs_omitted)) > 0)
  {
    ...
    {
      int nDPNinLastSplice(no_packs % splice_t::SPLICE_SIZE ? int(no_packs % splice_t::SPLICE_SIZE) - 1
                                                            : splice_t::SPLICE_SIZE - 1);
