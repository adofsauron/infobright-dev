...
bool RCEngine::AreConvertible(RCDataType &rcitem, enum_field_types my_type, uint length)
{
  ...
  {
    case MYSQL_TYPE_LONGLONG:
      if (bhtype == RC_INT || bhtype == RC_MEDIUMINT || bhtype == RC_BIGINT ||
          (bhtype == RC_NUM && dynamic_cast<RCNum &>(rcitem).Scale() == 0))
        return true;
      break;
    case MYSQL_TYPE_NEWDECIMAL:
    ...
    case MYSQL_TYPE_LONG_BLOB:
      return (bhtype == RC_STRING || bhtype == RC_VARCHAR || bhtype == RC_BYTE || bhtype == RC_VARBYTE ||
              bhtype == RC_BIN);
...
ATI RCEngine::GetCorrespondingATI(Field &field)
{
  ...
  if (ATI::IsNumericType(at))
  {
    ...
    if (at == RC_NUM)
    {
      ...
      return ATI(at, !field.maybe_null(), static_cast<Field_new_decimal &>(field).precision,
