...
GroupByWrapper::GroupByWrapper(int a_size, bool just_distinct, ConnectionInfo &conn) : distinct_watch(), m_conn(conn)

{
  ...
  for (int i = 0; i < attrs_size; i++) attr_mapping[i] = -1;
...
void GroupByWrapper::AddAggregatedColumn(int orig_attr_no, TempTable::Attr &a,
                                         // "a" must be a reference, because a.term will be taken as a pointer
                                         _int64 max_no_vals, _int64 min_v, _int64 max_v, ushort max_size)

{
  ...
  gt->AddAggregatedColumn(virt_col[attr_no], ag_oper, ag_distinct, ag_type, ag_size, ag_prec,
                          ag_collation);  // note: size will be automatically calculated for all numericals
...
void GroupByWrapper::Merge(GroupByWrapper &sec)
{
  ...
  no_groups +=
      gt->GetNoOfGroups() - old_groups;  // note that no_groups may be different than gt->..., because it is global
}

bool GroupByWrapper::AggregatePackInOneGroup(int attr_no, MIIterator &mit, _int64 uniform_pos, _int64 rows_in_pack,
                                             _int64 factor)
{
      ...
      virt_col[attr_no])
  {
    ...
    aggregated_roughly =
        gt->AggregatePack(attr_no, uniform_pos);  // check the aggregation basing on the above parameters
...
void GroupByWrapper::OmitInCache(int gr_a, _int64 obj_to_omit) { distinct_watch.gd_cache[gr_a].Omit(obj_to_omit); }
...
bool GroupByWrapper::IsCountOnly(int gr_a)  // true, if an attribute is count(*)/count(const), or if there is no columns
                                            // except constants and count (when no attr specified)
{
  if (gr_a != -1)
  {
  ...
  for (int i = 0; i < no_attr; i++)
  {  // function should return true for e.g.: "SELECT 1, 2, 'ala', COUNT(*), 5, COUNT(4) FROM ..."
...
void DistinctWrapper::DeclareAsDistinct(int attr) { is_dist[attr] = true; }
