...
class AggregationAlgorithm
{
 public:
  ...
  bool AggregateRough(GroupByWrapper &gbw, MIIterator &mit, bool &packrow_done, bool &part_omitted,
                      bool &ag_not_changeabe, bool &stop_all, _int64 &uniform_pos, _int64 rows_in_pack,
                      _int64 local_factor, int just_one_aggr = -1);
  void MultiDimensionalGroupByScan(GroupByWrapper &gbw, _int64 &limit, _int64 &offset,
                                   std::map<int, std::vector<PackOrderer::OrderingInfo>> &oi, ResultSender *sender,
...
class AggregationWorker
{
 public:
  ...
  // Return code for AggregatePackrow: 0 - success, 1 - stop aggregation (finished), 2 - killed, 3 - overflow, 4 - other
  // error, 5 - pack already aggregated (skip)
  int AggregatePackrow(MIInpackIterator &lmit, _int64 cur_tuple)
  {
