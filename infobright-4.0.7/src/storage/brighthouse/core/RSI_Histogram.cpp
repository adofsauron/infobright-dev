...
RSIndex_Hist::~RSIndex_Hist() { Deallocate(); }
...
void RSIndex_Hist::Alloc(uint64 new_no_pack)
{
  ...
  if (new_no_pack > no_pack_declared)
  {
    ...
    {
      // extend last block
      uint64 new_no_pack_in_last_block =
          new_bc > old_bc ? NO_HISTS_IN_BLOCK : NoPacksInLastBlock(new_no_pack_declared - start_pack);
      uint64 old_no_pack_in_last_block = NoPacksInLastBlock(no_pack_declared - start_pack);

      hist_buffers[old_bc - 1] =
          (uint *)(rc_realloc(hist_buffers[old_bc - 1], new_no_pack_in_last_block * RSI_HIST_SIZE, BLOCK_TEMPORARY));
      memset(hist_buffers[old_bc - 1] + old_no_pack_in_last_block * RSI_HIST_INT_RES, 255,
             (new_no_pack_in_last_block - old_no_pack_in_last_block) * RSI_HIST_SIZE);
    }
    ...
    if (new_bc > old_bc)
    {
      hist_buffers[new_bc - 1] =
          (uint *)alloc((size_t)NoPacksInLastBlock(new_no_pack_declared - start_pack) * RSI_HIST_SIZE, BLOCK_TEMPORARY);
      memset(hist_buffers[new_bc - 1], 255,
             (size_t)NoPacksInLastBlock(new_no_pack_declared - start_pack) * RSI_HIST_SIZE);
    }
...
{
  return hist_buffers[(pack_no - start_pack) / NO_HISTS_IN_BLOCK] +
         ((pack_no - start_pack) % NO_HISTS_IN_BLOCK) * RSI_HIST_INT_RES;
}

void RSIndex_Hist::Deallocate()
{
  for (hist_buffers_t::iterator it(hist_buffers.begin()), end(hist_buffers.end()); it != end; ++it) dealloc(*it);
  hist_buffers.clear();
}
...
bool RSIndex_Hist::UpToDate(_int64 cur_no_obj, int pack)  // true, if this pack is up to date (i.e. either
                                                          // cur_obj==no_obj, or pack is not the last pack in the index)
{
...
int RSIndex_Hist::NoOnes(int pack, int width)
{
  ...
  for (int i = start_int; i < stop_int; i++) d += CalculateBinSum(hist[i]);

  int bits_left = width % 32;
  if (stop_int - start_int < RSI_HIST_INT_RES && bits_left > 0)
  {  // there are bits left in exact mode
...
RSValue RSIndex_Hist::IsValue(_int64 min_v, _int64 max_v, int pack, _int64 pack_min, _int64 pack_max)
{
  ...
  else
  {
    ...
    if (max_v >= pack_max || min_v <= pack_min)
      return RS_SOME;  // pack_min xor pack_max are present
    // now we know that (max_v<pack_max) and (min_v>pack_min) and there is only RS_SOME or RS_NONE answer possible
    if (ExactMode(pack_min, pack_max))
    {                                       // exact mode
...
void RSIndex_Hist::PutValue(_int64 v, int pack, _int64 pack_min,
                            _int64 pack_max)  // set information that value v does exist in this pack
{
  if (pack < start_pack || pack > end_pack)
    return;
...
bool RSIndex_Hist::Intersection(int pack, _int64 pack_min, _int64 pack_max, RSIndex_Hist *sec, int pack2,
                                _int64 pack_min2, _int64 pack_max2)
{
  ...
  if (ExactMode(pack_min, pack_max) && ExactMode(pack_min2, pack_max2))
  {  // exact mode
    int bit1, bit2;
...
{
  uint no_objs_to_add_to_last_DP =
      uint(no_obj % MAX_PACK_ROW_SIZE ? MAX_PACK_ROW_SIZE - no_obj % MAX_PACK_ROW_SIZE : 0);
