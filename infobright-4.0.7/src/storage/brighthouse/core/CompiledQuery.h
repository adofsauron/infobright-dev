...
class CompiledQuery
{
 public:
  ...
  void CreateConds(CondID &c_out, const TabID &t1, CQTerm e1, Operator pr, CQTerm e2, CQTerm e3 = CQTerm(),
  ...
  void AddColumn(AttrID &a_out, const TabID &t1, CQTerm e1, ColOperation op, char const alias[] = 0,
                 bool distinct = false);
  ...
  void CreateVirtualColumn(AttrID &a_out, const TabID &t1, MysqlExpression *expr,
                           const TabID &src_table = TabID(NULL_VALUE_32));

  /*! \brief Create compilation step CREATE_VC for subquery
   * \param a_out - id of created virtual column
   * \param t1 - id of TempTable for which virtual column is created
   * \param subquery - id of TempTable representing subquery
   * \param on_result - indicator of which MultiIndex should be passed to Virtual column: one representing source data
   * of t1 (\e false) or one representing output columns of t1 (\e true), e.g. in case of subquery in HAVING \return
   * void
   */
  ...
  { /*assert(no_virt_cols.find(tt) != no_virt_cols.end());*/
 ...
 private:  // NOTE: new objects' IDs (e.g. while declaring new aliases, filters etc.) are reserved and obtained by these
           // functions.
  TabID NextTabID()
  {
    no_tabs++;
