...
class ValueMatching_HashTable : public TrackableObject, public ValueMatchingTable
{
 public:
  ...
  virtual void Init(_int64 mem_available, _int64 max_no_groups, int _total_width, int _input_buf_width,
                    int _match_width);
 ...
 protected:
  ...
  unsigned int ht_mask;  // (crc_code & ht_mask) is a position of code in ht table; ht_mask + 1 is the total size (in
                         // integers) of ht
