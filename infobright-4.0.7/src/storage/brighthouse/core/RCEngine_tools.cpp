...
bool RCEngine::IsBHRoute(THD *thd, TABLE_LIST *table_list, SELECT_LEX *selects_list,
    ...
    if (!get_parameter(thd, BH_DATAFORMAT, param, s_res))
    {
      ...
      else if (!df->CanExport())
      {
        my_message(ER_SYNTAX_ERROR,
                   (string("Brighthouse specific error: Export in '") + df->GetName() + ("' format is not supported."))
                       .c_str(),
                   MYF(0));
        return true;
      }
...
auto_ptr<IOParameters> RCEngine::CreateIOParameters(const string &path)
{
  ...
  for (; iter != end; ++iter) columns_collations.push_back(iter->collation->number);
...
BHError RCEngine::GetRejectFileIOParameters(THD &thd, IOParameters &io_params)
{
  ...
  if (abort_on_count != 0 && abort_on_threshold != 0)
    return BHError(BHERROR_WRONG_PARAMETER,
                   "BH_ABORT_ON_COUNT and BH_ABORT_ON_THRESHOLD parameters are mutualy exclusive.");
  ...
  if ((abort_on_count != 0 || abort_on_threshold != 0) && reject_file.empty())
    return BHError(
        BHERROR_WRONG_PARAMETER,
        "BH_ABORT_ON_COUNT or BH_ABORT_ON_THRESHOLD can by only specified with BH_REJECT_FILE_PATH parameter.");
...
BHError RCEngine::GetIOParameters(IOParameters &io_params, THD &thd, sql_exchange &ex, TABLE *table, bool for_exporter)
{
  ...
  if (IsPipe(ex.file_name))
  {
    ...
    if (!get_parameter(&thd, BH_PIPEMODE, param, s_res))
      io_params.SetOutput(io_mode, ex.file_name,
                          (int)param);  // BH_PIPEMODE == 0 - pipe work as a client otherwise as a server
    else
      io_params.SetOutput(io_mode, ex.file_name);
  }
  else
  ...
  if (unsupported_syntax)
    push_warning(&thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                 "Query contains syntax that is unsupported in Infobright and which will be ignored.");
  return BHERROR_SUCCESS;
#endif
}
