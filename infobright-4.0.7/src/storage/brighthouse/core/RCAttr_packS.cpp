...
AttrPackS::AttrPackS(PackCoordinate pc, AttributeType attr_type, int inserting_mode, bool no_compression,
                     DataCache *owner)
    : AttrPack(pc, attr_type, inserting_mode, no_compression, owner),
      ver(0),
      previous_no_obj(0),
      data(0),
      index(0),
      lens(0),
      decomposer_id(0),
      use_already_set_decomposer(false),
      outliers(0)
{
  Construct();
}
...
AttrPackS::AttrPackS(const AttrPackS &aps)
    : AttrPack(aps),
      ver(aps.ver),
      previous_no_obj(aps.previous_no_obj),
      data(0),
      index(0),
      lens(0),
      decomposer_id(aps.decomposer_id),
      use_already_set_decomposer(aps.use_already_set_decomposer),
      outliers(aps.outliers)
{
  try
  {
...
std::auto_ptr<AttrPack> AttrPackS::Clone() const { return std::auto_ptr<AttrPack>(new AttrPackS(*this)); }
...
void AttrPackS::Prepare(int no_nulls)
{
  ...
  for (int i = 0; i < no_nulls; i++) SetNull(i);  // no_nulls set inside
  is_empty = false;
  Unlock();
}
  ...
  try
  {
    ...
    else
    {
      ...
      else
      {
        // normal compression
        index = 0;
        if (IsModeCompressionApplied())  // 0,1	= Huffmann 16-bit compression of remainings with tree (0 - flat null
                                         // encoding, 1 - compressed nulls)
        {
...
void AttrPackS::Expand(int no_obj)
{
  ...
  for (uint o = this->no_obj; o < new_max_no_obj; o++) index[o] = data[cs++];
...
void AttrPackS::CopyBinded()
{
  ...
  {
    int size_sum = 0;
    for (uint i = last_copied + 1; i < no_obj; i++) size_sum += GetSize(i);
    if (size_sum > 0)
    {
        ...
        if(no_nulls > 0) {
                ...
                if(!IsModeNullsCompressed()) // flat null encoding
                        ...
                        if(no_nulls != nulls_counted)
                                rclog << lock << "Error: AttrPackT::Uncompress uncompressed wrong number of nulls." <<
unlock;
                }
        ...
        for (int g=0; g<no_groups; g++) {
                ...
                if((_uint64) * (cur_buf + 2) != 0) {
                        // decompress lenghts
                        nc.Decompress(lens, (char*) (cur_buf + 3), block_comp_len_buf_size, no_obj - no_nulls, *(uint*)
(cur_buf + 2));
                        ...
                        if(objs)
                                tc.Decompress(block_data, block_data_size, (char*)cur_buf,
block_lengths[g]-12-block_comp_len_buf_size, block_index, lens, objs); cur_buf = (uint*)((char*)cur_buf +
(block_lengths[g]-12-block_comp_len_buf_size));
  ...
  if (no_nulls > 0)
  {
    ...
    else
    {
      ...
      if (res != CPRS_SUCCESS)
      {
        ...
        msg_buf << "Decompression of nulls failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1) << ", pack "
                << (GetCoordinate()[bh::COORD::DP] + 1) << " (error " << res << ").";
        throw DatabaseRCException(msg_buf.str());
      }
      // For tests:
      uint nulls_counted = 0;
      for (uint i = 0; i < 2048; i++) nulls_counted += CalculateBinSum(nulls[i]);
      if (no_nulls != nulls_counted)
...
void AttrPackS::UncompressOld()
{
  ...
  if (no_nulls > 0)
  {
    ...
    else
    {
      ...
      // For tests:
      uint nulls_counted = 0;
      for (i = 0; i < 2048; i++) nulls_counted += CalculateBinSum(nulls[i]);
      if (no_nulls != nulls_counted)
        rclog << lock << "Error: AttrPackT::Uncompress uncompressed wrong number of nulls." << unlock;
    }
    cur_buf = (uint *)((char *)cur_buf + null_buf_size + 2);
    ...
    for (i = 0; i < (int)no_obj; i++) tmp_index[i] = FREE_PLACE;
  ...
  else
  {
    ...
    if ((_uint64) * (cur_buf + 1) != 0)
    {
      ...
      CprsErr res = nc.Decompress(cn_ptr.get(), (char *)(cur_buf + 2), comp_len_buf_size - 8, no_obj - no_nulls,
                                  *(uint *)(cur_buf + 1));
      if (res != CPRS_SUCCESS)
      {
        std::stringstream msg_buf;
        msg_buf << "Decompression of lengths of string values failed for column "
                << (GetCoordinate()[bh::COORD::COLUMN] + 1) << ", pack " << (GetCoordinate()[bh::COORD::DP] + 1)
                << " (error " << res << ").";
        throw DatabaseRCException(msg_buf.str());
      }
    ...
    if (objs)
    {
      ...
      CprsErr res = tc.Decompress((char *)*data, data_full_byte_size, (char *)cur_buf, dlen, tmp_index.get(),
                                  tmp_len.get(), objs);
      if (res != CPRS_SUCCESS)
      {
        std::stringstream msg_buf;
        msg_buf << "Decompression of string values failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1)
                << ", pack " << (GetCoordinate()[bh::COORD::DP] + 1) << " (error " << res << ").";
        throw DatabaseRCException(msg_buf.str());
      }
    }
  }
...
// void AttrPackS::Decompose(uint& size, std::vector<uchar**>& indexes, std::vector<ushort*>& lengths,
// std::vector<uint>& maxes, std::vector<uint>& sumlens)
//{
//	no_groups = 2;
...
CompressionStatistics AttrPackS::Compress8(DomainInjectionDecomposer &decomposer)
{
  ...
  if (no_nulls > 0)
  {
    ...
    else
    {
      ...
      {
        std::stringstream msg_buf;
        msg_buf << "Compression of nulls failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1) << ", pack "
                << (GetCoordinate()[bh::COORD::DP] + 1) << " (error " << res << ").";
  ...
  for (int b = 0; b < no_groups; b++) *(p++) = decomposedData[b]->GetType();  // types of block's compression
...
CompressionStatistics AttrPackS::CompressOld()
{
  ...
  if (maxv != 0)
  {
    ...
    {
      std::stringstream msg_buf;
      msg_buf << "Compression of lengths of string values failed for column "
              << (GetCoordinate()[bh::COORD::COLUMN] + 1) << ", pack " << (GetCoordinate()[bh::COORD::DP] + 1)
              << " (error " << res << ").";
      throw InternalRCException(msg_buf.str());
    }
  ...
  *(comp_len_buf.get() + 1) = maxv;
  ...
  if (data_full_byte_size)
  {
    ...
    CprsErr res = tc.Compress(comp_buf.get(), dlen, tmp_index.get(), tmp_len.get(), objs,
                              ShouldNotCompress() ? 0 : TextCompressor::VER);
    if (res != CPRS_SUCCESS)
    {
      std::stringstream msg_buf;
      msg_buf << "Compression of string values failed for column " << (GetCoordinate()[bh::COORD::COLUMN] + 1)
              << ", pack " << (GetCoordinate()[bh::COORD::DP] + 1) << " (error " << res << ").";
      throw InternalRCException(msg_buf.str());
    }
  }
...
void AttrPackS::StayCompressed()
{
  ...
  {
    int ds = (int)rc_msize(data) / sizeof(char *);
    for (int i = 0; i < ds && i < data_id; i++) dealloc(data[i]);
    dealloc(data);
    data = 0;
  }
