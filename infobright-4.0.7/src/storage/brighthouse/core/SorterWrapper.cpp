...
void SorterWrapper::InitOrderByVector(vector<int> &order_by)
{
  ...
  for (int i = 0; i < input_cols.size(); i++) order_by.push_back(-1);  // -1 means "no column"
...
void SorterWrapper::InitSorter(MultiIndex &mind)
{
  ...
  DimensionVector one_pack_dims(
      mind.NoDimensions());  // these dimensions contain only one used pack (potentially implicit)
  ...
  for (int dim = 0; dim < mind.NoDimensions(); dim++)
    if (one_pack_dims[dim] && !implicit_dims[dim])
    {  // potentially implicit, check sizes
  ...
  for (int i = 0; i < scol.size(); i++)
  {
    if (scol[i].IsEnabled())
    {
      ...
      else if (scol[i].GetSecondarySize() > 0)
      {  // secondary place needed for sorted columns?
  ...
  if (total_bytes > 0)
  {  // the sorter is nontrivial
    s = Sorter3::CreateSorter(no_of_rows, key_bytes, total_bytes, limit);
    input_buf = new unsigned char[total_bytes];
    if (mi_encoder)
      rccontrol.lock(s->conn->GetThreadID()) << s->Name() << " initialized for " << no_of_rows << " rows, " << key_bytes
                                             << "+" << total_bytes - key_bytes - mi_encoder->GetPrimarySize() << "+"
                                             << mi_encoder->GetPrimarySize() << " bytes each." << unlock;
    else
      rccontrol.lock(s->conn->GetThreadID()) << s->Name() << " initialized for " << no_of_rows << " rows, " << key_bytes
                                             << "+" << total_bytes - key_bytes << " bytes each." << unlock;
  }
}
...
{
  if (rough_sort_by > -1)
  {  // nontrivial rough_sort_by => order based on one dimension
...
bool SorterWrapper::PutValues(MIIterator &mit)
{
  ...
  bool update_stats = (rough_sort_by > -1 && limit > -1 &&
                       no_values_encoded <= limit);  // otherwise statistics are either not used or already prepared
  for (int i = 0; i < scol.size(); i++)
    if (scol[i].IsEnabled() && scol[i].IsNontrivial())  // constant num. columns are trivial
