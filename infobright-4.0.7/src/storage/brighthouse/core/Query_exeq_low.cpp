...
void ParameterizedFilter::SyntacticalDescriptorListPreprocessing(bool for_rough_query)
{
  ...
  for (uint i = 0; i < no_desc; i++)
  {                                        // Note that desc.size() may enlarge when joining with BETWEEN occur
  ...
  for (uint i = 0; i < no_desc; i++)
  {  // t1.x == t2.y && t2.y == 5   =>   t1.x == 5
    if (!descriptors[i].done && descriptors[i].op == O_EQ && descriptors[i].IsType_JoinSimple() &&
      ...
      for (uint j = 0; j < descriptors.Size(); j++)
      {
        if (i != j && !descriptors[j].done && descriptors[j].IsType_AttrValOrAttrValVal() && descriptors[j].IsInner())
        {
          ...
          if (descriptors[j].attr.vc == descriptors[i].attr.vc)
          {
            ...
            if (descriptors[j].op == O_EQ)
            {  // t2.y == t1.x && t2.y == 5  change to  t1.x == 5 && t2.y == 5
            ...
            {  // t2.y == t1.x && t2.y > 5  change to  t2.y == t1.x && t2.y > 5 && t1.x > 5
              Descriptor dd(table, mind->NoDimensions());
          ...
          if (descriptors[j].attr.vc == descriptors[i].val1.vc)
          {  // the same as above for val1
            // the same table and column
            if (descriptors[j].op == O_EQ)
            {  // t1.x == t2.y && t2.y == 5  change to  t1.x == 5 && t2.y == 5
              descriptors[i].val1 = descriptors[j].val1;
              descriptors[i].CalculateJoinType();
              descriptors[i].CoerceColumnTypes();
              break;
            ...
            {  // t1.x == t2.y && t2.y > 5  change to  t1.x == t2.y && t2.y > 5 && t1.x > 5
              Descriptor dd(table, mind->NoDimensions());
              dd.attr = descriptors[i].attr;
              dd.op = descriptors[j].op;
              dd.val1 = descriptors[j].val1;
  ...
  if (!false_desc)
  {
    for (uint i = 0; i < no_desc; i++)
    {
      ...
      if (descriptors[i].IsTrue())
      {  // again, because something might be simplified
        if (descriptors[i].IsInner())
          descriptors[i].done = true;
        continue;
      }
...
void ParameterizedFilter::DescriptorListOrdering()
{
  ...
  if (descriptors.Size() > 1 || descriptors[0].IsType_OrTree())
  {  // else all evaluations remain 0 (default)
...
void ParameterizedFilter::UpdateMultiIndex(bool count_only, _int64 limit)
{
  ...
  */
      ...
      if (!descriptors[i].done && !descriptors[i].IsType_Join() && descriptors[i].IsInner())
      {
        ...
        if (descriptors[i].IsDelayed())
          rccontrol.lock(mind->m_conn->GetThreadID())
              << "Delayed: " << buf << " \t(" << int(descriptors[i].evaluation * 100) / 100.0 << ")" << unlock;
        else
          rccontrol.lock(mind->m_conn->GetThreadID()) << "Cnd(" << i << "):  " << buf << " \t("
                                                      << int(descriptors[i].evaluation * 100) / 100.0 << ")" << unlock;
      }
  }
  ...
  for (uint i = 0; i < descriptors.Size(); i++)
  {
        ...
        !descriptors[i].IsDelayed())
    {
      ...
      if (cur_dim >= 0 && mind->GetFilter(cur_dim) && mind->GetFilter(cur_dim)->IsEmpty() && empty_cannot_grow)
      {
        ...
        {
          rccontrol.lock(mind->m_conn->GetThreadID())
              << "Empty result set after non-join condition evaluation (WHERE)" << unlock;
        }
        rough_mind->ClearLocalDescFilters();
        return;
      }
  ...
  for (int i = 0; i < mind->NoDimensions(); i++)
    if (mind->GetFilter(i))
      table->SetVCDistinctVals(
          i, mind->GetFilter(i)->NoOnes());  // distinct values - not more than the number of rows after WHERE
  rough_mind->ClearLocalDescFilters();
  ...
  for (uint i = 0; i < descriptors.Size(); i++)
  {
    ...
    if (!descriptors[i].done && !descriptors[i].IsDelayed())
    {
      ...
      if (join_desc[0].IsOuter())
      {
        for (uint i = 0; i < descriptors.Size(); i++)
        {
          if (descriptors[i].IsDelayed() && !descriptors[i].done && descriptors[i].op == O_IS_NULL &&
              join_desc[0].right_dims.Get(descriptors[i].attr.vc->GetDim()) && !descriptors[i].attr.vc->NullsPossible())
          {
      ...
      {
        // Optimization: count_only is true => do not materialize multiindex (just counts tuples). WARNING: in this case
        // cannot use multiindex for any operations other than NoTuples().
        if (count_only)
