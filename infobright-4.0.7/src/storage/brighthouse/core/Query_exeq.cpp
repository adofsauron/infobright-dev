...
TempTable *Query::Preexecute(CompiledQuery &qu, ResultSender *sender, bool display_now)
{
  ...
  {
    step = qu.Step(i);
    if (step.alias && strcmp(step.alias, "bh_force_mysql") == 0)
    {  // magical word (passed as table alias) to force MySQL execution
  ...
  for (i = 0; i < qu.NoSteps(); i++)
  {
    ...
    try
    {
      ...
      switch (step.type)
      {
        ...
        case CompiledQuery::CREATE_CONDS:
          assert(step.t1.n < 0);
          step.e1.vc = (step.e1.vc_id != NULL_VALUE_32)
                           ? ((TempTable *)ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e1.vc_id)
                           : NULL;
          step.e2.vc = (step.e2.vc_id != NULL_VALUE_32)
                           ? ((TempTable *)ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e2.vc_id)
                           : NULL;
          step.e3.vc = (step.e3.vc_id != NULL_VALUE_32)
                           ? ((TempTable *)ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e3.vc_id)
                           : NULL;
          if (step.n1 != OR_SUBTREE)
          {  // on result = false
            conds[step.c1.n] = new Condition();
            if (step.c2.IsNull())
            {
              conds[step.c1.n]->AddDescriptor(step.e1, step.op, step.e2, step.e3, (TempTable *)ta[-step.t1.n - 1].get(),
                                              qu.GetNoDims(step.t1),
                                              (step.op == O_LIKE || step.op == O_NOT_LIKE) ? char(step.n2) : '\\');
            }
            else
            {
        ...
        case CompiledQuery::ADD_COLUMN:
        {
          ...
          step.a1.n =
              ((TempTable *)ta[-step.t1.n - 1].get())->AddColumn(e, step.cop, step.alias, step.n1 ? true : false);
          break;
        }
        case CompiledQuery::CREATE_VC:
        {
          ...
          else if (step.virt_cols.size() > 0)
          {
            ...
            int c = ((TempTable *)ta[-step.t1.n - 1].get())
                        ->AddVirtColumn(new InSetColumn(ct, t->GetMultiIndexP(), vcs), step.a1.n);
            assert(c == step.a1.n);
          }
          else if (step.a2.n != NULL_VALUE_32)
          {
            ...
            int c =
                ((TempTable *)ta[-step.t1.n - 1].get())
                    ->AddVirtColumn(new SingleColumn(phc, mind, step.t2.n, step.a2.n, ta[-step.t2.n - 1].get(), dim),
                                    step.a1.n);
            assert(c == step.a1.n);
          }
          else
          {
            ...
            int c =
                ((TempTable *)ta[-step.t1.n - 1].get())
                    ->AddVirtColumn(new SubSelectColumn(dynamic_cast<TempTable *>(ta[-step.t2.n - 1].get()),
        ...
        case CompiledQuery::UNION:
          ...
          assert(ta[-step.t2.n - 1]->TableType() == TEMP_TABLE &&
                 (step.t3.n == NULL_VALUE_32 || ta[-step.t3.n - 1]->TableType() == TEMP_TABLE));
    ...
    {
      for (i = 0; i < no_conds; i++) delete conds[i];
      delete[] conds;
      throw;
    }
  }
