...
class ColumnBinEncoder
{
 public:
  ...
  static const int ENCODER_NONCOMPARABLE =
      0x04;  // store values, do not allow comparisons (e.g. UTF8 transformations not applied)
  ...
  bool PrepareEncoder(VirtualColumn *vc1,
                      VirtualColumn *vc2 = NULL);  // encoder for one column, or a common encoder for two of them
  ...
  void SetImplicit()
  {
    implicit = true;
    disabled = true;
  }  // implicit: the column will be read as vc->GetValue(iterator) instead of orig. values
  ...
  bool PutValue64(unsigned char *buf, _int64 v, bool sec_column,
                  bool update_stats = false);  // used in special cases only (e.g. rough), return false if cannot encode
  ...
  bool ImpossibleValues(_int64 pack_min,
                        _int64 pack_max);  // return true if the current contents of the encoder is out of scope
  bool ImpossibleValues(RCBString &pack_min,
                        RCBString &pack_max);  // return true if the current contents of the encoder is out of scope
...
class ColumnBinEncoder::ColumnValueEncoder  // base class for detailed encoders
{
 public:
  ...
  virtual void Encode(unsigned char *buf, unsigned char *buf_sec, VirtualColumn *vc, MIIterator &mit,
  ...
  virtual bool Encode(unsigned char *buf, unsigned char *buf_sec, RCBString &v, bool sec_column, bool update_stats)
  {
    return false;
  }
  ...
  virtual _int64 ValEncode(RCBString &v, bool update_stats)
  {
    assert(0);
    return 0;
  }  // used in special cases only
...
class ColumnBinEncoder::EncoderDecimal : public ColumnBinEncoder::EncoderInt
{
 public:
  ...
  EncoderDecimal(EncoderDecimal &sec)
...
class ColumnBinEncoder::EncoderDouble : public ColumnBinEncoder::ColumnValueEncoder
{
 public:
  ...
  EncoderDouble(EncoderDouble &sec)
 ...
 protected:
  _int64 multiplier_vc1;  // 0 if the first vc is double, otherwise it is regarded as int (decimal) which must be
                          // multiplied
  _int64 multiplier_vc2;  // 0 if the second vc is double, otherwise it is regarded as int (decimal) which must be
                          // multiplied
};
...
class ColumnBinEncoder::EncoderText_UTF : public ColumnBinEncoder::ColumnValueEncoder
{
 public:
  ...
  ~EncoderText_UTF();
  EncoderText_UTF(EncoderText_UTF &sec)
      : ColumnValueEncoder(sec), collation(sec.collation), mins(sec.mins), maxs(sec.maxs), min_max_set(sec.min_max_set)
  {
  }
...
class ColumnBinEncoder::EncoderTextStat : public ColumnBinEncoder::EncoderInt
{
 public:
  ...
  EncoderTextStat(EncoderTextStat &sec)
      : EncoderInt(sec),
        mins(sec.mins),
        maxs(sec.maxs),
