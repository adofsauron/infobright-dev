...
double ParameterizedFilter::EvaluateConditionNonJoinWeight(Descriptor &d, bool for_or)
{
  ...
  else if (d.IsType_AttrValOrAttrValVal())
  {  // typical condition: attr=val
    if (!d.encoded)
    {
  ...
  else if (d.IsType_AttrAttr())
  {                                          // attr=attr on the same table
  ...
  {  // expressions and other types, incl. joins (to be calculated separately)
...
double ParameterizedFilter::EvaluateConditionJoinWeight(Descriptor &d)
{
  ...
  */
  if (dim1 > -1 && dim2 > -1)
  {
    ...
    double r_select1 =
        0.5 + 0.5 * d.attr.vc->RoughSelectivity();  // 1 if no rough exclusions, close to 0.5 if good rough performance
    ...
    if (d.op == O_EQ)
    {
      ...
      no_distinct2 = d.val1.vc->GetApproxDistVals(false);
      if (no_distinct1 >= 0.99 * mind->DimSize(dim1))
      {  // potentially 1:n join
      ...
      else if (no_distinct2 >= 0.99 * mind->DimSize(dim2))
      {  // potentially n:1 join
      ...
      {
        // approximated size: (no_of_classes) * (ave_class_size1 * ave_class_size2) = no_of_classes * (no_obj1 /
        // no_dist1) * (no_obj2 / no_dist2)
  ...
  if (rccontrol.isOn())
  {
    ...
    for (int dim = 0; dim < rough_mind->NoDimensions(); dim++)
    {
      ...
      rccontrol.lock(mind->m_conn->GetThreadID())
          << "(t" << dim << ") Pckrows: " << pack_all << ", susp. " << pack_some << " ("
...
bool ParameterizedFilter::PropagateRoughToMind()
{
  ...
  bool is_nonempty = true;
  for (int i = 0; i < rough_mind->NoDimensions(); i++)
  {  // update classical multiindex
...
void ParameterizedFilter::RoughMakeProjections(int to_dim, bool update_reduced)
{
  ...
  {
    rccontrol.lock(mind->m_conn->GetThreadID())
        << "Packrows excluded by rough multidimensional projections: " << total_excluded << unlock;
