...
class PhysicalColumn : public Column
{
 public:
  ...
  PhysicalColumn(const PhysicalColumn &phc)
      : Column(phc), name(0), desc(0), is_unique(phc.is_unique), is_unique_updated(phc.is_unique_updated)
  {
  }
  ...
  {
    std::vector<_int64> empty;
    return empty;
  }

  //! provide the most probable approximation of number of objects matching the condition
  virtual _uint64 ApproxAnswerSize(Descriptor &d)
  {
    BHASSERT(false, "not overridden");
    return 0;
  }

  virtual _uint64 ApproxDistinctVals(
      bool incl_nulls, Filter *f, RSValue *rf,
      bool outer_nulls_possible) = 0;  // provide the best upper approximation of number of diff. values (incl. null)

  virtual _uint64 ExactDistinctVals(
      Filter *f) = 0;  // provide the exact number of diff. non-null values, if possible, or NULL_VALUE_64
  ...
  RSValue IsDistinct() const
  {
    return (IsUniqueUpdated() ? (IsUnique() ? RS_ALL : RS_NONE) : RS_UNKNOWN);
  }  // three-valued answer

  virtual _int64 RoughMin(Filter *f = NULL,
                          RSValue *rf = NULL) = 0;  // for numerical: best rough approximation of min for a given filter
                                                    // (or global min if filter is NULL)
  virtual _int64 RoughMax(Filter *f = NULL,
                          RSValue *rf = NULL) = 0;  // for numerical: best rough approximation of max for a given filter
                                                    // (or global max if filter is NULL)
  virtual void GetTextStat(TextStat &s, Filter *f = NULL) { s.Invalidate(); }
