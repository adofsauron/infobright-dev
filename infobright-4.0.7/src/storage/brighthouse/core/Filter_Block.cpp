...
Filter::Block::Block(Filter *owner, int _no_obj, bool all_full) { Init(owner, _no_obj, all_full); }
...
bool Filter::Block::Or(Block &b2)
{
  ...
  return no_set_bits == 0 ||
         no_set_bits == no_obj;  // 0 here means only a full pack, other possibilities are excluded by an upper level
}
...
void Filter::Block::Clean(bool value)
{
  if (!block_table)
  {
    owner->bit_mut->Lock();
    block_table =
        (uint *)owner->bit_block_pool->malloc();  //(uint*)alloc(block_size*sizeof(uint), BLOCK_UNCOMPRESSED, owner);
    owner->bit_mut->Unlock();
    if (!block_table)
      throw OutOfMemoryRCException();
  }
...
void Filter::Block::InitBits(int n1, int n2, bool value)
{
  if (value)
    ...
    for (int i = n1; i <= n2; i++) block_table[i / 32] &= ~lshift1[i & 31];
}
