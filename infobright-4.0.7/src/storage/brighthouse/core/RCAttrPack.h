...
class AttrPack : public TrackableObject
{
 ...
 public:
  ...
  virtual CompressionStatistics Compress(
      DomainInjectionManager &dim) = 0;  // Create optimal compressed buf. basing on full data.
  ...
  virtual int Save(IBStream *fcurfile,
                   DomainInjectionManager &dim) = 0;  // save all pack data (parameters, dictionaries, compressed data)
...
class AttrPackN : public AttrPack
{
 ...
 private:
  ...
  void CopyValues(D *&new_values, _int64 offset);
  template <ValueType VT>
  void AssignToAll(_int64 value, void *&new_data_full);
  template <typename T>
  void InitValues(T value, T *&values);
  static ValueType ChooseValueType(int bit_rate);
  template <typename etype>
  void DecompressAndInsertNulls(NumCompressor<etype> &nc, uint *&cur_buf);
  template <typename etype>
  void RemoveNullsAndCompress(NumCompressor<etype> &nc, char *tmp_comp_buffer, uint &tmp_cb_len, _uint64 &maxv);
...
class AttrPackS : public AttrPack
{
 public:
  ...
  void SetDecomposerID(uint decomposer_id)
  {
    this->decomposer_id = decomposer_id;
    use_already_set_decomposer = true;
    ver = decomposer_id != 0 ? 8 : 0;
  }
 ...
 private:
  ...
  void Decompose(uint &size, std::vector<uchar **> &indexes, std::vector<ushort *> &lengths, std::vector<uint> &maxes,
                 std::vector<uint> &sumlens);
};
