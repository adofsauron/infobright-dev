...
class TransactionBase
{
 ...
 public:
  ...
  TransactionBase(RCEngine &r, GlobalDataCache *d) : m_rcengine(r), m_explicit_lock_tables(false), m_globalCache(d)
  {
  ...
  // boost::shared_ptr<Splice<DPN, DEFAULT_SPLICE_SIZE> > GetSplice(const SpliceCoordinate& coord,
  // cached_dpn_splice_allocator const& a); // function used in on-demand splice loading
  ...
  virtual RCTablePtr RefreshTable(std::string const &table_path, uint packrow_size = MAX_PACK_ROW_SIZE,
                                  bool fill_up_last_packrow = false) = 0;

  bool IsTableModified(const std::string &table_path) const
  {
    return (m_modified_tables.find(table_path) != m_modified_tables.end());
  }
  ...
  virtual void ApplyPendingChanges(THD *thd, const std::string &table_path, struct st_table_share *table_share,
                                   RCTable *tab) = 0;
