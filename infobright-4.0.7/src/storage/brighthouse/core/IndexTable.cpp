    ...
    : CacheableItem("JW", "INT"), m_conn(ConnectionInfoOnTLS.Get())
{
  ...
  block_shift =
      CalculateBinSize(values_per_block) - 1;  // e.g. BinSize(16)==5, but shift by 4. WARNING: should it be (val...-1)?
                                               // Now it works, because v... is only 2^25, 2^24, 2^23
...
void IndexTable::ExpandTo(_int64 new_size)
{
  ...
  if (new_size <= (_int64)size)
    return;
  if (size * bytes_per_value == _int64(buffer_size_in_bytes))
  {  // the whole table was in one buffer
    if (buffer_size_in_bytes < 32 * MBYTE && new_size * bytes_per_value > 32 * MBYTE)
    {
      max_buffer_size_in_bytes =
          int(TrackableObject::MaxBufferSize(-1));       // recalculate, as it might not be done earlier
