...
RCTableImpl::RCTableImpl(string const &a_path, vector<DTCollation> cs, int current_state,
                         OpenMode::open_mode_t openMode_)
    : waiting_for_drop(0), db_name(), path(), path_no_ext(), charsets(cs), m_state(current_state), obsolete(false)
{
  ...
  try
  {
    ...
    while (block_offset > 0)
    {
      ...
      {
        rclog << lock << "Warning: special block (unknown type " << int(b_type) << ") ignored in table " << name
              << unlock;
      }
      if (b_next == 0)
      {
  ...
  for (int i(0); i < no_attr; ++i) a[i] = NULL;
...
RCTableImpl::~RCTableImpl() { CleanUp(); }
...
{
  for (int attr = 0; attr < no_attr; attr++) LoadAttribute(attr);
...
{
  for (std::set<int>::const_iterator it = columns.begin(); it != columns.end(); ++it) a[*it]->UnlockLastPackFromUse();
}
...
void RCTableImpl::UnlockLastPacksForOthers(const std::set<int> &columns)
{
  ...
  for (std::set<int>::const_iterator it = columns.begin(); it != columns.end(); ++it)
  {
    for (int attr = prev + 1; attr < *it; attr++) a[attr]->UnlockLastPackFromUse();
    prev = *it;
  }
  for (int attr = prev + 1; attr < no_attr; attr++) a[attr]->UnlockLastPackFromUse();
}

void RCTableImpl::AddAttribute(char *nm, AttributeType a_type, int a_field_size, int a_dec_places /*= 0*/,
                               unsigned int param /*= 0*/, DTCollation collation /*= DTCollation()*/)
{
  no_attr++;
  RCAttr **a_new = new RCAttr *[no_attr];  // warning C6211: Leaking memory 'a_new' due to an exception.
  for (int i = 0; i < no_attr - 1; i++) a_new[i] = a[i];
  delete[] a;
  a = a_new;
  a[no_attr - 1] =
      new RCAttr(nm, no_attr - 1, tab_num, a_type, a_field_size, a_dec_places, path, param, session, collation);
}
...
std::vector<ATI> RCTableImpl::GetATIs(bool orig)
{
  ...
  {
    LoadAttribute(j);
    deas += ATI(a[j]->TypeName(), a[j]->Type().GetNullsMode() == NO_NULLS, a[j]->Type().GetPrecision(),
                a[j]->Type().GetScale(), a[j]->Type().IsLookup(), false, a[j]->Type().GetCollation());
  }
  return deas;
}
...
int RCTableImpl::Verify()
{
  ...
  if (res == 0)
  {
    ...
    for (i = 0; i < no_attr; i++)
      if (a[i])
      {
        rclog << "  - " << a[i]->Name() << " \t no.obj.= " << a[i]->NoObj() << " \t sess.id= " << a[i]->GetSessionId()
              << endl;
      }
    rclog << unlock;
  }
...
uint RCTableImpl::CommitSaveSession(uint s_id)
{
  ...
  for (int i = 0; i < no_attr; i++)
  {
    ...
    if (a[i]->NoObj() != 0 && a[i]->NoObj() != (((int64)a[i]->NoPack() - 1) * MAX_PACK_ROW_SIZE +
                                                a[i]->GetNoValues(a[i]->NoPack() - a[i]->packs_omitted - 1)))
    {
      stringstream msg;
...
void RCTableImpl::DisplayRSI()
{
  ...
  for (int i = 0; i < no_attr; i++)
  {
    ...
    if (a[i])
    {
      ...
      for (int j = 0; j < display_limit; j++) rccontrol << " ";
      rccontrol << trivials << "\t\t";
      if (span != -1)
...
bool RCTableImpl::Iterator::operator==(const Iterator &iter) { return position == iter.position; }
...
void RCTableImpl::Iterator::FetchValues()
{
  ...
  {
    LockPacks();
    BOOST_FOREACH (values_fetchers_t::value_type &func, values_fetchers) func(position);
...
RCTableImpl::Iterator RCTableImpl::Iterator::CreateBegin(RCTableImpl &table, FilterPtr filter,
...
RCTableImpl::Iterator RCTableImpl::Iterator::CreateEnd() { return RCTableImpl::Iterator(); }
