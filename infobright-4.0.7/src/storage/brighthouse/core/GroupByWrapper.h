...
class GroupByWrapper
{
 public:
  ...
  void AddAggregatedColumn(int orig_no, TempTable::Attr &a, _int64 max_no_vals = 0, _int64 min_v = MINUS_INF_64,
                           _int64 max_v = PLUS_INF_64, ushort max_size = 0);
  ...
  void AddAllCountStar(_int64 row, MIIterator &mit,
                       _int64 val);  // set all count(*) values to val, or to 0 in case of nulls
  ...
  bool IsCountOnly(int a = -1);  // true, if an attribute is count(*)/count(const), or if there is no columns except
                                 // constants and count (when no attr specified)
  ...
  void TuplesResetBetween(_int64 from, _int64 to)
  {
    if (tuple_left)
      tuple_left->ResetBetween(from, to);
  }
  void TuplesReset(_int64 pos)
  {
    if (tuple_left)
