...
_int64 SafeMultiplication(_int64 x,
                          _int64 y)  // return a multiplication of two numbers or NULL_VALUE_64 in case of overflow
{
  if (x == NULL_VALUE_64 || y == NULL_VALUE_64 ||
      ((x > 0x7FFFFFFF || y > 0x7FFFFFFF) && CalculateBinSize(_uint64(x)) + CalculateBinSize(_uint64(y)) > 63))
...
_int64 MonotonicDouble2Int64(
    _int64 d)  // encode double value (bitwise stored as _int64) into _int64 to prevent comparison directions
{
#if !defined(__sun__)
...
uint HashValue(const char unsigned *data, int len)  // a simple hash function similar to the one used in boost::hash
{
  ...
  for (; i < len; i++)
    val ^= uint(data[i]) + 0x9e3779b9 + (val << 10) +
           (val >> 2);               // in boost... there is "<< 6" here, but it lead to too many collisions
