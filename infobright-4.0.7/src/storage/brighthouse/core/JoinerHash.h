...
class JoinerHash : public TwoDimensionalJoiner
{
 ...
 private:
  _int64 TraverseDim(MINewContents &new_mind, MIIterator &mit,
                     _int64 &outer_tuples);  // new_mind is used only for outer joins
  ...
  _int64 actually_traversed_rows;  // "traversed" side rows, which had a chance to be in the result (for VC distinct
                                   // values)
