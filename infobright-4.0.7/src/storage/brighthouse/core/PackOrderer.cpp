...
PackOrderer::PackOrderer(VirtualColumn *vc, RSValue *r_filter, OrderType order) { Init(vc, order, r_filter); }
...
bool PackOrderer::Init(vector<VirtualColumn *> vcs, vector<OrderType> orders, int no_unsorted_cols, bool one_group)
{
  ...
  if (one_group)
  {
    if (vcs.size() == 1 && no_unsorted_cols == 0 && orders[0] == Covering)
    {  // one column only
  ...
  for (int i = 0; i < vcs.size(); i++)
  {
    ...
    if (i > 0)
      BHASSERT_WITH_NO_PERFORMANCE_IMPACT(natural_order[i] || natural_order[i - 1] ||
                                          packs[i].size() == packs[i - 1].size());
  }
  return true;
}
...
void PackOrderer::RewindToMatch(VirtualColumn *vc, MIIterator &mit)
{
  ...
  if (mmtype[curvc] == MMT_Fixed)
  {
    ...
    vector<PackPair>::iterator it =
        lower_bound(packs[curvc].begin(), packs[curvc].end(), make_pair(mid, 0), comp_int_less);
...
{
  for (curvc = 0; curvc < ncols; curvc++) RewindToCurrentCol();
  if (visited.get())
    visited->Reset();
}
...
/*!
 * Based on the given set of virtual columns (all from the same dimension) with information how they will be used (e.g.
 * calculate MIN) this function should find the best ordering of packrows for this dimension.
 *
