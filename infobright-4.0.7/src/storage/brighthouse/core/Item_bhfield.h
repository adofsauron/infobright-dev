...
class Item_bhfield : public Item_field
{
 public:
  std::vector<VarID>
      varID;         // BH identifiers of the variable/field/column represented by 'ifield' for each bhfield usage
  ...
  {
    BHFIELD_ITEM = 12345
  };  // WARNING: it is risky. We are assuming sizeof(enum) >= 2 in a superclass (Item)
  ...
  virtual mysql_bool is_null_result() { return buf->null; }
  ...
  enum Item_result result_type() const { return was_aggregation ? aggregation_result : Item_field::result_type(); }
  ...
  bool result_as_longlong() { return ifield->result_as_longlong(); }
