...
FilterOnesIterator::FilterOnesIterator(Filter *ff) : buffer(max_ahead) { Init(ff); }
...
void FilterOnesIterator::Init(Filter *ff)  //!< reinitialize iterator (possibly with the new filter)
{
  ...
  inside_one_pack = true;
  while (iterator_b < f->no_blocks)
  {  // check whether there is more than one pack involved
...
{
  int pack = int(row >> 16);
  if (pack >= f->no_blocks || (pack == f->no_blocks - 1 && (row & 0xffff) >= f->no_of_bits_in_last_block))
    valid = false;
  else
    valid = true;
...
_int64 FilterOnesIterator::GetPackSizeLeft()  // how many 1-s in the current block left (including the current one)
{
  if (!valid)
    return 0;
  if (cur_block_full)
    return _int64(f->block_last_one[iterator_b]) + 1 - iterator_n;
...
bool FilterOnesIterator::IteratorBpp()
{
  ...
  {
    iterator_b++;
    while (iterator_b < f->no_blocks && f->block_status[iterator_b] == f->FB_EMPTY) iterator_b++;
...
bool FilterOnesIterator::NextInsidePack()  // return false if we just restarted the pack after going to its end
{
  ...
  while (!found)
  {
    ...
    else if (cur_block_empty)
    {  // Updating iterator can cause this
      return false;
    }
    else
    {
...
FilterOnesIteratorOrdered::FilterOnesIteratorOrdered(Filter *ff, PackOrderer *po) { Init(ff, po); }
