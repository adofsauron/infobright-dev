...
class RCTableImpl : public JustATable, public boost::noncopyable
{
 ...
 public:
  ...
  RCTableImpl(std::string const &path, std::vector<DTCollation> charsets, int current_state,
              OpenMode::open_mode_t = OpenMode::DEFAULT);
  ...
  void AddAttribute(char *nm, AttributeType a_type, int a_field_size, int a_dec_places = 0, unsigned int param = 0,
  ...
  _int64 RoughMin(int n_a, Filter *f = NULL);  // for numerical: best rough approximation of min for a given filter (or
                                               // global min if filter is NULL)
  _int64 RoughMax(int n_a, Filter *f = NULL);  // for numerical: best rough approximation of max for a given filter (or
 ...
 public:
  ...
  {
    return Iterator::CreateBegin(*this, FilterPtr(new Filter(filter)), attrs);
  }
  virtual Iterator Begin(const std::vector<bool> &attrs)
  {
    return Iterator::CreateBegin(*this, FilterPtr(new Filter(NoObj(), true)), attrs);
  }
