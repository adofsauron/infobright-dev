  ...
  for (uint k = 0; k < descriptors.Size(); k++)
  {
    for (uint i = 0; i < descriptors.Size() - 1; i++)
    {
      ...
      if (descriptors[i].evaluation > descriptors[j].evaluation &&
          descriptors[j].right_dims == descriptors[i].right_dims)
      {  // do not change outer join order, if incompatible
...

{
  ...
  do
  {
    TwoDimsJoinerAutoPtr joiner = TwoDimensionalJoiner::CreateJoiner(join_alg, *mind, *rough_mind, tips, table);
                                             ...
                                             bool is_outer, int conditions_used)
{
  if (rccontrol.isOn())
  {
    ...
    rccontrol.lock(mind->m_conn->GetThreadID())
        ...
        << tuples_after_join << " \t" << mind->Display() << unlock;
  }
}
