...
int Query::Item2CQTerm(Item *an_arg, CQTerm &term, const TabID &tmp_table, CondType filter_type, bool negative,
                       Item *left_expr_for_subselect, Operator *oper_for_subselect)
{
#ifdef PURE_LIBRARY
  ...
  if (filter_type == HAVING_COND)
  {
    ...
    if (!OperationUnmysterify(an_arg, oper, distinct,
                              true))  // is_having_clause may be true only in case group by clause was present
      return RETURN_QUERY_TO_MYSQL_ROUTE;
                                                  ...
                                                  CondID *and_me_filter, bool is_or_subtree)
{
  ...
  CondID filter;
  if (!and_me_filter)
    cq->CreateConds(filter, tmp_table, first_term, O_EQ, zero_term, CQTerm(),
  ...
  for (uint i = 0; i < cond_func->arg_count; i++)
  {
    ...
    else
    {
            ...
            an_arg->field_type() == MYSQL_TYPE_VAR_STRING || an_arg->field_type() == MYSQL_TYPE_BLOB))
      {
        my_message(
            ER_SYNTAX_ERROR,
            "Brighthouse specific error: Both operator and operand must be string data types for LIKE comparison",
            MYF(0));
...
CondID Query::ConditionNumberFromNaked(COND *conds, const TabID &tmp_table, CondType filter_type, CondID *and_me_filter,
                                       bool is_or_subtree)
{
#ifdef PURE_LIBRARY
  BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
...
CondID Query::ConditionNumber(COND *conds, const TabID &tmp_table, CondType filter_type, CondID *and_me_filter,
                              bool is_or_subtree)
{
#ifdef PURE_LIBRARY
  BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
      ...
      case Item_func::COND_OR_FUNC:
      {
        ...
        for (it = value_map.begin(); it != value_map.end(); ++it, --i)
        {
          try
          {
            ...
            if (vcv.size() > 1)
            {
              ...
              if (!or_cond)
              {
                if (and_me_filter && !create_or_subtree)
                {
                  ...
                  cq->CreateConds(c_id, tmp_table, terms[0], O_IN, terms[1], CQTerm(),
            ...
            else
            {
              ...
              if (!or_cond)
              {
                if (and_me_filter && !create_or_subtree)
                {
                  ...
                  cq->CreateConds(c_id, tmp_table, terms[0], O_EQ, terms[1], CQTerm(),
                                  create_or_subtree || filter_type == HAVING_COND);
                or_cond = new CondID(c_id.n);
              }
              else
            ...
            if (c_id.IsInvalid())
            {  // to deal with deleting dynamical variables (new_and_me_filter) properly
              // in case of failure
              delete or_cond;
              return c_id;
            }
          ...
          {  // to deal with deleting dynamical variables (new_and_me_filter) properly in case of an exception
            delete or_cond;
            throw;
          }
        }
  ...
  else if (cond_type == Item::FUNC_ITEM)
  {
        ...
        ((Item_subselect *)arg)->substype() == Item_subselect::EXISTS_SUBS)
    {
      ...
      if (!Item2CQTerm(arg, term, tmp_table, filter_type))
        return CondID(-1);
      if (!and_me_filter)
        cq->CreateConds(cond_id, tmp_table, term, O_NOT_EXISTS, CQTerm(), CQTerm(),
                        is_or_subtree || filter_type == HAVING_COND);
      else
      {
        if (is_or_subtree)
  ...
  else if (cond_type == Item::SUBSELECT_ITEM && ((Item_subselect *)conds)->substype() == Item_subselect::EXISTS_SUBS)
  {
    ...
    if (!and_me_filter)
    {
      cq->CreateConds(cond_id, tmp_table, term, O_EXISTS, CQTerm(), CQTerm(),
                      is_or_subtree || filter_type == HAVING_COND);
    }
    else
    {
...
bool Query::ClearSubselectTransformation(Operator &oper_for_subselect, Item *&field_for_subselect, COND *&conds,
                                         ...
                                         Item *left_expr_for_subselect)
{
         ...
         ((Item_func *)((Item_func *)having)->arguments()[0])->functype() != Item_func::ISNOTNULLTEST_FUNC))))
  {
    ...
    if (cond_removed->type() == Item::FUNC_ITEM && ((Item_func *)cond_removed)->functype() == Item_func::TRIG_COND_FUNC)
    {
