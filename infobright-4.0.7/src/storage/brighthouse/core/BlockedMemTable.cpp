...
MemBlockManager::MemBlockManager() : size_limit(-1), no_threads(1), block_size(-1), hard_size_limit(-1), current_size(0)
{
}

MemBlockManager::MemBlockManager(_int64 mem_limit, int no_th, int b_size, _int64 mem_hard_limit) : size_limit(-1)
{
  Init(mem_limit, no_th, b_size, mem_hard_limit);
}

MemBlockManager::MemBlockManager(const MemBlockManager &mbm)
    : block_size(mbm.block_size),
      size_limit(mbm.size_limit),
      hard_size_limit(mbm.hard_size_limit),
      no_threads(mbm.no_threads),
      current_size(0)
{
  // the copy must be empty, because the copied block would be unavailable externally
}

bool MemBlockManager::Init(_int64 mem_limit, int no_th, int b_size, _int64 mem_hard_limit)
{
  IBGuard g(mx);
  if (size_limit != -1)
    return false;
...
{
  for (int i = 0; i < free_blocks.size(); i++) dealloc(free_blocks[i]);
  //	assert(used_blocks.size() == 0);
  for (std::set<void *>::const_iterator it = used_blocks.begin(); it != used_blocks.end(); ++it) dealloc(*it);
}
...
int MemBlockManager::MemoryBlocksLeft()
{
  ...
  s = free_blocks.size();
  mx.Unlock();
  _int64 size_in_near_future = current_size + block_size * ((no_threads >> 1) - s);
...
BlockedRowMemStorage::BlockedRowMemStorage() : no_rows(0), no_blocks(0), current(-1), private_bman(false) {}

BlockedRowMemStorage::BlockedRowMemStorage(int row_len, MemBlockManager *mbm, _uint64 initial_size, int min_block_len)
{
...
void BlockedRowMemStorage::Init(int rowl, MemBlockManager *mbm, _uint64 initial_size, int min_block_len)
{
  ...
  if (initial_size > 0)
    for (int i = 0; i < (initial_size / rows_in_block) + 1; i++)
    {
      ...
      if (!b)
        throw OutOfMemoryRCException("too large initial BlockedRowMemStorage size");
      blocks.push_back(b);
      no_blocks++;
    }
...
{
  for (int i = 0; i < no_blocks; i++) bman->FreeBlock(blocks[i]);  // blocks[i] may be NULL already
  blocks.clear();
  no_blocks = 0;
  no_rows = 0;
  release = false;
...
BlockedRowMemStorage::BlockedRowMemStorage(const BlockedRowMemStorage &bs)
    : row_len(bs.row_len),
      ...
      no_rows(bs.no_rows),
      no_blocks(bs.no_blocks),
      current(bs.current),
      release(bs.release),
      released_until(bs.released_until),
      private_bman(bs.private_bman)
{
  if (private_bman)
  {
...
_int64 BlockedRowMemStorage::AddRow(const void *r)
{
  ...
  memcpy(((char *)blocks[no_blocks - 1]) + (no_rows & ndx_mask) * row_len, r, row_len);
...
_int64 BlockedRowMemStorage::AddEmptyRow()
{
  ...
  memset(((char *)blocks[no_blocks - 1]) + (no_rows & ndx_mask) * row_len, 0, row_len);
