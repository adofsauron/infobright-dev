...
#define PM_TRIVIAL \
  0                            //  0 - trivial data: all values are derivable from the statistics, or nulls only,
...
#define FILEFORMAT(buf_format)                                                                    \
  ((!strncmp(buf_format, CLMD_FORMAT_RSC9, CLMD_FORMAT_BUF_LEN))                                  \
       ? (CLMD_FORMAT_RSC9_ID)                                                                    \
       : ((!strncmp(buf_format, CLMD_FORMAT_RSC10, CLMD_FORMAT_BUF_LEN)) ? (CLMD_FORMAT_RSC10_ID) \
                                                                         : (CLMD_FORMAT_INVALID_ID)))
};  // namespace bh
/******************************************
 * ABSwitch implementation
 ******************************************/
typedef enum ABSwtich_tag
{
  ABS_A_STATE,
  ABS_B_STATE
} ABSwitch;
...
class RetVal
{
 public:
  ...
  RetVal(uint no_obj = 0, uint no_nulls = 0) : no_obj(no_obj), no_nulls(no_nulls) {}
};
...
class RCAttr : public TrackableObject, public PhysicalColumn
{
 ...
 public:
  ...
  RCAttr(char *a_name, int a_num, int t_num, AttributeType a_type, int a_field_size, int a_dec_places,
         std::string const &a_path, unsigned int param, unsigned int s_id = 0, DTCollation collation = DTCollation());

  // Connect to an attribute (on disk); Connection mode: 0/1 - read only (queries),
  // write session (s_id - Session Identifier)
  RCAttr(int a_num, int t_num, std::string const &a_path, int conn_mode = 0, unsigned int s_id = 0,
         DTCollation collation = DTCollation()) throw(DatabaseRCException);

  // Create a remote attribute
  RCAttr(int a_num, AttributeType a_type, int a_field_size, int a_dec_places, uint param, DTCollation collation,
         bool compress_lookups, std::string const &path_);
  ...
  void LockPackForUse(unsigned pack_no, Transaction &trans,
                      boost::function0<void> notify_pack_load = boost::function0<void>());
  ...
  RCValueObject GetValue(_int64 obj, bool lookup_to_num = false);  // should be removed to get rid of RCValueObject
                                                                   // class
  ...
  int EncodeValue_T(const RCBString &rcbs, int new_val = 0,
                    BHReturnCode *bhrc = 0);  // 1-level code value for a given 0-level (text) value, if new_val=1 then
                                              // add to dictionary if not present
  // no changes for REAL; rounded=true iff v has greater precision than the column and the returned result is rounded
  // down
  ...
  _uint64 ApproxDistinctVals(bool incl_nulls, Filter *f, RSValue *rf,
                             bool outer_nulls_possible);  // provide the best upper approximation of number of diff.
                                                          // values (incl. null, if flag set)
  _uint64 ExactDistinctVals(
      Filter *f);  // provide the exact number of diff. non-null values, if possible, or NULL_VALUE_64
  _uint64 ApproxAnswerSize(
      Descriptor &d);  // provide the most probable approximation of number of objects matching the condition
  ushort MaxStringSize(Filter *f = NULL);  // maximal byte string length in column
  bool IsDistinct(Filter *f);
  _int64 RoughMin(Filter *f, RSValue *rf = NULL);  // for numerical: best rough approximation of min for a given filter
                                                   // (or global min if filter is NULL) or rough filter
  _int64 RoughMax(Filter *f, RSValue *rf = NULL);  // for numerical: best rough approximation of max for a given filter
                                                   // (or global max if filter is NULL) or rough filter

  // Rough queries and indexes
  // Note that you should release all indexes after using a series of RoughChecks!
  RSValue RoughCheck(int pack, Descriptor &d,
                     bool additional_nulls_possible);  // check whether any value from the pack may meet the condition
  RSValue RoughCheck(
      int pack1, int pack2,
      Descriptor &d);  // check whether any pair from two packs of two different attr/tables may meet the condition
  RSValue RoughCheckBetween(
      int pack, _int64 min,
      _int64 max);  // check whether any value from the pack may meet the condition "... BETWEEN min AND max"

  void RoughStats(double &hist_density, int &trivial_packs,
                  double &span);     // calculate the number of 1's in histograms and other KN stats
  ...
  // DPN::local_min => Statistics (in 1-level codes) for data packs (NOTE: these two 64-bit values are used also by TEXT
  // and RC_BIN, as a kind of optimization/indexing) DPN::local_max => 2-level values start at 0 (or 1, if 0 is null
  // value) level_1_value = local_min + level_2_value + (-1 if nulls present)
  ...
  // DPN::no_nulls =>	// for every pack, number of nulls (cannot be short, because it may be between 0 and 65536
  // (including!)). DPN::pack_addr => // Address (byte) of the beginning of i-th pack in the file DPN::pack_file => //
  // Number of file where i-th pack is located. DPN::pack => Packs itself (when loaded). Encoded internally as 2-level
  // codes
  double
      rough_selectivity;  // a probability that simple condition "c = 100" needs to open a data pack, providing KNs etc.
  ...
  inline void LoadPackInfo(Transaction &trans)
  {
    if (pack_info_collapsed)
      LoadPackInfo_physical(&trans);
  }
  inline void LoadPackInfo()
  {
    if (pack_info_collapsed)
      LoadPackInfo_physical(GetCurrentTransaction());
  }
 ...
 private:
  ...
  _int64 ComputeCompressedSize();  // the compressed size of the attribute (for e.g. calculating compression ratio);
                                   // NOTE: may be slightly approximated
 ...
 private:
  ...
  inline _int64 GetTable64Loaded(uint pack, int n)  // low-level, fast: no pack loading etc.
  {
    assert(pack_type == PackN && dpns[pack].pack_file != PF_NULLS_ONLY &&
           (dpns[pack].no_nulls != 0 || dpns[pack].local_min != dpns[pack].local_max) &&
           dpns[pack].pack_mode == PACK_MODE_IN_MEMORY);
...
class DataPackLock : public FunctionExecutor
{
 public:
  DataPackLock(RCAttr &attr, const DataPackId &id)
      : FunctionExecutor(boost::bind(&RCAttr::LockPackForUse, boost::ref(attr), (int)id),
