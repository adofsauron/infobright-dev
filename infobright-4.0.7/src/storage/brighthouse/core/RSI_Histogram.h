...
class RSIndex_Hist : public RSIndex
{
 public:
  ...
  bool UpToDate(_int64 cur_no_obj, int pack);  // true, if this pack is up to date (i.e. either cur_no_obj==no_obj, or
                                               // pack is not the last pack in the index)

  // creation, update

  void Create(_int64 _no_obj, bool _fixed);  // create a new histogram based on _no_obj objects; _fixed is true for
                                             // fixed precision, false for floating point
  ...
  void PutValue(_int64 v, int pack, _int64 pack_min,
  ...
  bool Intersection(int pack, _int64 pack_min, _int64 pack_max, RSIndex_Hist *sec, int pack2, _int64 pack_min2,
                    _int64 pack_max2);
  ...
  bool ExactMode(_int64 pack_min, _int64 pack_max)
  {
    return (_uint64(pack_max - pack_min) <= (32 * RSI_HIST_INT_RES - 2));
  }
  // if true, then the histogram provides exact information, not just interval-based

  // Loading/saving

  void Load(IBFile *frs_index, int current_loc);  // fd - file descriptor, open for binary reading (the file can be
                                                  // newly created and empty - this shouldn't cause an error)
  ...
  void AppendKN(int pack, RSIndex_Hist *,
                uint no_values_to_add);  // append a given KN to KN list, extending the number of objects
 ...
 protected:
  char *GetRepresentation(int pack)
  {
    BHASSERT(pack <= end_pack, "Invalid pack request");
    return (char *)(PackBuf(pack));
  }

 private:
  uint BlockCount(uint64 no_pack) const { return (uint)((no_pack + (NO_HISTS_IN_BLOCK - 1)) / NO_HISTS_IN_BLOCK); }
  uint NoPacksInLastBlock(uint64 no_pack) const
  {
    return (uint)(no_pack % NO_HISTS_IN_BLOCK == 0 ? NO_HISTS_IN_BLOCK : no_pack % NO_HISTS_IN_BLOCK);
  }
  ...
  {
    return (pack_min < -4611686018427387900LL && pack_max > 4611686018427387900LL);
  }  // about 2^62
 ...
 private:
  ...
  // int		int_res;			// number of ints to describe one pack (default: 32 ints = 1024
  // bits)
