...
void GroupDistinctTable::InitializeVC(_int64 max_no_groups, VirtualColumn *vc, _int64 max_no_rows, _int64 max_bytes,
                                      bool decodable)
{
  ...
  if (encoder->MaxCode() != NULL_VALUE_64 && encoder->GetPrimarySize() <= 4 && max_no_groups < 1000000000)
  {
    _int64 filter_positions = max_no_groups * (encoder->MaxCode() + 1);
    if (filter_positions / 8 < max_total_size &&  // general memory limitation
        filter_positions / 8 < 4 * max_no_rows)
    {  // compare with a good case hash implementation
...
void GroupDistinctTable::InitializeBuffers(_int64 max_no_rows)  // max_no_rows = 0   =>  limit not known
{
  ...
  {  // special case of filter
    ...
    rccontrol.lock(m_conn->GetThreadID())
        << "GroupDistinctTable initialized as Filter for up to " << max_no_rows << " positions." << unlock;
    return;
  }
