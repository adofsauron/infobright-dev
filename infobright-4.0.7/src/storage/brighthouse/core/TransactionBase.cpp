...
TransactionBase::TableView::TableView(RCTablePtr tab)
    : table(tab),
      initial_no_packs(bh::common::NoObj2NoPacks(tab->NoObj())),
      _outliers(table->NoAttrs(), TransactionBase::NO_DECOMPOSITION),
      update_outliers_diff(table->NoAttrs(), INT_MIN),
      update_delete(false)
{
}

TransactionBase::~TransactionBase() { m_modified_tables.clear(); }
...
AttrPackPtr TransactionBase::GetAttrPack(const PackCoordinate &pack_coord, PackAllocator &a)
{
  AttrPackPtr ret;
  {
...
AttrPackPtr TransactionBase::GetAttrPackForUpdate(const PackCoordinate &pack_coord)
{
  ...
  {
    if (ret.use_count() > 2)
    {  // means somebody else (outside this scope) has a reference to this object. 1 ref count in cache + 1 ret
      AttrPackPtr tmp(ret);
      ret = tmp->Clone();
      tmp->Unlock();
...
boost::shared_ptr<Splice<DPN, DEFAULT_SPLICE_SIZE>> TransactionBase::GetSplice(const SpliceCoordinate &coord)
{
  boost::shared_ptr<Splice<DPN, DEFAULT_SPLICE_SIZE>> ret =
      m_localCache.GetObject<Splice<DPN, DEFAULT_SPLICE_SIZE>>(coord);
...
TransactionBase::GetSplice(const SpliceCoordinate& coord, cached_dpn_splice_allocator const& a)
{
        ...
        if ( !!GetModifiedTableView(coord[0]).table && for_local_cache( initial_no_packs, coord[2] ) ) {
                ret = m_localCache.GetObject<Splice<DPN, DEFAULT_SPLICE_SIZE> >(coord,
bind(&cached_dpn_splice_allocator::fetch, ref(a), coord[2])); } else { ret = m_globalCache->GetObject<Splice<DPN,
DEFAULT_SPLICE_SIZE> >(coord, bind(&cached_dpn_splice_allocator::fetch, ref(a), coord[2]));
        }

        return ret;
}*/

boost::shared_ptr<Splice<DPN, DEFAULT_SPLICE_SIZE>> TransactionBase::GetSpliceForUpdate(
    const SpliceCoordinate &splice_coord)
{
  boost::shared_ptr<Splice<DPN, DEFAULT_SPLICE_SIZE>> ret;
...
boost::shared_ptr<FTree> TransactionBase::GetFTreeForUpdate(const FTreeCoordinate &ftree_coord)
{
...
void TransactionBase::ReleasePackRow(int table, int pack_row) { m_localCache.ReleasePackRow(table, pack_row); }
...
/*void TransactionBase::SetOutliers( std::string const& table_, int column_, int64 outliers_ )
{
        ...
        BHASSERT( ( column_ >= 0 ) && ( column_ < it->second.table->NoAttrs() ), "bad column index while setting
outliers" ); BHASSERT( outliers_ >= TransactionBase::NO_DECOMPOSITION, "bad number of outliers" );
...
void TransactionBase::AddOutliers(int table_, int column_, int64 outliers_)
{
  ...
  while (it != end)
  {
    ...
    {
      BHASSERT((column_ >= 0) && (column_ < int(it->second.table->NoAttrs())),
               "bad column index while setting outliers");
...
{
  int col = 0;
  BOOST_FOREACH (int64 val, outliers_) AddOutliers(table_, col++, val);
}
...
/*int long TransactionBase::GetOutliers( std::string const& table_, int column_ )
{
        ...
        BHASSERT( ( column_ >= 0 ) && ( column_ < it->second.table->NoAttrs() ), "bad column index while getting
outliers" ); return ( it->second._outliers[ column_ ] );
}*/
