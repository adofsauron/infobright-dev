...
class PackOrderer
{
 public:
  enum OrderType
  {
    ...
    Covering,  // start with packs which quickly covers the whole attribute domain (MinAsc, then find the next minimally
               // overlapping pack)
    NotSpecified
  };
  enum State
  ...
  bool NaturallyOrdered()
  {
    return packs_passed >= packs_ordered_up_to;
 ...
 private:
  ...
  {
   public:
    inline bool operator()(const PackPair &v1, const PackPair &v2) const
    {
