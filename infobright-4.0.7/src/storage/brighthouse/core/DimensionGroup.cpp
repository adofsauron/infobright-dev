...
DimensionVector::DimensionVector(const DimensionVector &sec) : v(NULL)
{
  ...
  if (size > 0)
    v = new bool[size];
  for (int i = 0; i < size; i++) v[i] = sec.v[i];
}

DimensionVector::~DimensionVector() { delete[] v; }
...
void DimensionVector::Resize(int no_dims)
{
  ...
  if (no_dims > 0)
  {
    ...
    for (int i = 0; i < size; i++) new_v[i] = v[i];
    for (int i = size; i < no_dims; i++) new_v[i] = false;
    delete[] v;
    v = new_v;
    size = no_dims;
  }
...
DimensionGroupFilter::DimensionGroupFilter(
    int dim, Filter *f_source, int copy_mode)  // copy_mode: 0 - copy filter, 1 - ShallowCopy filter, 2 - grab pointer
{
  base_dim = dim;
  f = NULL;
  if (copy_mode == 0)
...
void DimensionGroupMaterialized::NewDimensionContent(
...
void DimensionGroupMaterialized::FillCurrentPos(DimensionGroup::Iterator *it, _int64 *cur_pos, int *cur_pack,
...
DimensionGroupMaterialized::DGMaterializedIterator::DGMaterializedIterator(_int64 _no_obj, DimensionVector &dims,
...
DimensionGroupMaterialized::DGMaterializedIterator::DGMaterializedIterator(const Iterator &sec)
    : DimensionGroup::Iterator(sec)
{
  ...
  else
  {
    ...
    if (loc_iterator < loc_limit)
    {
      ...
      while (loc_iterator < loc_limit &&  // find the first non-NULL row from another pack (but the same block)
             ((ndx = cur_t->Get64InsideBlock(loc_iterator)) == 0 || ((ndx - 1) >> 16) == loc_pack))
      {
        if (ndx == 0)
          nulls_found[dim] = true;
        ++loc_iterator;
...
int DimensionGroupMaterialized::DGMaterializedIterator::GetNextPackrow(int dim, int ahead)
{
  ...
  if (next_pack[dim] >= no_obj || _uint64(next_pack[dim]) >= end_block)
    return -1;
  _uint64 ahead_pos = 0;
  //	cout << "dim " << dim << ",  " << next_pack[dim] << " -> " << ahead1[dim] << "  " << ahead2[dim] << "  " <<
  // ahead3[dim] << "    (" << ahead << ")" << endl;
  if (ahead == 1)
...
bool DimensionGroupMaterialized::DGMaterializedIterator::BarrierAfterPackrow()
{
  ...
  if (next_pack_start >= no_obj)  // important e.g. for case when we're restarting the same dimension group (iterating
                                  // on a product of many groups)
    return true;
  for (int i = 0; i < no_dims; i++)
