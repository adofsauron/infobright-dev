...
{
  return desc->encoded == true && !(encoding_done || desc->op == O_IS_NULL || desc->op == O_NOT_NULL ||
...
void ConditionEncoder::DescriptorTransformation()
{
  ...
  else if (desc->op == O_BETWEEN && (desc->val1.IsNull() || (desc->val1.vc->IsConst() && desc->val1.vc->IsNull(mit)) ||
                                     desc->val2.IsNull() || (desc->val2.vc->IsConst() && desc->val2.vc->IsNull(mit))))
  {
    desc->op = O_FALSE;
    desc->null_after_simplify = true;
  }
  else if (desc->op == O_NOT_BETWEEN)
  {  // O_NOT_BETWEEN may be changed to inequalities in case of nulls
    ...
    {  // a NOT BETWEEN null AND 5  <=>  a > 5
      desc->val1 = desc->val2;
      desc->val2 = CQTerm();
      desc->op = O_MORE;
    }
    else if (second_null)
    {  // a NOT BETWEEN 5 AND null  <=>  a < 5
      desc->op = O_LESS;
    }
  }
...
void ConditionEncoder::TransformWithRespectToNulls()
{
  ...
  if ((IsSetAllOperator(desc->op) || desc->op == O_NOT_IN) && desc->val1.vc && desc->val1.vc->IsMultival())
  {
    MultiValColumn *mvc = static_cast<MultiValColumn *>(desc->val1.vc);
    MIIterator mit(0);
    // Change to O_FALSE for non-subselect columns, or non-correlated subselects (otherwise ContainsNulls needs feeding
    // arguments)
...
void ConditionEncoder::PrepareValueSet(MultiValColumn &mvc) { mvc.SetExpectedType(attr->Type()); }
...
void ConditionEncoder::TransformOtherThanINsOnNumerics()
{
  ...
  if (v1 == NULL_VALUE_64 || (desc->op == O_BETWEEN && v2 == NULL_VALUE_64))
  {  // any comparison with null values only
    desc->op = O_FALSE;
    return;
  }
  ...
  desc->val1 = CQTerm();
  desc->val1.vc = new ConstColumn(
      ValueOrNull(RCNum(v1, attr->Type().GetScale(), ATI::IsRealType(AttrTypeName()), AttrTypeName())), attr->Type());
  desc->val1.vc_id = desc->table->AddVirtColumn(desc->val1.vc);

  desc->val2 = CQTerm();
  desc->val2.vc = new ConstColumn(
      ValueOrNull(RCNum(v2, attr->Type().GetScale(), ATI::IsRealType(AttrTypeName()), AttrTypeName())), attr->Type());
...
void ConditionEncoder::TransformLIKEsIntoINsOnLookup()
{
  ...
  for (int i = 0; i < attr->dic->CountOfUniqueValues(); i++)
  {
    ...
    if (RequiresUTFConversions(desc->GetCollation()))
    {
      ...
      res = !wildcmp(desc->GetCollation(), s.val, s.val + s.len, pattern.val, pattern.val + pattern.len, desc->like_esc,
                     '_', '%');
    }
    else
...
void ConditionEncoder::TransformIntoINsOnLookup()
{
  ...
  if (desc->val1.vc->IsMultival() && static_cast<MultiValColumn &>(*desc->val1.vc).NoValues(mit) > 0)
  {
    ...
    else if ((desc->op == O_LESS_ANY || desc->op == O_LESS_EQ_ANY) ||
             (desc->op == O_MORE_ALL || desc->op == O_MORE_EQ_ALL))
...
void ConditionEncoder::TextTransformation()
{
  ...
  {  // lookup - transform into IN (numbers)
    if (desc->op == O_IN || desc->op == O_NOT_IN)
  ...
  else
  {
    if (no_dis_values == 1 && !mvc.ContainsNull(mit))
    {
      if (attr->PackType() == PackN && !attr->Type().IsLookup())
      {
        desc->val2 = CQTerm();
        desc->val2.vc = new ConstColumn(
            ValueOrNull(RCNum(attr->EncodeValue64(mvc.GetSetMin(mit), sharp), in_type.GetTypeName())), in_type);
        desc->val2.vc_id = desc->table->AddVirtColumn(desc->val2.vc);
      }
      else
      {
    ...
    else if (attr->PackType() == PackN && !mvc.ContainsNull(mit))
    {
      ...
      {
        desc->val2.vc =
            new ConstColumn(ValueOrNull(RCNum(val_max, attr->Type().GetScale(), ATI::IsRealType(in_type.GetTypeName()),
                                              in_type.GetTypeName())),
                            in_type);
        desc->val2.vc_id = desc->table->AddVirtColumn(desc->val2.vc);
        desc->val1.vc =
            new ConstColumn(ValueOrNull(RCNum(val_min, attr->Type().GetScale(), ATI::IsRealType(in_type.GetTypeName()),
                                              in_type.GetTypeName())),
                            in_type);
        desc->val1.vc_id = desc->table->AddVirtColumn(desc->val1.vc);
        if (desc->op == O_IN)
...
void ConditionEncoder::EncodeIfPossible(Descriptor &desc, bool for_rough_query, bool additional_nulls)
{
  ...
  if (desc.IsType_AttrAttr() && IsSimpleEqualityOperator(desc.op) && vcsc)
  {
    ...
    if (is_timestamp1 ||
        is_timestamp2 && !(is_timestamp1 && is_timestamp2))  // excluding timestamps compared with something else
      return;
  ...
  if (!encode_now)
  {
    ...
    {
      encode_now = (desc.IsType_AttrValOrAttrValVal() || desc.IsType_AttrMultiVal() || desc.op == O_IS_NULL ||
                    desc.op == O_NOT_NULL) &&
...
void ConditionEncoder::LookupExpressionTransformation()
{
  ...
  if (!null_positive)
  {
    ...
    desc->attr.vc = new SingleColumn(col, vcec->GetMultiIndex(), col_desc.var.tab, col_desc.col_ndx,
  ...
  {  // both nulls and not-nulls are positive - no single operator possible
    // do not encode
    desc->encoded = false;
  }
}
