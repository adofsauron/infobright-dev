...
class IndexTable : private CacheableItem, public TrackableObject
{
 public:
  ...
  // mem_modifier - default 0, may also be +1,2... or -1,2..., when positive => IndexTable will take more memory for a
  // buffer
  ...
  inline void Swap(_uint64 n1, _uint64 n2)
  {
    ...
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(IsLocked());
    BHASSERT_WITH_NO_PERFORMANCE_IMPACT(b1 ==
                                        int(n2 >> block_shift));  // Warning: otherwise serious performance problem
  ...
  {
    return orig_size;
  }  // the size of the original table, or the largest (incl. 0 = NULL) value which may occur in table
 ...
 private:
  ...
  _uint64 size;
  _int64
      orig_size;  // the size of the original table, or the largest value (incl. 0 = NULL) which may occur in the table
