...
enum SubSelectOptimizationType
{
  ROW_BASED,
  PACK_BASED
};
...
struct DescTreeNode
{
  DescTreeNode(LogicalOperator _lop, TempTable *t, int no_dims)
      : desc(t, no_dims), locked(0), left(NULL), right(NULL), parent(NULL)
  {
    desc.lop = _lop;
  }

  DescTreeNode(CQTerm e1, Operator op, CQTerm e2, CQTerm e3, TempTable *t, int no_dims, char like_esc)
      : desc(t, no_dims), locked(0), left(NULL), right(NULL), parent(NULL)
  {
    desc.attr = e1;
    desc.op = op;
...
class DescTree
{
 public:
  ...
  void AddDescriptor(LogicalOperator lop, CQTerm e1, Operator op, CQTerm e2, CQTerm e3, TempTable *t, int no_dims,
                     char like_esc);
