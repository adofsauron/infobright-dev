...
struct RSUsageStats
{
  ...
  void Clear() { all_packs = used_packs = pack_rsi = pack_rsr = 0; }
};
...
struct RSIndexID  // index identifier: index type plus index-dependent parameters (can be extended when needed)
{
  ...
  bool IsCorrect()
  {
    return type <= RSI_CMAP && (tab >= 0) && (col >= 0) && (IsType1() || ((tab2 >= 0) && (col2 >= 0)));
  }
  ...
  RSIndexID(RSIndexType t, int tb, int c)
  {
    type = t;
    tab = tb;
    col = c;
...
class RSI_Manager  // implements its own memory management based on requests of the general Memory Manager
{
 ...
 private:
  ...
  boost::shared_ptr<RSIndexPool>
      pool;  // definitions of all available indexes, with info which of them are loaded and/or locked
  IBMutex rsi_mutex;
  // pthread_mutex_t CS;
  int TryLock(RSIndexID id);
  void UnLock(int lock);
  RSIndex *LoadIndex(RSIndexID id, bool write, int read_loc);  // when write=true, file is created if needed; return
                                                               // NULL if file can't be loaded (e.g. already locked)
...
class RSIndex : public TrackableObject
{
 ...
 public:
  ...
  virtual void Load(IBFile *frs_index,
                    int current_loc) = 0;  // fd - file descriptor, open for binary reading (the file can be newly
                                           // created and empty - this shouldn't cause an error)
  virtual void LoadLastOnly(IBFile *frs_index,
                            int current_loc) = 0;             // read structure for update (do not create a full object)
...
class RSIndex_Test : public RSIndex
{
 public:
  ...
  char buf[SZ];
  void Load(IBFile *frs_index, int current_loc)
  {
    buf[frs_index->Read(buf, SZ - 1)] = 0;
    sscanf(buf, "%d", &time);
  }
  void LoadLastOnly(IBFile *frs_index, int current_loc)
  {
    buf[frs_index->ReadExact(buf, SZ - 1)] = 0;
    sscanf(buf, "%d", &time);
  }
