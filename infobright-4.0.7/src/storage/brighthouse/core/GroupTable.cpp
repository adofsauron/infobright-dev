...
GroupTable::GroupTable(GroupTable &sec)
{
  ...
  if (sec.input_buffer)
  {
    ...
    memcpy(input_buffer, sec.input_buffer,
           grouping_and_UTF_width);  // copy buffer, as it may already have some constants
  }
  else
    input_buffer = NULL;
...
void GroupTable::AddAggregatedColumn(VirtualColumn *vc, GT_Aggregation operation, bool distinct, AttributeType type,
                                     int size, int precision, DTCollation in_collation)
{
  GroupTable::ColTempDesc desc;
...
void GroupTable::Initialize(_int64 max_no_groups, bool parallel_allowed)
{
  ...
  for (int i = 0; i < no_attr; i++)
  {
    if (i < no_grouping_attr)
    {
      ...
      aggregator[i] = NULL;
      encoder[i] =
          new ColumnBinEncoder(ColumnBinEncoder::ENCODER_DECODABLE);  // TODO: not always decodable? (hidden cols)
  ...
  // Memory settings
  if (total_width < 1)
    total_width = 1;  // rare case: all constants, no actual buffer needed (but we create one anyway, just to avoid a
                      // special execution path for this boundary case)

  max_total_size = 64 * MBYTE;
  if (no_columns_with_distinct > 0 || max_no_groups * total_width > max_total_size ||
      (parallel_allowed && max_no_groups * total_width * 4 >
                               max_total_size))  // check if larger buffer is available to enable parallel aggr
  ...
  if (no_columns_with_distinct > 0)
  {
    ...
    if (primary_total_size <
        max_total_size - no_columns_with_distinct * GBYTE)  // additional limit: no more than 1 GB for every distincter
  ...
  if (grouping_buf_width == 2 && no_grouping_attr == 2 && encoder[1]->MaxCode() > 0)
    max_group_code =
        encoder[1]->MaxCode() * 256 +
        encoder[0]->MaxCode();  // wider than one-byte encoders are hard to interpret, because of endianess swap

  vm_tab = ValueMatchingTable::CreateNew_ValueMatchingTable(primary_total_size, declared_max_no_groups, max_group_code,
                                                            total_width, grouping_and_UTF_width, grouping_buf_width);
  ...
  for (int i = no_grouping_attr; i < no_attr; i++)
    if (distinct[i])
    {
      ...
      gdistinct[i]->InitializeVC(vm_tab->RowNumberScope(), vc[i],
  ...
  size_mb =
      (size_mb > 1000 ? (size_mb > 10000000 ? _int64(size_mb / 1000000) : _int64(size_mb / 1000) / 1000.0) : 0.001);
...
bool GroupTable::FindCurrentRow(
    _int64 &row)  // a position in the current GroupTable, row==NULL_VALUE_64 if not found and not added
