...
class Filter : public TrackableObject
{
 public:
  ...
  void ResetDelayed(_uint64 n) { ResetDelayed((int)(n >> 16), (int)(n & 65535)); }
  void SetDelayed(int b, int pos);
  void SetDelayed(_uint64 n) { SetDelayed((int)(n >> 16), (int)(n & 65535)); }
  ...
  void Reset();
  void Reset(_int64 n) { Reset((int)(n >> 16), (int)(n & 65535)); }
  ...
  bool IsFull() const;
  inline bool IsFull(int b) const
  {  // block b
  ...
  {
    track_changes = true;
    was_block_changed.resize(no_blocks);
  ...
  Block *GetBlock(int b) const
  {
    assert(b < no_blocks);
    return blocks[b];
  };
  uchar GetBlockStatus(int i)
  {
    assert(i < no_blocks);
 ...
 protected:
  ...
  bool track_changes;
  bool shallow;
  ushort *block_last_one;  // 65535, or a smaller number. Used only for FB_FULL: there is '1' on positions 0 -
 ...
 private:
  ...
  int delayed_stats;      // used for delayed Reset: -1 = initial value; -2 = delayed not possible; >=0 = actual set
                          // position
  int delayed_block;      // block with uncommitted (delayed) changes, or -1
  int delayed_stats_set;  // used for delayed Set: -1 = initial value; -2 = delayed not possible; >=0 = actual set
                          // position
 ...
 public:
  ...
  class Block
  {
   public:
    ...
    void Reset();
    inline bool Get(int n) { return (block_table[n >> 5] & lshift1[n & 31]); }
    ...
    void Store(IBFile &f);
    int BlockContinuous();  // return -1, or a number of continuous ones at the beginning of block (for FB_FULL
                            // optimization)
...
class FilterOnesIterator
{
 public:
  ...
  bool
  NextInsidePack();  // like ++, but rewind to pack beginning after its end. Return false if we just restarted the pack
  ...
  {
    return iterator_n == 65535 || (iterator_b == f->no_blocks - 1 && iterator_n == f->no_of_bits_in_last_block - 1) ||
           (cur_block_full && iterator_n >= f->block_last_one[iterator_b]);
  }

  _int64 GetPackSizeLeft();
  _int64 GetTotalSize() { return f->NoOnes(); }
...
class FilterOnesIteratorOrdered
    : public FilterOnesIterator  // iterating through all values 1 in a filter with given order of blocks (datapacks)
{
 public:
