...
Query::Query(ConnectionInfo *conn_info)
    : cq(NULL),
      ...
      capacity(5),
      last_join(NULL),
      notifier(Notifier::E_Query, conn_info),
      use_IBExpressions(true),
      parallel_aggr(true),
      m_conn(conn_info)
{
...
Query::~Query()
{
  for (expressions_t::iterator it = gc_expressions.begin(), end = gc_expressions.end(); it != end; ++it) delete *it;
  for (int i = (int)ta.size() - 1; i >= 0; i--) ta[i].reset();
  ta.clear();
  delete[] last_join;
  delete[] t;
}
...
void Query::Add(RCTable *tab)
{
  if (no_tab == capacity)
  {
    ...
    for (int i = 0; i < no_tab; i++) new_t[i] = t[i];
    delete[] t;
    t = new_t;
  }
  t[no_tab] = tab;
...
void Query::RemoveFromManagedList(const RCTablePtr tab) { ta.erase(std::remove(ta.begin(), ta.end(), tab), ta.end()); }

RCTable *Query::Table(int table_num) const
{
  if (table_num < 0 || table_num >= no_tab)
    return NULL;
  return t[table_num];
}
...
{
  for (int i = 0; i < no_tab; i++) t[i]->LockPackInfoForUse();
};

void Query::UnlockPackInfoFromUse()
{
  for (int i = 0; i < no_tab; i++) t[i]->UnlockPackInfoFromUse();
};
...
bool Query::IsCountStar(Item *item_sum)
{
  ...
  if (is)
    return (is->sum_func() == Item_sum::COUNT_FUNC) &&
           ((is->args[0]->type() == Item::INT_ITEM) || is->const_item() ||
...
*/					// end of disputable functions
      ...
      bool is_determ = true;
      for (uint i = 0; i < ifunc->argument_count(); i++) is_determ = is_determ && IsDeterministic(args[i]);
      return is_determ;
    }
...
bool Query::HasAggregation(Item *item)
{
  ...
  else if (i->type() == Item::FUNC_ITEM)
  {
    ...
    for (int arg = 0; (arg < arg_count) && !has; ++arg) has = HasAggregation(args[arg]);
  }
...
int Query::VirtualColumnAlreadyExists(const TabID &tmp_table, MysqlExpression *expression)
{
  ...
  for (tab_id2expression_t::const_iterator it = tab_id2expression.lower_bound(tmp_table),
                                           end = tab_id2expression.upper_bound(tmp_table);
       it != end; ++it)
  {
...
{
  int exists = NULL_VALUE_32;
  for (multimap<TabID, std::pair<int, TabID>>::const_iterator it = tab_id2subselect.lower_bound(tmp_table),
                                                              end = tab_id2subselect.upper_bound(tmp_table);
       it != end; ++it)
  {
...
{
  int exists = NULL_VALUE_32;
  for (tab_id2inset_t::const_iterator it = tab_id2inset.lower_bound(tmp_table),
                                      end = tab_id2inset.upper_bound(tmp_table);
       it != end; ++it)
  {
...
{
  for (multimap<pair<int, int>, std::pair<int, int>>::const_iterator
           it = phys2virt.lower_bound(make_pair(tab.n, at.n)),
           end = phys2virt.upper_bound(make_pair(tab.n, at.n));
       it != end; ++it)
  {
...
{
  return (op == O_EQ_ALL || op == O_EQ_ANY || op == O_NOT_EQ_ALL || op == O_NOT_EQ_ANY || op == O_LESS_ALL ||
          op == O_LESS_ANY || op == O_MORE_ALL || op == O_MORE_ANY || op == O_LESS_EQ_ALL || op == O_LESS_EQ_ANY ||
          op == O_MORE_EQ_ALL || op == O_MORE_EQ_ANY);
}
                                                 ...
                                                 int temp_table_alias, MultiIndex *mind)
{
  ...
  if (exprs.size() == 1)
  {
    ...
    {
      vc = BuildIBExpression(exprs[0]->GetItem(), ColumnType(exprs[0]->EvalType()), temp_table, temp_table_alias, mind,
                             false, true);
      ...
      if (rccontrol.isOn())
      {
        if (static_cast<int>(exprs[0]->GetItem()->type()) == Item::FUNC_ITEM)
        {
          ...
          rccontrol.lock(mind->m_conn->GetThreadID())
              << "Unoptimized expression near '" << ifunc->func_name() << "'" << unlock;
        }
      }
...
bool Query::SkipFix2FixConv(Item_func *item_func, ColumnType arg_type)
{
  ...
  return false;
#else
  bool is_arithm_op = (strcmp(item_func->func_name(), "+") == 0 || strcmp(item_func->func_name(), "-") == 0 ||
                       strcmp(item_func->func_name(), "*") == 0 || strcmp(item_func->func_name(), "/") == 0 ||
...
bool Query::IsLogicOperator(Item_func *item_func)
{
  ...
  return false;
#else
  return (strcmp(item_func->func_name(), "<") == 0 || strcmp(item_func->func_name(), ">") == 0 ||
          strcmp(item_func->func_name(), "<=") == 0 || strcmp(item_func->func_name(), ">=") == 0 ||
          strcmp(item_func->func_name(), "<>") == 0 || strcmp(item_func->func_name(), "=") == 0 ||
          strcmp(item_func->func_name(), " IN ") == 0 || strcmp(item_func->func_name(), "between") == 0);
#endif
}
