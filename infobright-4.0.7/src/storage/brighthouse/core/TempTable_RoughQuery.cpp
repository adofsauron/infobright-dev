...
void TempTable::RoughAggregateSum(VirtualColumn *vc, _int64 &min_val, _int64 &max_val, vector<Attr *> &group_by_attrs,
                                  bool nulls_only, bool distinct_present)
{
  ...
  if (!nulls_only && !is_const)
  {
    ...
    while (mit.IsValid())
    {
      ...
      for (int j = 0; j < group_by_attrs.size(); j++)
      {
        ...
        if (vc_gb == NULL || vc_gb->GetNoNulls(mit) != 0 || vc_gb->GetMinInt64(mit) == NULL_VALUE_64 ||
            vc_gb->GetMinInt64(mit) != vc_gb->GetMaxInt64(mit))
          no_groups_or_uniform =
              false;  // leave it true only when we are sure the grouping columns are uniform for this packrow
      }
  ...
  else if (success)
  {
    ...
    if (double_vals || (min_val_d > -9.223372037e+18 && max_val_d < 9.223372037e+18))
    {  // overflow check
      if (double_vals)
      {
        min_val = *(_int64 *)&min_val_d;
...
void TempTable::RoughAggregate(ResultSender *sender)
{
  ...
  // Assumptions:
  // filter.mind			- multiindex with nontrivial contents, although not necessarily updated by
  // conditions filter.rough_mind	- rough multiindex with more up-to-date contents than mind, i.e. a packrow may
  // exist in mind, but be marked as RS_NONE in rough_mind To check a rough status of a packrow, use both mind and
  // rough_mind. The method does not change mind / rough_mind.

  // Interpretation of the result:
  // Minimal and maximal possible value for a given column, if executed as exact. NULL if not known (unable to
  // determine).
  ...
  if (!aggregation_present || group_by_present)
  {  // otherwise even empty multiindex may produce nonempty result - checked later
  ...
  if (rough_is_empty == true || (mode.top && mode.param2 == 0))
  {  // empty or "limit 0"
    no_obj = 0;
    materialized = true;
    if (sender)
      ...
      filter.mind->NoDimensions() == 1)
  {
    ...
    if (order_by.size() > 0)
    {
      ...
      if (!vc->Type().IsString() && !vc->Type().IsLookup() && vc->GetDim() == 0)
      {
        ...
        po[0].Init(vc,
                   (asc ? PackOrderer::MaxAsc
                        : PackOrderer::MinDesc),  // start with best packs to possibly roughly exclude others
  ...
  for (uint i = 0; i < attrs.size(); i++)
  {
    ...
    if (vc && vc->IsConst() && !vc->IsSubSelect() && attrs[i]->mode != SUM && attrs[i]->mode != COUNT &&
        attrs[i]->mode != BIT_XOR)
    {
      if (attrs[i]->mode == STD_POP || attrs[i]->mode == VAR_POP || attrs[i]->mode == STD_SAMP ||
          attrs[i]->mode == VAR_SAMP)
      {
        attrs[i]->SetValueInt64(0, 0);  // deviations for constants = 0
        attrs[i]->SetValueInt64(1, 0);
      }
      else
      {  // other rough values for constants: usually just these constants
        MIIterator mit(filter.mind);
        if (vc->IsNull(mit))
        {
          attrs[i]->SetNull(0);
      ...
      switch (attrs[i]->mode)
      {
        ...
        case AVG:       // easy implementation of AVG: between min and max
          if (!vc->Type().IsString() && !vc->Type().IsLookup())
          {
            ...
            if (!double_vals && ATI::IsRealType(attrs[i]->TypeName()) && !nulls_only && min_val != NULL_VALUE_64)
            {  // decimal column, double result (e.g. for AVG)
            ...
            if (!nulls_only)
            {
              ...
              while (mit.IsValid())
              {
                ...
                if (res != RS_NONE && vc->GetPackOntologicalStatus(mit) != NULLS_ONLY)
                {
                  ...
                  {
                    _int64 exact_val = is_min ? vc->GetMinInt64Exact(mit) : vc->GetMaxInt64Exact(mit);
        ...
        case COUNT:
        {
          ...
          bool skip_counting = (IsTempTableColumn(vc) || SubqueryInFrom());
          if (vc && !attrs[i]->distinct && !skip_counting)
          {  // COUNT(a)
            int dim = vc->GetDim();
            if (dim != -1)
            {
          ...
          else if (vc && attrs[i]->distinct && !skip_counting)
          {  // COUNT(DISTINCT a)
            vc->MarkUsedDims(dims);
            MIIterator mit(filter.mind, dims);
            min_val = 0;
            max_val = vc->GetApproxDistVals(
                false, filter.rough_mind);  // Warning: mind used inside - may be more exact if rmind is also used
            ...
            if (!nulls_only)
            {
              ...
              for (int dim = 0; dim < dims.Size(); dim++)
                if (dims[dim])
                {
                  ...
                  max_count_star =
                      (max_count_star == NULL_VALUE_64 ? loc_max : SafeMultiplication(max_count_star, loc_max));
          ...
          if (!vc->Type().IsString() && !vc->Type().IsLookup() && vc->GetDim() != -1)
          {
            ...
            if (!nulls_only)
            {
              ...
              if (min_val != NULL_VALUE_64 && max_val != NULL_VALUE_64)
              {
                ...
                attrs[i]->SetValueInt64(1,
                                        *(_int64 *)(&diff));  // May be approximated better. For now Var <= difference^2
                value_set = true;
              }
              else
              {
