...
void TempTable::AttrBuffer<T>::Set(_int64 idx, T value)
{
  if (constant_value && value != CachedBuffer<T>::Get(0))
  {
    ...
    for (_int64 i = 1; i < idx; i++) CachedBuffer<T>::Set(i, val);
  }
  if (constant_value)
...
TempTable::Attr::Attr(CQTerm t, ColOperation m, bool dis, char *a, int dim, AttributeType type, uint scale,
                      uint no_digits, NullMode n, DTCollation collation, bool is_unsigned)
...
void TempTable::Attr::SetMinusInf(_int64 obj) { SetValueInt64(obj, MINUS_INF_64); }

void TempTable::Attr::SetPlusInf(_int64 obj) { SetValueInt64(obj, PLUS_INF_64); }
...
_uint64 TempTable::Attr::ApproxDistinctVals(
    bool incl_nulls, ::Filter *f, RSValue *rf,
    bool outer_nulls_possible)  // provide the best upper approximation of number of diff. values (incl. null)
{
  ...
  if (buffer)
  {
    switch (TypeName())
    {
      ...
      case RC_FLOAT:
        ...
        break;
      default:
        BHASSERT(0, "Attr::SetNewPageSize: unrecognized type");
        break;
    }
  }
...
TempTable::~TempTable()
{
  ...
  try
  {
    if (is_vc_owner)
    {
      for (int i = 0; i < virt_cols.size(); i++) delete virt_cols[i];
    }
    else
    {
      TranslateBackVCs();
...
TempTablePtr TempTable::CreateMaterializedCopy(
    bool translate_order, bool in_subq)  // move all buffers to a newly created TempTable, make this VC-pointing to it;
                                         // the new table must be deleted by DeleteMaterializedCopy()
{
  ...
  TempTablePtr working_copy = Create(
      *this, in_subq);  // Original VCs of this will be copied to working_copy, and then deleted in its destructor
  for (uint i = 0; i < attrs.size(); i++)
  {
  ...
  {  // Create() did move vc's used in order_by into working_copy, so we should bring them back
...
void TempTable::DeleteMaterializedCopy(
    TempTablePtr &old_t)  // delete the external table and remove VC pointers, make this fully materialized
{
  MEASURE_FET("TempTable::DeleteMaterializedCopy(...)");
  for (uint i = 0; i < attrs.size(); i++)
  {  // Make sure VCs are deleted before the source table is deleted
...
void TempTable::MoveVC(int colnum, std::vector<VirtualColumn *> &from, std::vector<VirtualColumn *> &to)
{
  ...
  for (int i = 0; i < vv.size(); i++) MoveVC(vv[i], from, to);
}
...
void TempTable::CreateDisplayableAttrP()
{
  ...
  for (int i = idx; i < attrs.size(); i++) displayable_attr[i] = NULL;
}
...
int TempTable::AddColumn(CQTerm e, ColOperation mode, char *alias, bool distinct)
{
  ...
  attrs.push_back(new Attr(e, mode, distinct, alias, -2, type, scale, precision, is_nullable,
                           e.vc ? e.vc->GetCollation() : DTCollation(), is_unsigned));
...
void TempTable::Union(TempTable *t, int all)
{
  ...
  if (!t)
  {  // trivial union with single select and external order by
    this->Materialize();
    return;
  }
  ...
  if (!all)
  {
    ...
    {  // block to ensure deleting encoder before deleting first_vcs, sec_vcs
      ...
      for (int i = 0; i < (int)NoDisplaybleAttrs(); i++)
      {
        ...
        sec_vcs.push_back(
            vc_ptr_t(new SingleColumn(t->GetDisplayableAttrP(i), t->GetOutputMultiIndexP(), 1, -i - 1, t, 0)));
      ...
      dist_table.InitializeB(
          size, NoObj() + t->NoObj() /
                              2);  // optimization assumption: a half of values in the second table will be repetitions
      ...
      do
      {
        ...
        while (first_mit.IsValid())
        {
          ...
          if (first_f.Get(pos))
          {
            for (int i = 0; i < encoder.size(); i++) encoder[i].Encode(input_buf, first_mit);
        ...
        while (sec_mit.IsValid())
        {
          ...
          if (sec_f.Get(pos))
          {
            for (int i = 0; i < encoder.size(); i++) encoder[i].Encode(input_buf, sec_mit, sec_vcs[i].get());
            GDTResult res = dist_table.Add(input_buf);
            if (res == GDT_EXISTS)
  ...
  for (uint i = 0; i < NoDisplaybleAttrs(); i++)
  {
    ...
    Attr *new_attr =
        new Attr(CQTerm(), LISTING, false, first_attr->alias, 0, new_type.GetTypeName(), new_type.GetScale(),
                 new_type.GetPrecision(), new_type.GetNullsMode(), new_type.GetCollation());
    new_attr->page_size = new_page_size;
    new_attr->CreateBuffer(new_no_obj, &m_conn);
    if (first_attr->TypeName() == RC_NUM && sec_attr->TypeName() == RC_NUM &&
...
_uint64 TempTable::ApproxAnswerSize(
    int attr, Descriptor &d)  // provide the most probable approximation of number of objects matching the condition
{
  // TODO: can it be done better?
  return no_obj / 2;
}
...
int TempTable::DimInDistinctContext()
{
  ...
  if (group_by_exists && aggregation_exists)  // group by with aggregations - not distinct context
    return -1;
  if (group_by_exists ||
      (mode.distinct && !aggregation_exists))  // select distinct a,b,c...; select a,b,c group by a,b,c
    return d;
  // aggregations exist - check their type
  for (uint i = 0; i < attrs.size(); i++)
    if (attrs[i] && attrs[i]->mode != MIN && attrs[i]->mode != MAX && !attrs[i]->distinct)
      return -1;
  return d;
}
...
{
  while (lazy && no_materialized < min(((_int64)pack_no << 16) + 0x10000, no_obj)) Materialize(false, NULL, true);
}
...
void TempTable::Materialize(bool in_subq, ResultSender *sender, bool lazy)
{
  ...
  if (materialized && (order_by.size() > 0 || limits_present) && no_obj)
  {
    ...
    {
      TempTablePtr temporary_source_table =
...
bool TempTable::IsFullyMaterialized() { return materialized && ((order_by.size() == 0 && !mode.top) || !no_obj); }

RCDataType &TempTable::Record::operator[](uint i) const { return *(_it.dataTypes[i]); }
  ...
  if (_currentRNo < _uint64(table->NoObj()))
  {
    ...
    for (uint att = 0; att < no_disp_attr; ++att)
    {
      ...
      else
      {
        ...
        (*(TempTable::AttrBuffer<RCBString> *)table->GetDisplayableAttrP(att)->buffer)
            .GetString(*(RCBString *)dataTypes[att], _currentRNo);
      }
    }
  }
}
...
TempTable::RecordIterator TempTable::begin(ConnectionInfo *conn) { return (RecordIterator(this, conn, 0)); }

TempTable::RecordIterator TempTable::end(ConnectionInfo *conn) { return (RecordIterator(this, conn, NoObj())); }

TempTable::RecordIterator::RecordIterator(void) : table(NULL), _currentRNo(0), _conn(NULL), is_prepared(false) {}
...
{
  for (std::vector<RCDataType *>::const_iterator it = dataTypes.begin(); it != dataTypes.end(); it++) delete (*it);
  dataTypes.clear();
}
...
TempTableForSubquery::~TempTableForSubquery()
{
  ...
  SetAttrsForExact();
  for (int i = 0; i < attrs_for_rough.size(); i++) delete attrs_for_rough[i];
  for (int i = 0; i < template_attrs.size(); i++) delete template_attrs[i];
}
...
void TempTableForSubquery::SetAttrsForRough()
{
  ...
  {
    for (int i = (int)attrs_for_exact.size(); i < attrs.size(); i++) delete attrs[i];
...
void TempTableForSubquery::SetAttrsForExact()
{
  if (is_attr_for_rough)
  {
    for (int i = (int)attrs_for_rough.size(); i < attrs.size(); i++) delete attrs[i];
...
void TempTableForSubquery::CreateTemplateIfNotExists()
{
  if (attrs_for_rough.size() == 0)
  {
    ...
    is_attr_for_rough = false;
    for (int i = 0; i < attrs.size(); i++) attrs_for_rough.push_back(new Attr(*attrs[i]));
  }
  if (!template_filter && IsParametrized())
  {
    template_filter = new ParameterizedFilter(filter);
    for (int i = 0; i < attrs.size(); i++) template_attrs.push_back(new Attr(*attrs[i]));
    no_global_virt_cols = int(virt_cols.size());
