...
struct ColumnType
{
 public:
  ...
  ColumnType(AttributeType t, NullMode nulls = AS_MISSED, bool lookup = false, int prec = 0, int sc = 0,
  ...
  void Initialize(AttributeType t, NullMode nulls, bool lookup, int prec, int sc, DTCollation collation = DTCollation(),
