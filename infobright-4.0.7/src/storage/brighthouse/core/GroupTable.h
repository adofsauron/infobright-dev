...
class GroupTable : public TrackableObject
{
 public:
  ...
  void AddAggregatedColumn(VirtualColumn *vc, GT_Aggregation operation, bool distinct, AttributeType type, int b_size,
                           int precision, DTCollation in_collation);
  void AggregatedColumnStatistics(int ag_col, _int64 max_no_vals, _int64 min_v = MINUS_INF_64,
  ...
  void Initialize(_int64 max_no_groups,
                  bool parallel_allowed);  // initialize buffers basing on the previously defined grouping/aggregated
                                           // columns and the best possible upper approximation of number of groups;
  ...
  bool PutCachedValue(int col, GroupDistinctCache &cache,
                      bool as_text);  // mainly for numerics, and only some aggregators
  ...
  bool AttrMayBeUpdatedByPack(
      int col, MIIterator &mit);           // false if a grouping attribute pack is out of scope of the current contents
 ...
 private:
  ...
  int *aggregated_col_offset;  // a table of byte offsets of aggregated column beginnings wrt. "grouping_and_UTF_width"
                               // position
  ...
  bool initialized;
  struct ColTempDesc  // temporary description - prior to buffers initialization, then sometimes used for complex
                      // aggregations
  {
  ...
  GT_Aggregation
      *operation;  // Note: these tables will be created in Initialize(); using vectors inside grouper may be too slow
  bool *distinct;
