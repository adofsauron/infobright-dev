...
class DataCache
{
 protected:
  struct IOPackReq
  {
    ...
    size_t operator()(IOPackReq const &o) const { return (o.coord.hash() * (_uint64)at); }
    bool operator<(IOPackReq const &o) const { return (coord < o.coord); }
  };
 ...
 public:
  ...
  DataCache(bool local = true)
      : m_cacheHits(0),
        m_cacheMisses(0),
        m_objectsReleased(0),
        m_readWait(0),
        m_falseWakeup(0),
        m_readWaitInProgress(0),
        m_packLoads(0),
        m_packLoadInProgress(0),
        m_loadErrors(0),
        m_reDecompress(0),
        m_local(local)
  {
