...
void PrintItemTree(Item *item, int indent)
{
  ...
  fprintf(stderr, "%s %s @%p %s %s max_length=%d", name, item->full_name(), (void *)item, FieldType(item->field_type()),
          result, item->max_length);

  if (item->result_type() == DECIMAL_RESULT)
    fprintf(stderr, " [prec=%d,dec=%d,int=%d]", item->decimal_precision(), (int)item->decimals,
  ...
  switch (type)
  {
    ...
    case Item::SUM_FUNC_ITEM:
    {
      ...
      for (uint i = 0; i < args_count; i++) PrintItemTree(args[i], indent);
      return;
    }
    case Item::REF_ITEM:
    {
