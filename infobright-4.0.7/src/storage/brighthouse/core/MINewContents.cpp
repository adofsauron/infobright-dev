...
void MINewContents::Init(_int64 initial_size)  // initialize temporary structures (set approximate size)
{
  ...
  for (int dim = 0; dim < no_dims; dim++)
  {
    if (dim_involved[dim] && !forget_now[dim])
    {
      ...
      {  // more than one unforgotten found
...
void MINewContents::CommitCountOnly(
    ...
    _int64 joined_tuples)  // commit changes to multiindex - must be called at the end, or changes will be lost
{
  ...
  {  // optimized version: just exchange filters
    DimensionGroupFilter *nf =
        new DimensionGroupFilter(optimized_dim_stay, f_opt, 2);  // mode 2: pass Filter ownership to the DimensionGroup
    f_opt = NULL;
  ...
  {  // optimized version: virtual dimension group
...
void MINewContents::CommitNewTableValues()  // set a value (NULL_VALUE_64 is a null object index); if the index is
                                            // larger than the current size, enlarge table automatically
{
  if (content_type == MCT_FILTER_FORGET)
  {
    _int64 val = new_value[optimized_dim_stay];
    if (val == NULL_VALUE_64 || (optimized_dim_stay != ignore_repetitions_dim && f_opt->Get(val)))
    {
  ...
  {
    _int64 val = new_value[optimized_dim_stay];
    if (val == NULL_VALUE_64 || val <= max_filter_val)
    {  // can only forward iterate in virtual case
      // repetition or null object found
      DisableOptimized();
      CommitNewTableValues();  // again, content_type is changed now
      return;
  ...
  if (content_type != MCT_FILTER_FORGET)
  {
    ...
    {
      if (t_new[dim])
      {  // == !forget_now[dim] && dim_involved[dim] && optimized_dim_stay != dim
